public class Compute {
  private static void mxmKernel(
      KernelContext context, Matrix2DFloat A, Matrix2DFloat B, Matrix2DFloat C, final int size) {
  ...
  public void run(Matrix2DFloat A, Matrix2DFloat B, Matrix2DFloat C, final int size) {
    ...
    GridScheduler gridScheduler =
        new GridScheduler("s0.t0", workerGrid); // Attach the worker to the Grid
    ...
    TaskGraph taskGraph =
        new TaskGraph("s0")
            .transferToDevice(
                ...
                A,
                B) // Transfer data from host to device only during the first execution
            .task(
                "t0",
                Compute::mxmKernel,
                context,
                ...
                B,
                C,
                size) // Each task points to an existing Java method
            .transferToHost(
                DataTransferMode.EVERY_EXECUTION,
                C); // Transfer data from device to host in every execution.
