...
public class TornadoReduceReplacement extends BasePhase<TornadoSketchTierContext> {
  ...
  /**
   * It checks if there is a reduction in the IR. For now it checks simple reductions. It assumes
   * that the value to store is a binary arithmetic and then load index. As soon as we discover more
   * cases, new nodes should be inspected here.
   *
   * <p>Cover all the cases here as soon as we discover more reductions use-cases.
   *
   * @param arrayToStore Array to store
   * @param indexToStore Index used in the store array
   * @param currentNode Current node to be inspected
   * @return boolean
   */
  private boolean recursiveCheck(
      ValueNode arrayToStore, ValueNode indexToStore, ValueNode currentNode) {
  ...
  private ReductionMetadataNode createReductionNode(
      StructuredGraph graph, Node store, ValueNode inputArray, ValueNode startNode)
    ...
    if (storeValue instanceof AddNode addNode) {
      final TornadoReduceAddNode atomicAdd =
          graph.addOrUnique(new TornadoReduceAddNode(addNode.getX(), addNode.getY()));
      accumulator = addNode.getY();
      value = atomicAdd;
      addNode.safeDelete();
    } else if (storeValue instanceof MulNode mulNode) {
      final TornadoReduceMulNode atomicMultiplication =
          graph.addOrUnique(new TornadoReduceMulNode(mulNode.getX(), mulNode.getY()));
      accumulator = mulNode.getX();
      value = atomicMultiplication;
      mulNode.safeDelete();
    } else if (storeValue instanceof SubNode subNode) {
      final TornadoReduceSubNode atomicSub =
          graph.addOrUnique(new TornadoReduceSubNode(subNode.getX(), subNode.getY()));
      ...
      if (storeValue instanceof MarkFloatingPointIntrinsicsNode
          || storeValue instanceof MarkIntIntrinsicNode) {
  ...
  private void performNodeReplacement(
      StructuredGraph graph,
      FixedWithNextNode node,
      Node predecessor,
      ReductionMetadataNode reductionNode,
      ValueNode outArray) {
    ...
    if (node instanceof StoreIndexedNode store) {
      StoreAtomicIndexedNodeExtension storeAtomicIndexedNodeExtension =
          new StoreAtomicIndexedNodeExtension(startNode);
      graph.addOrUnique(storeAtomicIndexedNodeExtension);
      storeNode =
          graph.addOrUnique(
              new StoreAtomicIndexedNode(
                  store.array(),
                  store.index(),
                  store.elementKind(), //
                  store.getBoundsCheck(),
                  value,
                  accumulator,
                  inputArray,
                  storeAtomicIndexedNodeExtension));
      ...
      storeNode =
          graph.addOrUnique(
              new WriteAtomicNode(
                  javaWriteNode.getWriteKind(),
                  javaWriteNode.getAddress(),
                  value,
                  accumulator,
                  inputArray,
                  outArray,
                  writeAtomicNodeExtension));
    }
      ...
      if (node instanceof StoreIndexedNode store) {
        ...
        for (OffsetAddressNode offsetAddressNode : node.usages().filter(OffsetAddressNode.class)) {
          if (offsetAddressNode.usages().filter(JavaWriteNode.class).isNotEmpty()) {
            JavaWriteNode javaWriteNode =
                offsetAddressNode.usages().filter(JavaWriteNode.class).first();
            // follow these steps but adapted to javaWriteNode
            ParameterNode parameterNode = node.inputs().filter(ParameterNode.class).first();
            boolean isReductionValue =
                recursiveCheck(parameterNode, offsetAddressNode, javaWriteNode.value());
            if (!isReductionValue) {
              continue;
            }
      ...
      for (Annotation annotation : parameterAnnotations[index]) {
        if (annotation instanceof Reduce) {
          ...
          if (!graph.method().isStatic()
              || getNumberOfParameterNodes(graph) > parameterAnnotations.length) {
            index++;
          }
  ...
  private record ReductionMetadataNode(
      ValueNode value, ValueNode accumulator, ValueNode inputArray, ValueNode startNode) {}
}
