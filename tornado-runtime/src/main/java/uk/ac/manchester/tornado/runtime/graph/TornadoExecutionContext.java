...
public class TornadoExecutionContext {
  ...
  public int insertVariable(Object parameter) {
    ...
    if (parameter.getClass().isPrimitive()
        || RuntimeUtilities.isBoxedPrimitiveClass(parameter.getClass())) {
      ...
      if (parameter.getClass().isArray()) {
        ...
        if (dataTypeSize == null) {
          throw new TornadoRuntimeException(
              "[UNSUPPORTED] Data type not supported for processing in batches");
        }
  ...
  public int replaceVariable(Object oldObj, Object newObj) {
     ...
     */
    int index;
    if (oldObj.getClass().isPrimitive()
        || RuntimeUtilities.isBoxedPrimitiveClass(oldObj.getClass())) {
   ...
   *
   * @param tornadoDevice The {@link TornadoDevice} to which all tasks will be mapped.
   * @throws RuntimeException if the current device is not supported.
   */
  public void mapAllTasksToSingleDevice(TornadoDevice tornadoDevice) {
    if (tornadoDevice instanceof TornadoXPUDevice tornadoAcceleratorDevice) {
      ...
      throw new TornadoRuntimeException(
          "Device " + tornadoDevice.getClass() + " not supported yet");
    }
  }
  ...
  /**
   * It assigns a task to a {@link TornadoXPUDevice} based on the current task scheduling strategy.
   *
   * @param index The index of the task.
   * @param task The {@link SchedulableTask} to be assigned.
   */
  ...
  /**
   * It calculates the number of valid contexts. A valid context refers to a context that is not
   * null within the list of devices. This behavior is caused in the ExecutionContext that does not
   * append devices sequentially, but they are placed in an order/index to preserve their original
   * order in the driver.
   *
   ...
   *
   * @return {@code true} if the tasks are mutually independent, {@code false} otherwise.
   */
  ...
  /**
   * It retrieves a list of tasks for a specific device and driver. Both deviceContext and
   * backendIndex are checked to ensure the correct task assignment.
   *
   * @param deviceContext The device context of the device.
   * @return A list of {@link SchedulableTask} objects associated with the specified device and
   *     driver.
   */
