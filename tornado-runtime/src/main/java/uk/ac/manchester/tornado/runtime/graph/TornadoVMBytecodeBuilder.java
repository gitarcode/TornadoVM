...
public class TornadoVMBytecodeBuilder {
  ...
  public static final int MAX_TORNADO_VM_BYTECODE_SIZE =
      Integer.parseInt(getProperty("tornado.tvm.maxbytecodesize", "4096"));
  ...
  /**
   * It constructs a new TornadoVMBytecodeBuilder instance. Initializes the byte array to hold the
   * bytecode with the maximum bytecode size. Initializes the TornadoVMBytecodeAssembler with the
   * byte array.
   */
  ...
  void emitAsyncNode(
      AbstractNode node, int dependencyBC, long offset, long batchSize, long nThreads) {
    if (node instanceof AllocateMultipleBuffersNode) {
      ...
      bitcodeASM.transferToDeviceOnce(
          ((CopyInNode) node).getValue().getIndex(), dependencyBC, offset, batchSize);
    } else if (node instanceof AllocateNode) {
      new TornadoLogger()
          .info(
              "[%s]: Skipping deprecated node %s",
              getClass().getSimpleName(), AllocateNode.class.getSimpleName());
      ...
      bitcodeASM.transferToDeviceAlways(
          ((StreamInNode) node).getValue().getIndex(), dependencyBC, offset, batchSize);
      ...
      bitcodeASM.launch(
          taskNode.getContext().getDeviceIndex(),
          taskNode.getTaskIndex(),
          taskNode.getNumArgs(),
          dependencyBC,
          offset,
          nThreads);
