...
public class TornadoGraphBuilder {
  ...
  private static void createStreamInNode(
      ContextNode context,
      TornadoGraph graph,
      ObjectNode arg,
      AbstractNode[] args,
      int argIndex,
      AllocateMultipleBuffersNode persistNode) {
  ...
  private static void createAllocateNode(
      ContextNode context,
      TornadoGraph graph,
      AbstractNode arg,
      AbstractNode[] args,
  ...
  private static void createCopyInNode(
      ContextNode context,
      TornadoGraph graph,
      AbstractNode arg,
      AbstractNode[] args,
  ...
  private static boolean shouldPerformSharedObjectCopy(AbstractNode arg, ContextNode contextNode) {
    return ((ContextOpNode) arg).getContext().getUses().size() != 1
        && contextNode.getDeviceIndex() != ((ContextOpNode) arg).getContext().getDeviceIndex();
  }

  /**
   * It constructs a {@link TornadoGraph} from the provided {@link TornadoExecutionContext} and
   * ByteBuffer.
   *
   * @param executionContext The {@link TornadoExecutionContext} that contains the context of the
   *     graph.
   * @param buffer The {@link ByteBuffer} containing the bytecode representation of the graph.
   * @return The constructed {@link TornadoGraph}.
   */
  public static TornadoGraph buildGraph(
      TornadoExecutionContext executionContext, ByteBuffer buffer) {
    ...
    while (!shouldExit && buffer.hasRemaining()) {
      ...
      if (op == TornadoGraphBitcodes.ARG_LIST.index()) {
        ...
        if (accesses[argIndex] == Access.WRITE_ONLY || accesses[argIndex] == Access.READ_WRITE) {
          ...
          if (objectNodes[variableIndex] instanceof ObjectNode) {
            ...
            throw new TornadoRuntimeException(
                "Invalid graph node in TornadoGraph builder for node: "
                    + objectNodes[variableIndex].getClass().getName());
          }
        ...
        context =
            graph.addUnique(
                new ContextNode(
                    executionContext.getDevices().indexOf(deviceForTask), deviceForTask));
