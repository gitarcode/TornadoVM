...
/**
 * It presents the configuration for processing data in batches. This class provides methods to
 * compute chunk sizes based on the batch size and input objects.
 */
/**
 * How to test?
 *
...
public class BatchConfiguration {
  ...
  public static BatchConfiguration computeChunkSizes(
      TornadoExecutionContext context, long batchSize) {
      ...
      if (o.getClass().isArray()) {
        ...
        byte elementSize =
            switch (tornadoNativeArray) {
              ...
              default ->
                  throw new TornadoRuntimeException("Unsupported array type: " + o.getClass());
            };
    ...
    if (inputSizes.size() > 1) {
      throw new TornadoRuntimeException(
          "[UNSUPPORTED] Input objects with different sizes not currently supported");
    }

    if (elementSizes.size() > 1) {
      throw new TornadoRuntimeException(
          "[UNSUPPORTED] Input objects with different element sizes not currently supported");
    }
