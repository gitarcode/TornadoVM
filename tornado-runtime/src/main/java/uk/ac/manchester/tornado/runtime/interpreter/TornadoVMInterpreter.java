...
/**
 * TornadoVMInterpreter: serves as a bytecode interpreter for TornadoVM bytecodes. Also, it
 * functions as a memory manager for various devices, including FPGAs, GPUs, and multicore
 * processors that adhere to any of the supported programming models. Additionally, it features a
 * Just-In-Time (JIT) compiler that compiles Java bytecode to OpenCL, PTX, and SPIR-V.
 */
public class TornadoVMInterpreter {
   ...
   *
   * @param graphExecutionContext The {@link TornadoExecutionContext}
   * @param bytecodeResult The {@link TornadoVMBytecodeResult}.
   * @param timeProfiler The {@link TornadoProfiler} for time measurements.
   * @param device The {@link TornadoXPUDevice} device.
   */
  public TornadoVMInterpreter(
      TornadoExecutionContext graphExecutionContext,
      TornadoVMBytecodeResult bytecodeResult,
      TornadoProfiler timeProfiler,
  ...
  private void rewindBufferToBegin() {
    ...
    while (op != TornadoVMBytecodes.BEGIN.value()) {
      TornadoInternalError.guarantee(
          op == TornadoVMBytecodes.CONTEXT.value(), "invalid code: 0x%x", op);
  ...
  public void printTimes() {
    System.out.printf(
        "bc: complete %d iterations - %.9f s mean and %.9f s total%n",
    ...
    for (int i = profiles.nextSetBit(0); i != -1; i = profiles.nextSetBit(i + 1)) {
      if (eventSet.getDevice() instanceof TornadoXPUDevice device) {
        ...
        if (profile.getStatus() == COMPLETE) {
          System.out.printf(
              "task: %s %s %9d %9d %9d %9d %9d%n",
              device.getDeviceName(),
              meta.getId(),
              profile.getElapsedTime(),
              profile.getQueuedTime(),
              profile.getSubmitTime(),
  ...
  public void dumpProfiles() {
    for (final SchedulableTask task : taskExecutionContexts) {
      ...
      meta.getProfiles(graphExecutionContext.getExecutionPlanId())
          .forEach(eventSet -> dumpEventProfiled(eventSet, meta));
    }
  }
  ...
  private Event execute(boolean isWarmup) {
    ...
    if (isMemoryLimitEnabled() && graphExecutionContext.doesExceedExecutionPlanLimit()) {
      throw new TornadoMemoryException(
          "OutofMemoryException due to executionPlan.withMemoryLimit of "
              + graphExecutionContext.getExecutionPlanMemoryLimit());
    }
    ...
    if (TornadoOptions.PRINT_BYTECODES) {
      ...
      tornadoVMBytecodeList
          .append(
              InterpreterUtilities.debugHighLightHelper(
                  "Interpreter instance running bytecodes for: "))
          .append(interpreterDevice)
          .append(InterpreterUtilities.debugHighLightHelper(" Running in thread: "))
    ...
    while (bytecodeResult.hasRemaining()) {
      ...
      if (op == TornadoVMBytecodes.ALLOC.value()) {
        ...
        transferHostToDeviceOnce(
            tornadoVMBytecodeList, objectIndex, offset, eventList, sizeBatch, waitList);
        ...
        XPUExecutionFrame info =
            compileTaskFromBytecodeToBinary(
                callWrapperIndex, numArgs, eventList, taskIndex, batchThreads);
        if (isWarmup) {
        ...
        lastEvent =
            executeLaunch(
                tornadoVMBytecodeList, numArgs, eventList, taskIndex, batchThreads, offset, info);
        ...
        if (!isWarmup && TornadoOptions.PRINT_BYTECODES) {
          tornadoVMBytecodeList
              .append("bc: ")
              .append(InterpreterUtilities.debugHighLightBC("END\n"))
              .append("\n");
        }
        break;
      } else {
    ...
    if (!isWarmup) {
      if (useDependencies) {
        final int event =
            interpreterDevice.enqueueMarker(graphExecutionContext.getExecutionPlanId());
    ...
    if (graphExecutionContext.meta().isDebug()) {
      logger.debug(
          "bc: complete elapsed=%.9f s (%d iterations, %.9f s mean)",
    ...
    if (TornadoOptions.isProfilerEnabled()) {
      ...
      for (XPUDeviceBufferState objectState : objectStates) {
        timeProfiler.addValueToMetric(
            ProfilerType.ALLOCATION_BYTES,
            TimeProfiler.NO_TASK_NAME,
            objectState.getXPUBuffer().size());
      }
    }
  ...
  private int executeDeAlloc(StringBuilder tornadoVMBytecodeList, final int objectIndex) {
    ...
    graphExecutionContext.setCurrentDeviceMemoryUsage(
        graphExecutionContext.getCurrentDeviceMemoryUsage() - spaceDeallocated);
    return -1;
  }

  private void transferHostToDeviceOnce(
      StringBuilder tornadoVMBytecodeList,
      final int objectIndex,
      final long offset,
      final int eventList,
      final long sizeBatch,
      final int[] waitList) {
    Object object = objects.get(objectIndex);
    ...
    if (sizeBatch > 0) {
      ...
      allEvents =
          interpreterDevice.ensurePresent(
              graphExecutionContext.getExecutionPlanId(),
              object,
              objectState,
              waitList,
    ...
    if (TornadoOptions.PRINT_BYTECODES && isNotObjectAtomic(object)) {
      DebugInterpreter.logTransferToDeviceOnce(
          allEvents,
          object,
          interpreterDevice,
          sizeBatch,
    ...
    if (TornadoOptions.isProfilerEnabled() && allEvents != null) {
      for (Integer e : allEvents) {
        ...
        timeProfiler.addValueToMetric(
            ProfilerType.TOTAL_COPY_IN_SIZE_BYTES,
            TimeProfiler.NO_TASK_NAME,
            objectState.getXPUBuffer().size());
  ...
  private void transferHostToDeviceAlways(
      StringBuilder tornadoVMBytecodeList,
      final int objectIndex,
      final long offset,
      final int eventList,
    ...
    if (TornadoOptions.PRINT_BYTECODES && isNotObjectAtomic(object)) {
      DebugInterpreter.logTransferToDeviceAlways(
          object, interpreterDevice, sizeBatch, offset, eventList, tornadoVMBytecodeList);
    }
  ...
  private int transferDeviceToHost(
      StringBuilder tornadoVMBytecodeList,
      final int objectIndex,
      final long offset,
      final int eventList,
      ...
      String verbose =
          String.format(
              "bc: "
                  ...
                  + "[0x%x] %s on %s, size=%d, offset=%d [event list=%d]",
              object.hashCode(),
              object,
              InterpreterUtilities.debugDeviceBC(interpreterDevice),
              sizeBatch,
    ...
    final XPUDeviceBufferState objectState = resolveObjectState(objectIndex);
    int readEvent =
        interpreterDevice.streamOutBlocking(
            graphExecutionContext.getExecutionPlanId(), object, offset, objectState, waitList);

    resetEventIndexes(eventList);

    if (TornadoOptions.isProfilerEnabled() && readEvent != -1) {
      Event event =
          interpreterDevice.resolveEvent(graphExecutionContext.getExecutionPlanId(), readEvent);
      event.waitForEvents(graphExecutionContext.getExecutionPlanId());
      ...
      timeProfiler.addValueToMetric(
          ProfilerType.TOTAL_COPY_OUT_SIZE_BYTES,
          TimeProfiler.NO_TASK_NAME,
          objectState.getXPUBuffer().size());
  ...
  private void transferDeviceToHostBlocking(
      StringBuilder tornadoVMBytecodeList,
      final int objectIndex,
      final long offset,
      final int eventList,
  ...
  private boolean isRecompilationNeededForLastBatch(
      int taskIndex, SchedulableTask task, long batchThreads) {
    return (!shouldCompile(installedCodes[globalToLocalTaskIndex(taskIndex)])
        && task.getBatchThreads() != 0
        && task.getBatchThreads() != batchThreads);
  }
  ...
  private void updateBatchThreads(
      SchedulableTask task, long batchThreads, boolean indexInWrite, int currentBatch) {
  ...
  private void updateMeta(TaskContextInterface meta) {
    ...
    meta.setCompilerFlags(
        TornadoVMBackendType.OPENCL,
        graphExecutionContext.meta().getCompilerFlags(TornadoVMBackendType.OPENCL));
    meta.setCompilerFlags(
        TornadoVMBackendType.PTX,
        graphExecutionContext.meta().getCompilerFlags(TornadoVMBackendType.PTX));
    meta.setCompilerFlags(
        TornadoVMBackendType.SPIRV,
        graphExecutionContext.meta().getCompilerFlags(TornadoVMBackendType.SPIRV));
  }

  private XPUExecutionFrame compileTaskFromBytecodeToBinary(
      final int callWrapperIndex,
      final int numArgs,
      final int eventList,
      final int taskIndex,
      final long batchThreads) {

    if (interpreterDevice.getDeviceContext().wasReset() && finishedWarmup) {
      throw new TornadoFailureException(
          "[ERROR] reset() was called after warmup() on device: " + interpreterDevice + "!");
    }

    boolean redeployOnDevice = graphExecutionContext.redeployOnDevice();

    final KernelStackFrame callWrapper =
        resolveCallWrapper(
            callWrapperIndex, numArgs, kernelStackFrame, interpreterDevice, redeployOnDevice);
    ...
    if (isRecompilationNeededForLastBatch(taskIndex, task, batchThreads)
        || currentBatchUsesThreadId(currentBatch, indexInWrite)) {
    ...
    if (timeProfiler instanceof TimeProfiler) {
      ...
      timeProfiler.registerDeviceID(
          task.getId(), task.meta().getBackendIndex() + ":" + task.meta().getDeviceIndex());
      timeProfiler.registerDeviceName(
          task.getId(), task.getDevice().getPhysicalDevice().getDeviceName());
    }
    ...
    if (shouldCompile(installedCodes[globalToLocalTaskIndex(taskIndex)])) {
      ...
      try {
        ...
        throw new TornadoBailoutRuntimeException(
            "[Internal Error] Unable to compile "
                + task.getFullName()
                + "\n"
                + Arrays.toString(e.getStackTrace()));
      }
  ...
  private int executeLaunch(
    ...
    atomicsArray =
        (task instanceof PrebuiltTask prebuiltTask)
            ? prebuiltTask.getAtomics()
            : interpreterDevice.checkAtomicsForTask(task);
    ...
    for (int i = 0; i < numArgs; i++) {
      ...
      if (argType == TornadoVMBytecodes.PUSH_CONSTANT_ARGUMENT.value()) {
        ...
        final XPUDeviceBufferState objectState =
            globalState.getDeviceBufferState(interpreterDevice);
        ...
        if (!isObjectInAtomicRegion(objectState, interpreterDevice, task)) {
          ...
          atomicsArray =
              interpreterDevice.updateAtomicRegionAndObjectState(
                  task, atomicsArray, i, objects.get(argIndex), objectState);
        }
      } else {
    ...
    if (atomicsArray != null) {
      ...
      List<Integer> allEvents =
          bufferAtomics.enqueueWrite(
              graphExecutionContext.getExecutionPlanId(), null, 0, 0, null, false);
      if (TornadoOptions.isProfilerEnabled()) {
        for (Integer e : allEvents) {
          Event event =
              interpreterDevice.resolveEvent(graphExecutionContext.getExecutionPlanId(), e);
          event.waitForEvents(graphExecutionContext.getExecutionPlanId());
        ...
        String verbose =
            String.format(
                "bc: "
                    ...
                    + "  ATOMIC [0x%x] %s on %s, size=%d, offset=%d [event list=%d]",
    ...
    if (TornadoOptions.PRINT_BYTECODES) {
      String verbose =
          String.format(
              "bc: "
                  + InterpreterUtilities.debugHighLightBC("LAUNCH")
                  + " %s on %s, size=%d, offset=%d [event list=%d]",
              task.getFullName(),
              interpreterDevice,
              batchThreads,
              offset,
    ...
    if (task.meta() instanceof TaskDataContext dataContext) {
      ...
      try {
        int lastEvent =
            useDependencies
                ? installedCode.launchWithDependencies(
                    graphExecutionContext.getExecutionPlanId(),
                    stackFrame,
                    bufferAtomics,
                    dataContext,
                    batchThreads,
                    waitList)
                : installedCode.launchWithoutDependencies(
                    graphExecutionContext.getExecutionPlanId(),
                    stackFrame,
                    bufferAtomics,
                    dataContext,
  ...
  private void executeDependency(
      StringBuilder tornadoVMBytecodeList, int lastEvent, int eventList) {
    if (useDependencies && lastEvent != -1) {
      ...
      TornadoInternalError.guarantee(
          eventsIndexes[eventList] < events[eventList].length, "event list is too small");
  ...
  private int executeBarrier(StringBuilder tornadoVMBytecodeList, int eventList, int[] waitList) {
    if (TornadoOptions.PRINT_BYTECODES) {
      tornadoVMBytecodeList.append(
          String.format(
              "bc: " + InterpreterUtilities.debugHighLightBC("BARRIER") + " event-list %d%n",
              eventList));
    }
  ...
  private KernelStackFrame resolveCallWrapper(
      int index,
      int numArgs,
      KernelStackFrame[] kernelStackFrame,
      TornadoXPUDevice device,
      boolean redeployOnDevice) {
    ...
    if (kernelStackFrame[index] == null || !kernelStackFrame[index].isValid() || redeployOnDevice) {
      kernelStackFrame[index] =
          device.createKernelStackFrame(graphExecutionContext.getExecutionPlanId(), numArgs);
    }
  ...
  /**
   * Converts a global task index to a corresponding local task index within the local task list.
   * This is inorder to preserve the original task list.
   *
   * @param taskIndex The global task index to convert.
   * @return The corresponding local task index, or 0 if the task is not found in the local task
   *     list.
   */
  private int globalToLocalTaskIndex(int taskIndex) {
    return localTaskList.indexOf(taskExecutionContexts.get(taskIndex)) == -1
        ? 0
        : localTaskList.indexOf(taskExecutionContexts.get(taskIndex));
  }
  ...
  private void profilerUpdateForPreCompiledTask(SchedulableTask task) {
    if (task instanceof PrebuiltTask prebuiltTask && timeProfiler instanceof TimeProfiler) {
      timeProfiler.registerDeviceID(
          task.getId(),
          prebuiltTask.meta().getXPUDevice().getBackendIndex()
              + ":"
              + prebuiltTask.meta().getDeviceIndex());
      timeProfiler.registerDeviceName(
          task.getId(), prebuiltTask.meta().getXPUDevice().getPhysicalDevice().getDeviceName());
    }
  }
  ...
  private boolean isObjectInAtomicRegion(
      XPUDeviceBufferState objectState, TornadoXPUDevice device, SchedulableTask task) {
  ...
  private static class DebugInterpreter {
    static void logTransferToDeviceOnce(
        List<Integer> allEvents,
        Object object,
        TornadoXPUDevice deviceForInterpreter,
        long sizeBatch,
        long offset,
        final int eventList,
        StringBuilder tornadoVMBytecodeList) {
      ...
      String verbose =
          String.format(
              "bc: %s [Object Hash Code=0x%x] %s on %s, size=%d, offset=%d [event list=%d]",
              coloredText,
              object.hashCode(),
              object,
    ...
    static void logTransferToDeviceAlways(
        Object object,
        TornadoXPUDevice deviceForInterpreter,
        long sizeBatch,
        long offset,
      ...
      String verbose =
          String.format(
              "bc: %s [0x%x] %s on %s, size=%d, offset=%d [event list=%d]",
