...
public enum TornadoVMBytecodes {
  ...
  /**
   * Native buffer allocation. If there is not enough space on the target device, then we throw an
   * exception.
   *
   * <p>Format: <code>
   ...
   *
   * <p>If there is no ALLOC associated with the TRANSFER_HOST_TO_DEVICE_ONCE-in, an exception is
   * launched.
   *
   * <p>Format: <code>
  ...
  /**
   * Send data from Host -> Device in every execution of a task-graph. If there is no ALLOC
   * associated with the TRANSFER_HOST_TO_DEVICE_ALWAYS, an exception is launched.
   *
   * <p>Format: <code>
  ...
  /**
   * Send data from Device -> Host in every execution of the task-graph. If there is no ALLOC
   * associated with the TRANSFER_DEVICE_TO_HOST_ALWAYS, an exception is launched.
   *
   * <p>Format: <code>
  ...
  /** */
  TRANSFER_DEVICE_TO_HOST_ALWAYS_BLOCKING(
      (byte) 14), // TRANSFER_DEVICE_TO_HOST_ALWAYS_BLOCKING(obj, src, dest)

  /**
   * Compile the code the first iteration that the task-graph is executed and then execute the
   * kernel. If the kernel is already compiled, the kernel is directly launched.
   *
   * <p>Format: <code>
  ...
  /**
   * Sync point. The BC interpreter waits for an event to be finished before continuing the
   * execution.
   *
   * <p>Format: <code>
   * BARRIER <event>
  ...
  /**
   * Add a reference (e.g., a Java array reference) to be used as an argument for a compute-kernel.
   *
   * <p>Format: <code>
