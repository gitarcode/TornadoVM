...
public final class RuntimeUtilities {
   ...
   *     -byte-size-into-human-readable-format-in-java >Reference to StackOverflow</a>
   */
  ...
  /**
   * It prints the induction variables for counted loops in the given StructuredGraph. This method
   * can be used for post-processing parallel loops with identify information for the induction
   * variables.
   *
   * @param graph The StructuredGraph to analyze and print induction variables in the graph.
   */
      ...
      for (ParallelRangeNode parRange : parRanges) {
        for (Node n : parRange.offset().usages()) {
          if (loop.getInductionVariables().containsKey(n)) {
            BasicInductionVariable iv =
                (BasicInductionVariable) loop.getInductionVariables().get(n);
            System.out.printf(
                "[%d] parallel loop: %s -> init=%s, cond=%s, stride=%s, op=%s\n",
                parRange.index(),
                loop.loopBegin(),
                parRange.offset().value(),
                parRange.value(),
                parRange.stride(),
                iv.getOp());
          }
  ...
  public static void systemCall(
      String[] command, boolean printStandardOutput, String loggingDirectory) throws IOException {
    String stdOutput;
    ...
    try {
      ...
      errorOutput
          .append("Standard error (if any) of the command (")
  ...
  public static void writeStreamToFile(File file, byte[] source, boolean append) {
    try (FileOutputStream fos =
        (append ? new FileOutputStream(file, true) : new FileOutputStream(file))) {
      fos.write(source);
    } catch (IOException e) {
