 ...
 *
 * <p>This class bridges the external (user requirements) from the internals (runtime and compiler
 * parameters).
 */
public abstract class AbstractRTContext implements TaskContextInterface {
  ...
  AbstractRTContext(String id, AbstractRTContext parent) {
    ...
    if (isDeviceDefined) {
      BackendSelectionContainer backendSelection =
          MetaDataUtils.resolveDriverDeviceIndexes(getProperty(id + ".device"));
    ...
    compilerOptionsPerBackend.put(
        TornadoVMBackendType.OPENCL, TornadoOptions.DEFAULT_OPENCL_COMPILER_FLAGS);
    compilerOptionsPerBackend.put(
        TornadoVMBackendType.PTX, TornadoOptions.DEFAULT_PTX_COMPILER_FLAGS);
    compilerOptionsPerBackend.put(
        TornadoVMBackendType.SPIRV, TornadoOptions.DEFAULT_SPIRV_LEVEL_ZERO_COMPILER_FLAGS);
  ...
  protected static String getDefault(String keySuffix, String id, String defaultValue) {
    ...
    return (propertyValue != null)
        ? propertyValue
        : Tornado.getProperty("tornado" + "." + keySuffix, defaultValue);
  }
  ...
  private boolean hasSamePlatformName(
      TornadoAcceleratorBackend backend, TornadoDevice device, int index) {
    return backend.getDevice(index).getPlatformName().equals(device.getPlatformName());
  }

  private boolean hasSameDeviceName(
      TornadoAcceleratorBackend backend, TornadoDevice device, int index) {
    return backend
        .getDevice(index)
        .getPhysicalDevice()
        .getDeviceName()
        .equals(device.getPhysicalDevice().getDeviceName());
  }

  private int getDeviceIndex(int driverIndex, TornadoDevice device) {
    TornadoAcceleratorBackend backend =
        TornadoCoreRuntime.getTornadoRuntime().getBackend(driverIndex);
    int totalNumDevices = backend.getNumDevices();
    for (int deviceIndex = 0; deviceIndex < totalNumDevices; deviceIndex++) {
      if (hasSamePlatformName(backend, device, deviceIndex)
          && hasSameDeviceName(backend, device, deviceIndex)) {
        return deviceIndex;
      }
    }
    throw new TornadoRuntimeException(
        "[ERROR] Device not found: " + device.getPhysicalDevice().getDeviceName());
  }
