...
public class TaskUtils {
  ...
  private static final String JDK_VM_CI_HOTSPOT_JDK_REFLECTION =
      "jdk.vm.ci.hotspot.HotSpotJDKReflection";
  private static final String JDK_VM_CI_HOTSPOT_RESOLVED_JAVA_METHOD_IMPL =
  ...
  /**
   * JDK distributions before JDK 13 that to not backport JVMCI do not implement {@link
   * jdk.vm.ci.hotspot.HotSpotJDKReflection#getMethod}. Instead, we have to rely on {@link
   * jdk.vm.ci.hotspot.HotSpotResolvedJavaMethodImpl#toJava} that uses pure reflection.
   */
  private static Method callToJava(JavaMethod javaMethod) {
    try {
      Class hotSpotResolvedJavaMethodImpl =
          Class.forName(JDK_VM_CI_HOTSPOT_RESOLVED_JAVA_METHOD_IMPL);
      ...
      TornadoInternalError.shouldNotReachHere(
          "Both HotSpotResolvedJavaMethodImpl::toJava and HotSpotJDKReflection::getMethod are"
              + " missing from the JDK !");
    }
    return null;
  }

  /**
   * JDK distributions that backport JVMCI or after JDK 13 implement {@link
   * jdk.vm.ci.hotspot.HotSpotJDKReflection#getMethod} which relies on a native call to the JVM. If
   * this method is not available, then we call {@link
   * jdk.vm.ci.hotspot.HotSpotResolvedJavaMethodImpl#toJava} that uses pure reflection.
   */
  private static Method callGetMethod(JavaMethod javaMethod) {
    try {
      ...
      new TornadoLogger()
          .debug(
              "HotSpotJDKReflection::getMethod is missing from the JDK distribution. Falling back"
                  + " to HotSpotResolvedJavaMethodImpl::toJava");
  ...
  /**
   * When obtaining the method to be compiled it returns a lambda expression that contains the
   * invocation to the actual code. The actual code is an INVOKE that is inside the apply method of
   * the lambda. This method searches for the nested method with the actual code to be compiled.
   *
   * @param task Input Tornado task that corresponds to the user code.
   */
  public static Method resolveMethodHandle(Object task) {
    ...
    final ResolvedJavaMethod resolvedMethod =
        TornadoCoreRuntime.getVMBackend().getMetaAccess().lookupJavaMethod(entryPoint);
  ...
  public static CompilableTask createTask(
      Method method, ScheduleContext meta, String id, Task code) {
    return createTask(meta, id, method, code, true);
  }

  public static <T1> CompilableTask createTask(
      Method method, ScheduleContext meta, String id, Task1<T1> code, T1 arg1) {
    return createTask(meta, id, method, code, true, arg1);
  }

  public static <T1, T2> CompilableTask createTask(
      Method method, ScheduleContext meta, String id, Task2<T1, T2> code, T1 arg1, T2 arg2) {
    return createTask(meta, id, method, code, true, arg1, arg2);
  }

  public static <T1, T2, T3> CompilableTask createTask(
      Method method,
      ScheduleContext meta,
      String id,
      Task3<T1, T2, T3> code,
      T1 arg1,
      T2 arg2,
  ...
  public static <T1, T2, T3, T4, T5, T6, T7, T8, T9> CompilableTask createTask(
      ...
      T9 arg9) {
    return createTask(
        meta, id, method, code, true, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9);
  }
  ...
  public static <T1, T2, T3, T4, T5, T6, T7, T8, T9, T10> CompilableTask createTask(
      ...
      T10 arg10) {
    return createTask(
        meta, id, method, code, true, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10);
  }

  public static <T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11> CompilableTask createTask(
      Method method,
      ScheduleContext meta,
      String id,
  ...
  public static <T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12> CompilableTask createTask(
      Method method,
      ScheduleContext meta,
      String id,
  ...
  public static <T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13> CompilableTask createTask(
      Method method,
      ScheduleContext meta,
      String id,
  ...
  private static Object[] extractCapturedVariables(Object code) {
    ...
    for (Field field : type.getDeclaredFields()) {
      if (!field.getType().getName().contains("$$Lambda$")
          && !field.getName().contains("LAMBDA_INSTANCE$")) {
        count++;
      }
    }
  ...
  private static CompilableTask createTask(
      ScheduleContext meta,
      String id,
      Method method,
      Object code,
