...
public class TornadoAutoParalleliser extends BasePhase<TornadoSketchTierContext> {
  ...
  protected void run(StructuredGraph graph, TornadoSketchTierContext context) {
    if (!TornadoOptions.AUTO_PARALLELISATION
        || graph.getNodes().filter(ParallelRangeNode.class).isNotEmpty()) {
  ...
  private void autoParallelise(StructuredGraph graph) {
    if (graph.hasLoops()) {
      ...
      for (int i = 0; i < loops.size() && parallelDepth < 3; i++) {
        ...
        if (!phis.isEmpty()) {
          ...
          throw new TornadoBailoutRuntimeException(
              "unable to parallelise because of loop-dependencies.");
        }
      ...
      logger.info(
          "automatically parallelised %s (%dD kernel)\n", graph.method().getName(), parallelDepth);
    }
  }
  ...
  private void parallelizationReplacement(
    ...
    if (iv.isConstantInit() && iv.isConstantStride()) {
      final ConstantNode newInit =
          graph.addWithoutUnique(ConstantNode.forInt((int) iv.constantInit()));

      final ConstantNode newStride =
          graph.addWithoutUnique(ConstantNode.forInt((int) iv.constantStride()));

      final ParallelOffsetNode offset =
          graph.addWithoutUnique(new ParallelOffsetNode(parallelDepth, newInit));

      final ParallelStrideNode stride =
          graph.addWithoutUnique(new ParallelStrideNode(parallelDepth, newStride));

      final ParallelRangeNode range =
          graph.addWithoutUnique(
              new ParallelRangeNode(parallelDepth, maxIterations, offset, stride));
