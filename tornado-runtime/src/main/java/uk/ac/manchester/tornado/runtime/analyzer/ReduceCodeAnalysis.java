...
public class ReduceCodeAnalysis {
      ...
      if (operation instanceof TornadoReduceAddNode) {
        ...
        if (invoke.callTarget().targetName().equals("Math.max")) {
          ...
          throw new TornadoRuntimeException(
              "[ERROR] Automatic reduce operation not supported yet: " + operation);
        }
      } else if (operation instanceof BinaryNode
          && operation instanceof MarkFloatingPointIntrinsicsNode) {
  ...
  public static List<REDUCE_OPERATION> getReduceOperation(
      StructuredGraph graph, List<Integer> reduceIndices) {
  ...
  public static List<REDUCE_OPERATION> getReduceOperatorFromSketch(
      Graph graph, List<Integer> reduceIndices) {
    List<ValueNode> reduceOperation = new ArrayList<>();
    ...
    for (Integer paramIndex : reduceIndices) {
      ...
      for (Node node : usages) {
        if (node instanceof StoreAtomicIndexedNode store) {
          if (store.value() instanceof BinaryNode
              || store.value() instanceof BinaryArithmeticNode) {
            ValueNode value = store.value();
            reduceOperation.add(value);
          ...
          if (write.value() instanceof BinaryNode
              || write.value() instanceof BinaryArithmeticNode) {
  ...
  private static void getInputRageForReductionNode(
      ParameterNode parameterNode, ArrayList<ValueNode> loopBound) {
    // Get Input-Range for the reduction loop
  ...
  /**
   * A method can apply multiple reduction variables. We return a list of all its loop bounds.
   *
   * @param graph Graal-IR graph to be analyzed
   * @param reduceIndexes List of reduce indexes within the method parameter list
   * @return ArrayList<ValueNode>
   */
  private static ArrayList<ValueNode> findLoopUpperBoundNode(
      StructuredGraph graph, ArrayList<Integer> reduceIndexes) {
        ...
        for (ValueNode valueNode : loopBound) {
          ...
          if (valueNode.equals(graph.getParameter(position))) {
            ...
            if (object instanceof TornadoNativeArray tornadoNativeArray) {
              ...
              throw new TornadoRuntimeException(
                  "[ERROR] Unsupported type for reductions: " + object.getClass());
            }
   ...
   *
   * @param graph Input Graal {@link StructuredGraph}
   * @param lowValue Low value to include in the compile-graph
   */
