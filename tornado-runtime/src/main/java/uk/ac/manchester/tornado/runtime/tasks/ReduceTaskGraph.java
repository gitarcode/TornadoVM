...
class ReduceTaskGraph {
  ...
  ReduceTaskGraph(
      ...
      List<Object> streamInObjects,
      List<StreamingObject> streamingObjects,
      List<Object> streamOutObjects,
      List<StreamingObject> outputModeObjects,
      Graph graph,
      TornadoTaskGraph originalTaskGraph) {
  ...
  /**
   * @param driverIndex Index within the Tornado drivers' index
   * @param device Index of the device within the Tornado's device list.
  ...
  private static int obtainSizeArrayResult(int driverIndex, int device, int inputSize) {
    TornadoDeviceType deviceType =
        TornadoCoreRuntime.getTornadoRuntime()
            .getBackend(driverIndex)
            .getDevice(device)
            .getDeviceType();
    TornadoDevice deviceToRun =
        TornadoCoreRuntime.getTornadoRuntime().getBackend(driverIndex).getDevice(device);
    return switch (deviceType) {
      case CPU -> deviceToRun.getAvailableProcessors() + 1;
      case GPU, ACCELERATOR ->
          inputSize > calculateAcceleratorGroupSize(deviceToRun, inputSize)
              ? (inputSize / calculateAcceleratorGroupSize(deviceToRun, inputSize)) + 1
              : 2;
      default -> 0;
    };
  }
   ...
   * @param globalWorkSize Number of global threads to run.
  ...
  private static void inspectBinariesFPGA(
      String taskScheduleName, String graphName, String taskName, boolean sequential) {
    ...
    if (originalBinaries != null) {
      ...
      for (int i = 0; i < binaries.length; i += 2) {
        ...
        if (givenTaskName.equals(idTaskGraph)) {
          BackendSelectionContainer info =
              MetaDataUtils.resolveDriverDeviceIndexes(
                  MetaDataUtils.getProperty(idTaskGraph + ".device"));
          int deviceNumber = info.deviceIndex();

          if (!sequential) {
            originalBinaries.append(
                ","
                    + binaries[i]
                    + ","
                    + taskScheduleName
  ...
  private BackendSelectionContainer changeDriverAndDeviceIfNeeded(
      String taskScheduleName, String graphName, String taskName) {
    String idTaskGraph = graphName + "." + taskName;
    ...
    if (isDeviceDefined) {
      ...
      TornadoRuntimeProvider.setProperty(
          taskScheduleName + "." + taskName + ".device", backendIndex + ":" + deviceNumber);
      return info;
    }
    return null;
  }
  ...
  /**
   * Returns true if the input size is not power of 2 and the target device is either the GPU or the
   * FPGA.
   *
   * @param targetDeviceToRun index of the target device within the Tornado device list.
   * @return boolean
   */
  private boolean isTaskEligibleSplitHostAndDevice(
      final int targetDeviceToRun, final long elementsReductionLeftOver) {
    if (elementsReductionLeftOver > 0) {
      TornadoDeviceType deviceType =
          TornadoCoreRuntime.getTornadoRuntime()
              .getBackend(0)
              .getDevice(targetDeviceToRun)
              .getDeviceType();
      return (deviceType == TornadoDeviceType.GPU
          || deviceType == TornadoDeviceType.FPGA
          || deviceType == TornadoDeviceType.ACCELERATOR);
    }
    return false;
  }
  ...
  private void joinHostThreads() {
    if (threadSequentialExecution != null && !threadSequentialExecution.isEmpty()) {
      threadSequentialExecution.forEach(
          thread -> {
            try {
              thread.join();
  ...
  private ReduceCompilationThread createCompilationThread(
      final TaskPackage taskPackage, final int sizeTargetDevice) {
    ...
    for (int taskNumber = 0; taskNumber < taskPackages.size(); taskNumber++) {
      ...
      for (int i = 0; i < streamInObjects.size(); i++) {
        ...
        if (tableReduce.containsKey(taskNumber)
            && (!reduceOperandTable.containsKey(streamInObjects.get(i)))) {
      ...
      TornadoTaskGraph.performStreamInObject(
          rewrittenTaskGraph, streamInObjects, DataTransferMode.EVERY_EXECUTION);

      for (StreamingObject so : inputModeObjects) {
        if (so.getMode() == DataTransferMode.FIRST_EXECUTION) {
          TornadoTaskGraph.performStreamInObject(
              rewrittenTaskGraph, so.getObject(), DataTransferMode.FIRST_EXECUTION);
        }
      }
  ...
  private boolean isDeviceAnAccelerator(final int deviceToRun) {
    TornadoDeviceType deviceType =
        TornadoRuntimeProvider.getTornadoRuntime()
            .getBackend(0)
            .getDevice(deviceToRun)
            .getDeviceType();
    return (deviceType == TornadoDeviceType.ACCELERATOR);
  }

  private void updateGlobalAndLocalDimensionsFPGA(
      final int deviceToRun,
      String taskScheduleReduceName,
    ...
    if (isAheadOfTime() && isDeviceAnAccelerator(deviceToRun)) {
      TornadoRuntimeProvider.setProperty(
          taskScheduleReduceName
              + "."
              + taskPackage.getId()
              + TaskDataContext.GLOBAL_WORKGROUP_SUFFIX,
          Integer.toString(inputSize));
      TornadoRuntimeProvider.setProperty(
          taskScheduleReduceName
              + "."
              + taskPackage.getId()
              + TaskDataContext.LOCAL_WORKGROUP_SUFFIX,
          "64");
    }
  }

  private Object createHostArrayForHybridMode(
      Object originalReduceArray, TaskPackage taskPackage, int sizeTargetDevice) {
    hybridMode = true;
  ...
  /**
   * Compose and execute the new reduction. It dynamically creates a new task-schedule expression
   * that contains: a) the parallel reduction; b) the final sequential reduction.
   *
   * <p>It also creates a new thread in the case the input size for the reduction is not power of
   * two and the target device is either the FPGA or the GPU. In this case, the new thread will
   * compile the host part with the corresponding sub-range that does not fit into the power-of-two
   * part.
   *
   * @param metaReduceTable Metadata to create all new tasks for the reductions dynamically.
  ...
  TaskGraph scheduleWithReduction(MetaReduceCodeAnalysis metaReduceTable) {
    ...
    for (int taskNumber = 0; taskNumber < taskPackages.size(); taskNumber++) {
      ...
      BackendSelectionContainer selectionContainer =
          changeDriverAndDeviceIfNeeded(taskScheduleReduceName, graphName, taskPackage.getId());
      ...
      if (tableReduce.containsKey(taskNumber)) {
        ...
        for (Integer paramIndex : listOfReduceIndexParameters) {
          ...
          updateGlobalAndLocalDimensionsFPGA(
              deviceToRun, taskScheduleReduceName, taskPackage, inputSize);
          ...
          if (!isInputPowerOfTwo) {
            ...
            if (isTaskEligibleSplitHostAndDevice(deviceToRun, elementsReductionLeftOver)) {
              hostHybridModeArray =
                  createHostArrayForHybridMode(originalReduceArray, taskPackage, sizeTargetDevice);
            }
          }
        ...
        if (hybridMode) {
          ReduceCompilationThread compilationThread =
              createCompilationThread(taskPackage, inputSize);
          ...
          SequentialExecutionThread sequentialExecutionThread =
              new SequentialExecutionThread(compilationThread, taskPackage, hostHybridVariables);
    ...
    for (int taskNumber = 0; taskNumber < taskPackages.size(); taskNumber++) {
      ...
      if (tableReduce.containsKey(taskNumber)) {
        ...
        for (int i = 0; i < taskPackages.get(taskNumber).getTaskParameters().length - 1; i++) {
          ...
          if (reduceOperandTable.containsKey(parameterToMethod)
              && (reduceOperandTable.get(parameterToMethod).size() > 1)) {
            rewrittenTaskGraph.transferToDevice(
                DataTransferMode.FIRST_EXECUTION, parameterToMethod);
          }
        }
      }
      ...
      if (tableReduce.containsKey(taskNumber)) {
        ...
        MetaReduceTasks metaReduceTasks = tableReduce.get(taskNumber);
        List<Integer> listOfReduceParameters =
            metaReduceTasks.getListOfReduceParameters(taskNumber);
        StructuredGraph graph = metaReduceTasks.getGraph();
        List<REDUCE_OPERATION> operations =
            ReduceCodeAnalysis.getReduceOperation(graph, listOfReduceParameters);

        if (operations.isEmpty()) {
          // perform analysis with cached graph (after sketch phase)
          operations =
              ReduceCodeAnalysis.getReduceOperatorFromSketch(sketchGraph, listOfReduceParameters);
        }
        ...
        for (int i = 0; i < streamUpdateList.size(); i++) {
          ...
          for (REDUCE_OPERATION operation : operations) {
            ...
            TornadoRuntimeProvider.setProperty(
                fullName + ".device", backendToRun + ":" + deviceToRun);
            inspectBinariesFPGA(taskScheduleReduceName, graphName, taskPackage.getId(), true);

            switch (operation) {
              case SUM ->
                  ReduceFactory.handleAdd(
                      newArray, rewrittenTaskGraph, sizeReduceArray, newTaskSequentialName);
              case MUL ->
                  ReduceFactory.handleMul(
                      newArray, rewrittenTaskGraph, sizeReduceArray, newTaskSequentialName);
              case MAX ->
                  ReduceFactory.handleMax(
                      newArray, rewrittenTaskGraph, sizeReduceArray, newTaskSequentialName);
              case MIN ->
                  ReduceFactory.handleMin(
                      newArray, rewrittenTaskGraph, sizeReduceArray, newTaskSequentialName);
              default ->
    ...
    // mode.
    TornadoTaskGraph.performStreamOutThreads(
        DataTransferMode.EVERY_EXECUTION, rewrittenTaskGraph, streamOutObjects);
  ...
  void executeExpression() {
    ...
    if (hybridMode && !hybridInitialized) {
      ...
      for (HybridThreadMeta meta : hybridThreadMetas) {
        threadSequentialExecution.add(
            new SequentialExecutionThread(
                meta.compilationThread, meta.taskPackage, hostHybridVariables));
      }
    ...
    switch (newArray) {
      ...
      default ->
          throw new TornadoRuntimeException(
              "[ERROR] Reduce data type not supported yet: " + newArray.getClass().getTypeName());
    }
  }
  ...
  private void mergeHybridMode(Object originalReduceVariable, Object newArray) {
    switch (newArray) {
      case int[] intArray -> {
        ...
        int b = intArray[0];
        ((int[]) originalReduceVariable)[0] =
            operateFinalReduction(a, b, hybridMergeTable.get(intArray));
      }
      case float[] floatArray -> {
        float af = ((float[]) hostHybridVariables.get(floatArray))[0];
        float bf = floatArray[0];
        ((float[]) originalReduceVariable)[0] =
            operateFinalReduction(af, bf, hybridMergeTable.get(floatArray));
      }
      case double[] doubleArray -> {
        double ad = ((double[]) hostHybridVariables.get(doubleArray))[0];
        double bd = doubleArray[0];
        ((double[]) originalReduceVariable)[0] =
            operateFinalReduction(ad, bd, hybridMergeTable.get(doubleArray));
      }
      case long[] longArray -> {
        long al = ((long[]) hostHybridVariables.get(longArray))[0];
        long bl = longArray[0];
        ((long[]) originalReduceVariable)[0] =
            operateFinalReduction(al, bl, hybridMergeTable.get(longArray));
      }
      case IntArray panamaIntArray -> {
        int ani = ((IntArray) hostHybridVariables.get(panamaIntArray)).get(0);
        int bni = panamaIntArray.get(0);
        ((IntArray) originalReduceVariable)
            .set(0, operateFinalReduction(ani, bni, hybridMergeTable.get(panamaIntArray)));
      }
      case FloatArray panamaFloatArray -> {
        float anf = ((FloatArray) hostHybridVariables.get(panamaFloatArray)).get(0);
        float bnf = panamaFloatArray.get(0);
        ((FloatArray) originalReduceVariable)
            .set(0, operateFinalReduction(anf, bnf, hybridMergeTable.get(panamaFloatArray)));
      }
      case DoubleArray panamaDoubleArray -> {
        double and = ((DoubleArray) hostHybridVariables.get(panamaDoubleArray)).get(0);
        double bnd = panamaDoubleArray.get(0);
        ((DoubleArray) originalReduceVariable)
            .set(0, operateFinalReduction(and, bnd, hybridMergeTable.get(panamaDoubleArray)));
      }
      case LongArray panamaLongArray -> {
        long anl = ((LongArray) hostHybridVariables.get(panamaLongArray)).get(0);
        long bnl = panamaLongArray.get(0);
        ((LongArray) originalReduceVariable)
            .set(0, operateFinalReduction(anl, bnl, hybridMergeTable.get(panamaLongArray)));
      }
      default ->
          throw new TornadoRuntimeException(
              "[ERROR] Reduce data type not supported yet: " + newArray.getClass().getTypeName());
   ...
   *
   * <p>If the hybrid mode is enabled, it performs the final 1D reduction between the two elements
   * left (one from the accelerator and the other from the CPU)
   */
  ...
  private static class SequentialExecutionThread extends Thread {
    ...
    SequentialExecutionThread(
        ReduceCompilationThread compilationThread,
        TaskPackage taskPackage,
        Map<Object, Object> hostHybridVariables) {
      this.compilationThread = compilationThread;
      this.taskPackage = taskPackage;
     ...
     *
     * @param taskPackage {@link TaskPackage} metadata that stores the method parameters.
     * @param code {@link InstalledCode} code to be executed
     * @param hostHybridVariables HashMap that relates the GPU buffer with the new CPU buffer.
     */
    private void runBinaryCodeForReduction(
        TaskPackage taskPackage, InstalledCode code, Map<Object, Object> hostHybridVariables) {
      try {
