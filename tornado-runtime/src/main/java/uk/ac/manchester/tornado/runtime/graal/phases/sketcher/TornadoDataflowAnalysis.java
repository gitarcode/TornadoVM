...
public class TornadoDataflowAnalysis extends BasePhase<TornadoSketchTierContext> {
   ...
   */
  private MetaControlFlow analyseControlFlowForWriting(
      final Node currentNode,
      IfNode fatherNodeStore,
      final boolean isWrittenTrueCondition,
      final boolean isWrittenFalseCondition) {
  ...
  private boolean isNodeFromKnownObject(Node currentNode) {
    ...
    return currentNode
            .getClass()
            .getName()
            .equals("uk.ac.manchester.tornado.drivers.opencl.graal.nodes.IncAtomicNode")
        || currentNode
            .getClass()
            .getName()
            .equals("uk.ac.manchester.tornado.drivers.opencl.graal.nodes.DecAtomicNode");
  }
    ...
    while (!nodesToProcess.isEmpty()) {
      ...
      if (currentNode instanceof LoadIndexedNode) {
        ...
        if (((ValueNode) currentNode).stamp(NodeView.DEFAULT).javaType(metaAccess).isArray()) {
          ...
          || currentNode instanceof StoreAtomicIndexedNode
          || currentNode instanceof WriteNode
          || currentNode instanceof JavaWriteNode) {
        MetaControlFlow meta =
            analyseControlFlowForWriting(
                currentNode, fatherNodeStore, isWrittenTrueCondition, isWrittenFalseCondition);
        ...
        isWritten = true;
          ...
          || currentNode instanceof AddressNode
          || currentNode instanceof OffsetAddressNode) {
  ...
  private boolean isTornadoNativeArray(LoadFieldNode loadFieldNode) {
    ...
    return field.contains("uk.ac.manchester.tornado.api.types.arrays.IntArray")
  ...
  /**
   * Quick check to obtain if the store index of the store operation covers all iterations or a
   * subset. This check is useful to determine READ-ONLY or WRITE-ONLY Accesses over an array.
   *
   * @param storeIndexedNode store indexed (array) value
  ...
  private static class MetaControlFlow {
    ...
    MetaControlFlow(
        boolean isWrittenTrueCondition, boolean isWrittenFalseCondition, IfNode fatherNodeStore) {
      super();
