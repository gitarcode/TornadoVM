...
public class PTXObjectWrapper implements XPUBuffer {
  ...
  public PTXObjectWrapper(final PTXDeviceContext device, Object object) {
    ...
    resolvedType =
        (HotSpotResolvedJavaType)
            getVMRuntime().getHostJVMCIBackend().getMetaAccess().lookupJavaType(object.getClass());
    ...
    for (int index = 0; index < fields.length; index++) {
      ...
      if (DEBUG) {
        logger.trace(
            "field: name=%s, kind=%s, offset=%d",
            field.getName(), type.getName(), field.getOffset());
      }
      ...
      if (type.isArray()) {
        ...
        wrappedField =
            new PTXMemorySegmentWrapper(
                device, ((FloatArray) objectFromField).getSegmentWithHeader().byteSize(), 0);
      } else if (type == ByteArray.class) {
        Object objectFromField = TornadoUtils.getObjectFromField(reflectedField, object);
        wrappedField =
            new PTXMemorySegmentWrapper(
                device, ((ByteArray) objectFromField).getSegmentWithHeader().byteSize(), 0);
      } else if (type == DoubleArray.class) {
        Object objectFromField = TornadoUtils.getObjectFromField(reflectedField, object);
        wrappedField =
            new PTXMemorySegmentWrapper(
                device, ((DoubleArray) objectFromField).getSegmentWithHeader().byteSize(), 0);
      } else if (type == IntArray.class) {
        Object objectFromField = TornadoUtils.getObjectFromField(reflectedField, object);
        wrappedField =
            new PTXMemorySegmentWrapper(
                device, ((IntArray) objectFromField).getSegmentWithHeader().byteSize(), 0);
      } else if (type == ShortArray.class) {
        Object objectFromField = TornadoUtils.getObjectFromField(reflectedField, object);
        wrappedField =
            new PTXMemorySegmentWrapper(
                device, ((ShortArray) objectFromField).getSegmentWithHeader().byteSize(), 0);
      } else if (type == LongArray.class) {
        Object objectFromField = TornadoUtils.getObjectFromField(reflectedField, object);
        wrappedField =
            new PTXMemorySegmentWrapper(
                device, ((LongArray) objectFromField).getSegmentWithHeader().byteSize(), 0);
      } else if (type == HalfFloatArray.class) {
        Object objectFromField = TornadoUtils.getObjectFromField(reflectedField, object);
        wrappedField =
            new PTXMemorySegmentWrapper(
                device, ((HalfFloatArray) objectFromField).getSegmentWithHeader().byteSize(), 0);
      } else if (object.getClass().getAnnotation(Vector.class) != null) {
        wrappedField =
            new PTXVectorWrapper(
                device, TornadoUtils.getObjectFromField(reflectedField, object), 0);
      } else if (field.getJavaKind().isObject()) {
        // We capture the field by the scope definition of the input
        // lambda expression
        wrappedField =
            new PTXObjectWrapper(device, TornadoUtils.getObjectFromField(reflectedField, object));
      }
  ...
  public void allocate(Object reference, long batchSize) {
    if (DEBUG) {
      logger.debug(
          "object: object=0x%x, class=%s", reference.hashCode(), reference.getClass().getName());
    }
      ...
      if (buffer != null) {
        ...
        if (batchSize > 0) {
          throw new TornadoMemoryException(
              "[ERROR] BatchSize Allocation currently not supported for Objects Fields. BatchSize ="
                  + " "
                  + batchSize
                  + " (bytes)");
        }
  ...
  public void write(long executionPlanId, Object object) {
    ...
    deviceContext.writeBuffer(
        executionPlanId, toBuffer(), getObjectSize(), buffer.array(), 0, null);
    for (int i = 0; i < fields.length; i++) {
  ...
  public int enqueueRead(
    ...
    for (FieldBuffer fb : wrappedFields) {
      if (fb != null) {
        internalEvents[index] =
            fb.enqueueRead(executionPlanId, reference, (useDeps) ? events : null, useDeps);
        index++;
      }
    }
  ...
  public List<Integer> enqueueWrite(
    ...
    for (final FieldBuffer field : wrappedFields) {
      if (field != null) {
        eventList.addAll(
            field.enqueueWrite(executionPlanId, ref, (useDeps) ? events : null, useDeps));
      }
    }
    return eventList;
  }
