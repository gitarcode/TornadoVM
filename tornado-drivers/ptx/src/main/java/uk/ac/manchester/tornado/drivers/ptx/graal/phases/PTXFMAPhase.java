        ...
        .forEach(
            addNode -> {
              ...
              if (inputMuls.count() > 0) {
                ...
                ValueNode z =
                    (ValueNode) addNode.inputs().filter(node -> !node.equals(mul)).first();

                // do not apply this optimization for half floats
                if (x instanceof ReadHalfFloatNode
                    || y instanceof ReadHalfFloatNode
                    || z instanceof ReadHalfFloatNode) {
                  return;
                }

                if (x instanceof VectorElementOpNode
                    || y instanceof VectorElementOpNode
                    || z instanceof VectorElementOpNode) {
                  return;
                }
