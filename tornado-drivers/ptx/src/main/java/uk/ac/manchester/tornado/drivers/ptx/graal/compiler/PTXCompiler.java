...
public class PTXCompiler {
  ...
  private static PTXCompilationResult compile(PTXCompilationRequest r) {
    ...
    try (DebugContext.Scope s0 =
            TornadoCoreRuntime.getDebugContext()
                .scope("GraalCompiler", r.graph, r.providers.getCodeCache());
        DebugCloseable a = CompilerTimer.start(TornadoCoreRuntime.getDebugContext())) {
      emitFrontEnd(r);
      boolean isParallel =
          r.meta != null
              && (r.meta.isParallel()
                  || (r.meta.isGridSchedulerEnabled() && !r.meta.isGridSequential()));
  ...
  private static void emitBackEnd(PTXCompilationRequest r, boolean isParallel) {
    try (DebugContext.Scope s =
            TornadoCoreRuntime.getDebugContext().scope("PTXBackend", r.graph.getLastSchedule());
        DebugCloseable a = BackEnd.start(TornadoCoreRuntime.getDebugContext())) {
      LIRGenerationResult lirGen = emitLIR(r);
      try (DebugContext.Scope s2 =
          TornadoCoreRuntime.getDebugContext().scope("PTXCodeGen", lirGen, lirGen.getLIR())) {
  ...
  private static void emitCode(
      PTXCompilationRequest r, LIRGenerationResult lirGenRes, boolean isParallel) {
    try (DebugCloseable a = EmitCode.start(TornadoCoreRuntime.getDebugContext())) {
      FrameMap frameMap = lirGenRes.getFrameMap();
      final PTXCompilationResultBuilder crb =
          r.backend.newCompilationResultBuilder(
              lirGenRes,
              frameMap,
              r.compilationResult,
              r.factory,
      ...
      if (TornadoCoreRuntime.getDebugContext().isCountEnabled()) {
        ...
        DebugContext.counter("CodeBytesEmitted")
            .add(TornadoCoreRuntime.getDebugContext(), r.compilationResult.getTargetCodeSize());
      }

      TornadoCoreRuntime.getDebugContext()
          .dump(DebugContext.BASIC_LEVEL, r.compilationResult, "After code generation");
    }
  }

  private static LIRGenerationResult emitLIR(PTXCompilationRequest r) {
    try (DebugContext.Scope ds = TornadoCoreRuntime.getDebugContext().scope("EmitLIR");
        DebugCloseable a = EmitLIR.start(TornadoCoreRuntime.getDebugContext())) {
      ...
      LIR lir = null;
      try (DebugContext.Scope s =
          TornadoCoreRuntime.getDebugContext().scope("ComputeLinearScanOrder", lir)) {
        ...
        TornadoCoreRuntime.getDebugContext()
            .dump(DebugContext.INFO_LEVEL, lir, "After linear scan order");
      } catch (Throwable e) {
        throw TornadoCoreRuntime.getDebugContext().handle(e);
      }
      RegisterAllocationConfig registerAllocationConfig =
          r.backend.newRegisterAllocationConfig(null, new String[] {});
      FrameMapBuilder frameMapBuilder = r.backend.newFrameMapBuilder(null);
      LIRGenerationResult lirGenRes =
          r.backend.newLIRGenerationResult(
              r.graph.compilationId(), lir, frameMapBuilder, registerAllocationConfig);
      ...
      // LIR generation
      PTXLIRGenerationPhase.LIRGenerationContext context =
          new PTXLIRGenerationPhase.LIRGenerationContext(
              lirGen, nodeLirGen, r.graph, schedule, r.isKernel);
      LIR_GENERATION_PHASE.apply(r.backend.getTarget(), lirGenRes, context);

      try (DebugContext.Scope s =
          TornadoCoreRuntime.getDebugContext().scope("LIRStages", nodeLirGen, lir)) {
        TornadoCoreRuntime.getDebugContext()
            .dump(DebugContext.BASIC_LEVEL, lir, "After LIR generation");
        LIRGenerationResult result =
            emitLowLevel(
                r.backend.getTarget(), lirGenRes, lirGen, r.lirSuites, registerAllocationConfig);
        TornadoCoreRuntime.getDebugContext()
            .dump(DebugContext.BASIC_LEVEL, lir, "Before code generation");
        return result;
      } catch (Throwable e) {
        throw TornadoCoreRuntime.getDebugContext().handle(e);
      }
  ...
  private static LIRGenerationResult emitLowLevel(
      TargetDescription target,
      LIRGenerationResult lirGenRes,
      LIRGeneratorTool lirGen,
      TornadoLIRSuites lirSuites,
      RegisterAllocationConfig registerAllocationConfig) {
    final PreAllocationOptimizationPhase.PreAllocationOptimizationContext preAllocOptContext =
        new PreAllocationOptimizationPhase.PreAllocationOptimizationContext(lirGen);
    lirSuites.getPreAllocationStage().apply(target, lirGenRes, preAllocOptContext);
    AllocationPhase.AllocationContext allocContext =
        new AllocationPhase.AllocationContext(
            lirGen.getSpillMoveFactory(), registerAllocationConfig);
  ...
  private static void emitFrontEnd(PTXCompilationRequest r) {
    try (DebugContext.Scope s =
            TornadoCoreRuntime.getDebugContext()
                .scope("PTXFrontend", new DebugDumpScope("PTXFrontend"));
        DebugCloseable a = FrontEnd.start(TornadoCoreRuntime.getDebugContext())) {
      final TornadoHighTierContext highTierContext =
          new TornadoHighTierContext(
              r.providers,
              r.graphBuilderSuite,
              r.optimisticOpts,
      ...
      if (r.buildGraph) {
        if (isGraphEmpty(r.graph)) {
          ...
          new DeadCodeEliminationPhase(DeadCodeEliminationPhase.Optionality.Optional)
              .apply(r.graph);
        } else {
          TornadoCoreRuntime.getDebugContext()
              .dump(DebugContext.INFO_LEVEL, r.graph, "initial state");
        }
      }
      r.suites.getHighTier().apply(r.graph, highTierContext);
      r.graph.maybeCompress();

      final TornadoMidTierContext midTierContext =
          new TornadoMidTierContext(
              r.providers,
              r.backend,
              r.optimisticOpts,
              r.profilingInfo,
      ...
      final TornadoLowTierContext lowTierContext =
          new TornadoLowTierContext(r.providers, r.backend, r.meta);
      r.suites.getLowTier().apply(r.graph, lowTierContext);

      TornadoCoreRuntime.getDebugContext()
          .dump(DebugContext.BASIC_LEVEL, r.graph.getLastSchedule(), "Final HIR schedule");
    } catch (Throwable e) {
      throw TornadoCoreRuntime.getDebugContext().handle(e);
    }
  }
  ...
  public static synchronized PTXCompilationResult compileSketchForDevice(
      Sketch sketch,
      CompilableTask task,
      PTXProviders providers,
      PTXBackend backend,
      TornadoProfiler profiler) {
    final StructuredGraph kernelGraph =
        (StructuredGraph) sketch.getGraph().copy(TornadoCoreRuntime.getDebugContext());
    ResolvedJavaMethod resolvedMethod = kernelGraph.method();

    new TornadoLogger()
        .info(
            "Compiling sketch %s on %s",
            resolvedMethod.getName(), backend.getDeviceContext().getDevice().getDeviceName());

    final TaskDataContext taskMeta = task.meta();
    final Object[] args = task.getArguments();
    final long batchThreads =
        (taskMeta.getNumThreads() > 0) ? taskMeta.getNumThreads() : task.getBatchThreads();
    ...
    BatchCompilationConfig batchCompilationConfig =
        new BatchCompilationConfig(batchThreads, batchNumber, batchSize);

    OptimisticOptimizations optimisticOpts = OptimisticOptimizations.ALL;
    ProfilingInfo profilingInfo = resolvedMethod.getProfilingInfo();

    PTXCompilationResult kernelCompResult =
        new PTXCompilationResult(
            PTXCodeUtil.buildKernelName(resolvedMethod.getName(), task), taskMeta);
    ...
    PTXCompilationRequest kernelCompilationRequest =
        PTXCompilationRequest.PTXCompilationRequestBuilder.getInstance() //
    ...
    final Deque<ResolvedJavaMethod> workList =
        new ArrayDeque<>(kernelCompResult.getNonInlinedMethods());
    while (!workList.isEmpty()) {
      ...
      Sketch currentSketch =
          TornadoSketcher.lookup(
              currentMethod, task.meta().getBackendIndex(), task.meta().getDeviceIndex());
      final PTXCompilationResult compResult =
          new PTXCompilationResult(currentMethod.getName(), taskMeta);
      final StructuredGraph graph =
          (StructuredGraph) currentSketch.getGraph().copy(TornadoCoreRuntime.getDebugContext());

      // @formatter:off
      PTXCompilationRequest methodCompilationRequest =
          PTXCompilationRequest.PTXCompilationRequestBuilder.getInstance()
              .withGraph(graph)
  ...
  public static PTXCompilationResult compileCodeForDevice(
    ...
    new TornadoLogger()
        .info(
            "Compiling %s on %s",
            resolvedMethod.getName(), backend.getDeviceContext().getDevice().getDeviceName());
    final TornadoCompilerIdentifier id =
        new TornadoCompilerIdentifier(
            "compile-kernel" + resolvedMethod.getName(), compilationId.getAndIncrement());

    StructuredGraph.Builder builder =
        new StructuredGraph.Builder(
            TornadoCoreRuntime.getTornadoRuntime().getOptions(),
            TornadoCoreRuntime.getDebugContext(),
    ...
    PTXCompilationResult kernelCompResult =
        new PTXCompilationResult(resolvedMethod.getName(), meta);
    CompilationResultBuilderFactory factory = CompilationResultBuilderFactory.Default;
    final PTXSuitesProvider suitesProvider = (PTXSuitesProvider) providers.getSuitesProvider();
    ...
    while (!workList.isEmpty()) {
      final ResolvedJavaMethod currentMethod = workList.pop();
      final PTXCompilationResult compResult =
          new PTXCompilationResult(currentMethod.getName(), meta);
      StructuredGraph.Builder builder1 =
          new StructuredGraph.Builder(
              TornadoCoreRuntime.getOptions(),
              TornadoCoreRuntime.getDebugContext(),
              StructuredGraph.AllowAssumptions.YES);
  ...
  public static class PTXCompilationRequest {
    ...
    public static class PTXCompilationRequestBuilder {
      ...
      public PTXCompilationRequestBuilder withGraphBuilderSuite(
          PhaseSuite<HighTierContext> builderSuite) {
      ...
      public PTXCompilationRequestBuilder withResultBuilderFactory(
          CompilationResultBuilderFactory factory) {
        this.factory = factory;
        return this;
      }
      ...
      public PTXCompilationRequestBuilder setBatchCompilationConfig(
          BatchCompilationConfig batchCompilationConfig) {
        this.batchCompilationConfig = batchCompilationConfig;
        return this;
      }
