...
public class PTXCodeUtil {
  ...
  private static final String PACKAGE_PANAMA_COLLECTION =
      "uk_ac_manchester_tornado_api_types_collections_";
  private static final String PTX_HEADER_FORMAT =
      PTXAssemblerConstants.COMPUTE_VERSION
          + " %s \n"
          + PTXAssemblerConstants.TARGET_ARCH
          + " %s \n"
          + PTXAssemblerConstants.ADDRESS_HEADER
          + " %s \n";

  /** Create a calling convention from a {@link ResolvedJavaMethod}. */
  public static CallingConvention getCallingConvention(
      CodeCacheProvider codeCache, ResolvedJavaMethod method) {
  ...
  private static CallingConvention getCallingConvention(
    ...
    for (int i = 0; i < argTypes.length; i++, variableIndex++) {
      inputParameters[i] =
          new Variable(
              LIRKind.value(target.arch.getPlatformKind(argTypes[i].getJavaKind())), variableIndex);
    }
   ...
   * @param task
   */
  private static void emitSignatureForArrayParameter(
      StringBuilder sb, Object arg, SchedulableTask task) {
   ...
   * @param task
   */
  private static void emitSignatureForOffHeapSegments(
      StringBuilder sb, Object arg, SchedulableTask task) {
    Class<?> argClass = arg.getClass();
    if (task.getBatchThreads() != 0) {
      sb.append(task.getBatchThreads());
  ...
  /**
   * Since with objects there is no way to know what will be a constant differentiate using the
  ...
  public static String buildKernelName(String methodName, SchedulableTask task) {
    ...
    return sb.toString()
        .replaceAll(PACKAGE_PANAMA_TYPES, "")
  ...
  public static byte[] getCodeWithAttachedPTXHeader(byte[] targetCode, PTXBackend backend) {
    ...
    String header =
        String.format(
            PTX_HEADER_FORMAT,
            device.getTargetPTXVersion(),
            device.getTargetArchitecture(),
            backend.getTarget().getArch().getWordSize() * 8);
