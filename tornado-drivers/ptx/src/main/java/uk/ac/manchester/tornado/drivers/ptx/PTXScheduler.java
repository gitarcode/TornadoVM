...
public class PTXScheduler {
  ...
  public int[] calculateBlockDimension(PTXModule module, TaskDataContext taskMeta) {
    ...
    return calculateBlockDimension(
        taskMeta.getGlobalWork(), maxThreadsPerBlock, taskMeta.getDims(), module.javaName);
  }

  public int[] calculateBlockDimension(
      long[] globalWork, long maxThreadBlocks, int dimension, String javaName) {
    ...
    try {
      ...
      for (int i = 0; i < dimension; i++) {
        defaultBlocks[i] =
            calculateBlockSize(
                calculateEffectiveMaxWorkItemSize(dimension, maxBlockThreads), globalWork[i]);
      }
    } catch (Exception e) {
  ...
  public int[] calculateGridDimension(
      PTXModule module, TaskDataContext taskMeta, int[] blockDimension) {
  ...
  public int[] calculateGridDimension(
      String javaName, int dimension, int[] globalWork, int[] blockDimension) {
    ...
    try {
      ...
      for (int i = 0; i < dimension; i++) {
        ...
        defaultGrids[i] =
            Math.max(Math.min(workSize / blockDimension[i], (int) maxGridSizes[i]), 1);
      }
    } catch (Exception e) {
