...
public class PTXTornadoDevice implements TornadoXPUDevice {
  ...
  public TornadoInstalledCode installCode(SchedulableTask task) {
    return switch (task) {
      ...
      default ->
          throw new TornadoInternalError(
      ...
      if (!deviceContext.isCached(resolvedMethod.getName(), executable)) {
        ...
        result =
            new PTXCompilationResult(
                buildKernelName(resolvedMethod.getName(), executable), taskMeta);
      }

      profiler.start(ProfilerType.TASK_COMPILE_DRIVER_TIME, taskMeta.getId());
      TornadoInstalledCode installedCode =
          deviceContext.installCode(result, resolvedMethod.getName());
  ...
  private TornadoInstalledCode compilePreBuiltTask(SchedulableTask task) {
    ...
    try {
      ...
      return deviceContext.installCode(
          functionName, source, executable.getEntryPoint(), task.meta().isPrintKernelEnabled());
    } catch (IOException e) {
      e.printStackTrace();
    }
    return null;
  ...
  private XPUBuffer createArrayWrapper(
      Class<?> type, PTXDeviceContext deviceContext, long batchSize) {
    XPUBuffer result = null;
    if (type == int[].class) {
  ...
  public int streamOutBlocking(
    ...
    return objectState
        ...
        .read(
            ...
            object,
            hostOffset,
            objectState.getPartialCopySize(),
            events,
            events != null);
  }
  ...
  /**
   * In CUDA the context is not attached to the whole process, but to individual threads Therefore,
   * in the case of new threads executing a task schedule, we must make sure that the context is set
   * for that thread.
   */
  @Override
