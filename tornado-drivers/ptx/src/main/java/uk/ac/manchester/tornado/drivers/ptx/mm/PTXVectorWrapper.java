...
public class PTXVectorWrapper implements XPUBuffer {
  ...
  public PTXVectorWrapper(final PTXDeviceContext device, final Object object, long batchSize) {
    TornadoInternalError.guarantee(
        object instanceof PrimitiveStorage, "Expecting a PrimitiveStorage type");
  ...
  public void allocate(Object value, long batchSize) {
    TornadoInternalError.guarantee(
        value instanceof PrimitiveStorage, "Expecting a PrimitiveStorage type");
    ...
    if (TornadoOptions.FULL_DEBUG) {
      logger.info(
          "allocated: array kind=%s, size=%s, length offset=%d, header size=%d",
          kind.getJavaName(),
  ...
  public void markAsFreeBuffer() {
    TornadoInternalError.guarantee(
        buffer != INIT_VALUE, "Fatal error: trying to deallocate an invalid buffer");
    ...
    if (TornadoOptions.FULL_DEBUG) {
      logger.info(
          "deallocated: array kind=%s, size=%s, length offset=%d, header size=%d",
          kind.getJavaName(),
          humanReadableByteCount(bufferSize, true),
          arrayLengthOffset,
          arrayHeaderSize);
   ...
   */
  private int enqueueReadArrayData(
      long executionPlanId,
      long address,
      long bytes,
      Object value,
    ...
    return switch (kind) {
      case JavaKind.Int ->
          deviceContext.enqueueReadBuffer(
              executionPlanId, address, bytes, (int[]) value, hostOffset, waitEvents);
      case JavaKind.Float ->
          deviceContext.enqueueReadBuffer(
              executionPlanId, address, bytes, (float[]) value, hostOffset, waitEvents);
      case JavaKind.Double ->
          deviceContext.enqueueReadBuffer(
              executionPlanId, address, bytes, (double[]) value, hostOffset, waitEvents);
      case JavaKind.Long ->
          deviceContext.enqueueReadBuffer(
              executionPlanId, address, bytes, (long[]) value, hostOffset, waitEvents);
      case JavaKind.Short ->
          deviceContext.enqueueReadBuffer(
              executionPlanId, address, bytes, (short[]) value, hostOffset, waitEvents);
      case JavaKind.Byte ->
          deviceContext.enqueueReadBuffer(
              executionPlanId, address, bytes, (byte[]) value, hostOffset, waitEvents);
      case JavaKind.Object ->
          deviceContext.enqueueReadBuffer(
              executionPlanId,
              address,
              bytes,
              ((TornadoNativeArray) value).getSegmentWithHeader().address(),
              hostOffset,
              waitEvents);
  ...
  public List<Integer> enqueueWrite(
    ...
    final int returnEvent =
        enqueueWriteArrayData(
            executionPlanId, toBuffer(), bufferSize, array, hostOffset, (useDeps) ? events : null);
    listEvents.add(returnEvent);
    return listEvents;
  }

  private int enqueueWriteArrayData(
      long executionPlanId,
      long address,
      long bytes,
      Object value,
  ...
  public String toString() {
    return String.format(
        "buffer<%s> %s", kind.getJavaName(), humanReadableByteCount(bufferSize, true));
  }
    ...
    switch (kind) {
      case JavaKind.Int ->
          deviceContext.writeBuffer(
              executionPlanId, address, bytes, (int[]) value, (int) hostOffset, waitEvents);
      case JavaKind.Float ->
          deviceContext.writeBuffer(
              executionPlanId, address, bytes, (float[]) value, (int) hostOffset, waitEvents);
      case JavaKind.Double ->
          deviceContext.writeBuffer(
              executionPlanId, address, bytes, (double[]) value, (int) hostOffset, waitEvents);
      case JavaKind.Long ->
          deviceContext.writeBuffer(
              executionPlanId, address, bytes, (long[]) value, (int) hostOffset, waitEvents);
      case JavaKind.Short ->
          deviceContext.writeBuffer(
              executionPlanId, address, bytes, (short[]) value, hostOffset, waitEvents);
      case JavaKind.Byte ->
