...
public class PTXBuiltinTool {
  ...
  public Value genFloatCosPI(
      Value input, Value piConstant, Value resultMult, LIRGeneratorTool gen) {
  ...
  /**
   * The radians operation is implemented as: (pi / 180) * degrees. In PTX, the first argument of
   * the multiplication is a constant value x {@value
   * uk.ac.manchester.tornado.drivers.ptx.graal.asm.PTXAssemblerConstants#DEGREES_TO_RADIANS} while
   * the second argument that corresponds to the degrees is value y.
   *
   * @param x the constant value of (pi/180)
   * @param y the angle value measured in degrees
  ...
  public Value genFloatRadians(Value x, Value y) {
    Logger.traceBuildLIR(
        Logger.BACKEND.PTX,
        "genFloatRadians: radians corresponds to PTX instruction mul.rn.f32(%s, %s)",
        x,
        y);
