...
public class PTXVersion {
  private enum PTX_VERSION_TO_ARCHITECTURE {
    ...
    PTX_VERSION_TO_ARCHITECTURE(
        CUDAComputeCapability ptxIsa, TargetArchitecture targetArchitecture) {
      this.ptxIsa = ptxIsa;
  ...
  public TargetArchitecture getArchitecture(CUDAComputeCapability deviceCapability) {
    CUDAComputeCapability computeCapability =
        maxArch.compareTo(deviceCapability) > 0 ? deviceCapability : maxArch;
