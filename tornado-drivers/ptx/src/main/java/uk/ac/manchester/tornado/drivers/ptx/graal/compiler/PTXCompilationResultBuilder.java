...
public class PTXCompilationResultBuilder extends CompilationResultBuilder {
  ...
  public PTXCompilationResultBuilder(
  ...
  void emitBlock(HIRBlock HIRBlock) {
    ...
    if (breakInst != null) {
      try {
        ...
        throw e.addContext(
            "lir instruction", HIRBlock + "@" + breakInst.id() + " " + breakInst + "\n");
      }
    }
  }
  ...
  private boolean isFalseSuccessorWithLoopEnd(IfNode ifNode, HIRBlock HIRBlock) {
    return isCurrentBlockAFalseBranch(ifNode, HIRBlock)
        && HIRBlock.getEndNode() instanceof LoopEndNode;
  }
   ...
   * HIRBlock to generate always the true condition before the false condition only if we have a
   ...
   *
   * @param HIRBlock {@link HIRBlock}
   * @param visitor {@link PTXBlockVisitor}
