...
public class PTXArithmeticTool extends ArithmeticLIRGenerator {
  ...
  protected Variable emitAdd(LIRKind resultKind, Value a, Value b, boolean setFlags) {
    Logger.traceBuildLIR(
        Logger.BACKEND.PTX,
        "emitAdd resultKind=%s a=%s b=%s setFlags=%b",
        resultKind,
        a,
        b,
        setFlags);
  ...
  protected Variable emitSub(LIRKind resultKind, Value a, Value b, boolean setFlags) {
    Logger.traceBuildLIR(
        Logger.BACKEND.PTX,
        "emitSub resultKind=%s a=%s b=%s setFlags=%b",
        resultKind,
        a,
        b,
        setFlags);
  ...
  public Value emitNegate(Value input, boolean setFlags) {
    ...
    return emitUnaryAssign(
        PTXAssembler.PTXUnaryOp.NEGATE, LIRKind.value(input.getPlatformKind()), input);
  }
  ...
  public Value emitMul(Value a, Value b, boolean setFlags) {
    ...
    PTXBinaryOp op =
        ((PTXKind) resultKind.getPlatformKind()).isFloating()
            ? PTXBinaryOp.MUL
            : PTXBinaryOp.MUL_LO;
  ...
  public Value emitDiv(Value a, Value b, LIRFrameState state) {
    ...
    LIRKind resultKind = LIRKind.combine(a, b);
    PTXBinaryOp op =
        resultKind.getPlatformKind() == PTXKind.F32 ? PTXBinaryOp.DIV_FULL : PTXBinaryOp.DIV;
    return emitBinaryAssign(op, resultKind, a, b);
  }
  ...
  public Value emitNot(Value input) {
    ...
    return emitUnaryAssign(
        PTXAssembler.PTXUnaryOp.NOT, LIRKind.value(input.getPlatformKind()), input);
  }
  ...
  public Value emitSignExtend(Value inputVal, int fromBits, int toBits) {
    Logger.traceBuildLIR(
        Logger.BACKEND.PTX,
        "emitSignExtend inputVal=%s fromBits=%d toBits=%d",
        inputVal,
        fromBits,
        toBits);
  ...
  public Value emitZeroExtend(Value inputVal, int fromBits, int toBits) {
    Logger.traceBuildLIR(
        Logger.BACKEND.PTX,
        "emitZeroExtend inputVal=%s fromBits=%d toBits=%d",
        inputVal,
        fromBits,
        toBits);
    PTXLIRKindTool kindTool = getGen().getLIRKindTool();
  ...
  public Value emitMathCopySign(Value magnitude, Value sign) {
    Logger.traceBuildLIR(
        Logger.BACKEND.PTX, "emitMathCopySign magnitude=%s sign=%s", sign, magnitude);
    PTXBuiltinTool builtinTool = getGen().getPtxBuiltinTool();
      ...
      getGen()
          .append(
              new PTXLIRStmt.LoadStmt(
                  (PTXUnary.MemoryAccess) address, dest, PTXAssembler.PTXNullaryOp.LD));
    }
    return dest;
  }
  ...
  public void emitStore(
    ...
    Logger.traceBuildLIR(
        Logger.BACKEND.PTX, "emitStore: kind=%s, address=%s, input=%s", kind, address, input);
  ...
  public Value emitMultiplyAdd(Value op1, Value op2, Value op3) {
    ...
    getGen()
        .append(
            new PTXLIRStmt.AssignStmt(result, new PTXTernary.Expr(op, resultKind, op1, op2, op3)));
    return result;
  }
    ...
    getGen()
        .append(
            new PTXLIRStmt.AssignStmt(
                result,
                new PTXUnary.Intrinsic(RSQRT, LIRKind.value(operand.getPlatformKind()), operand)));
    return result;
  }
}
