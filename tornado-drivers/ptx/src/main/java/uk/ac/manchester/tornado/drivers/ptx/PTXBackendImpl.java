...
public final class PTXBackendImpl implements TornadoAcceleratorBackend {
  ...
  public PTXBackendImpl(
    ...
    if (deviceCount == 0) {
      throw new TornadoBailoutRuntimeException(
          "[WARNING] No PTX devices found. Deoptimizing to sequential execution.");
    }
  ...
  private void installDevice(
    ...
    backends[deviceIndex] =
        PTXHotSpotBackendFactory.createJITCompiler(options, vmRuntime, vmConfig, device);
  }
  ...
  public TornadoXPUDevice getDevice(int index) {
    if (index < backends.length) {
      ...
      throw new TornadoDeviceNotFound(
          "[ERROR]-[PTX-DRIVER] Device required not found: "
              + index
              + " - Max: "
              + backends.length);
    }
