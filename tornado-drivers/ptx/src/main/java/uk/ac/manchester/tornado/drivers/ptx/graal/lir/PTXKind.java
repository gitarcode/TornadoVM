...
public enum PTXKind implements PlatformKind {
  ...
  public static PTXKind fromResolvedJavaType(ResolvedJavaType elementType) {
    if (!elementType.isArray()) {
      for (PTXKind k : PTXKind.values()) {
        if (k.javaClass != null
            && (k.javaClass.getSimpleName().equalsIgnoreCase(elementType.getJavaKind().name())
                || k.javaClass.getSimpleName().equals(elementType.getUnqualifiedName()))) {
          return k;
        }
      }
    }
  ...
  public static PTXAssembler.PTXBinaryTemplate resolvePrivatePointerCopyTemplate(
