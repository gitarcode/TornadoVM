  ...
  public int read(
    ...
    if (partialReadSize != 0) {
      ...
      returnEvent =
          deviceContext.readBuffer(
              executionPlanId,
              toBuffer() + TornadoNativeArray.ARRAY_HEADER,
              numBytes,
              segment.address(),
  ...
  public void write(long executionPlanId, Object reference) {
    ...
    if (batchSize <= 0) {
      deviceContext.writeBuffer(
          executionPlanId, toBuffer(), bufferSize, segment.address(), 0, null);
    } else {
      throw new TornadoUnsupportedError(
          "[UNSUPPORTED] Batch processing for the writeBuffer operation");
    }
  }
