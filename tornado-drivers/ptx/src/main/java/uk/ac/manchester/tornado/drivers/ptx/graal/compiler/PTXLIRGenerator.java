...
public class PTXLIRGenerator extends LIRGenerator {
  ...
  public PTXLIRGenerator(Providers providers, LIRGenerationResult lirGenRes) {
    super(
        new PTXLIRKindTool((PTXTargetDescription) providers.getCodeCache().getTarget()),
        new PTXArithmeticTool(),
        new PTXBarrierSetLIRGenerator(),
        new PTXMoveFactory(),
        providers,
        lirGenRes);
  ...
  public LIRKind getLIRKind(Stamp stamp) {
    return (stamp instanceof PTXStamp)
        ? LIRKind.value(((PTXStamp) stamp).getPTXKind())
        : super.getLIRKind(stamp);
  }
  ...
  @Override
  public Variable emitLogicCompareAndSwap(
      LIRKind accessKind,
      Value address,
      Value expectedValue,
      Value newValue,
  ...
  @Override
  public Value emitValueCompareAndSwap(
      LIRKind accessKind,
      Value address,
      Value expectedValue,
      Value newValue,
  ...
  @Override
  public Value emitAtomicReadAndWrite(
      LIRKind accessKind, Value address, Value newValue, BarrierType barrierType) {
    return null;
  }
  ...
  public void emitDeoptimize(Value actionAndReason, Value failedSpeculation, LIRFrameState state) {
    ...
    Logger.traceBuildLIR(
        Logger.BACKEND.PTX, "emitDeoptimize: id=%d, reason=%s, action=%s", debugId, reason, action);
  ...
  public void emitReturn(JavaKind javaKind, Value input) {
    ...
    if (input != null) {
      ...
      if (returnKind.isVector()) {
        append(
            new PTXLIRStmt.VectorStoreStmt(
                (Variable) input,
                new PTXUnary.MemoryAccess(PTXArchitecture.paramSpace, returnVar, null)));
      } else {
  ...
  public void emitJump(LabelRef label, boolean isLoopEdgeBack) {
    Logger.traceBuildLIR(
        Logger.BACKEND.PTX, "emitJump: label=%s isLoopEdgeBack=%b", label, isLoopEdgeBack);
    append(new PTXControlFlow.Branch(label, false, isLoopEdgeBack));
  }

  @Override
  public void emitCompareBranch(
      PlatformKind cmpKind,
      Value left,
      Value right,
      Condition cond,
  ...
  public void emitOverflowCheckBranch(
      LabelRef overflow, LabelRef noOverflow, LIRKind cmpKind, double overflowProbability) {
    unimplemented();
  }

  @Override
  public void emitIntegerTestBranch(
      Value left,
      Value right,
      LabelRef trueDestination,
      LabelRef falseDestination,
  ...
  public Variable emitConditionalMove(
    ...
    append(
        new PTXLIRStmt.AssignStmt(
            predicate, new PTXBinary.Expr(getConditionalOp(cond), kind, left, right)));
    append(
        new PTXLIRStmt.AssignStmt(
            result,
            new PTXTernary.Expr(
                PTXAssembler.PTXTernaryOp.SELP, kind, trueValue, falseValue, predicate)));

    return result;
  }

  /**
   * It generates an IntegerTestMove operation, which moves a value to a parameter based on a
   ...
   */
  @Override
  public Variable emitIntegerTestMove(
      Value leftVal, Value right, Value trueValue, Value falseValue) {
    Logger.traceBuildLIR(
        Logger.BACKEND.PTX,
        "emitIntegerTestMove: "
            + leftVal
            ...
            + " : "
            + falseValue);
    assert leftVal.getPlatformKind() == right.getPlatformKind()
        && ((PTXKind) leftVal.getPlatformKind()).isInteger();
    ...
    final ConstantValue zeroConstant =
        new ConstantValue(andResult.getValueKind(), JavaConstant.forInt(0));
    final Variable predicate = newVariable(LIRKind.value(PTXKind.PRED));
    final Variable result = newVariable(kind);

    append(
        new PTXLIRStmt.AssignStmt(
            andResult, new PTXBinary.Expr(PTXBinaryOp.BITWISE_AND, kind, leftVal, right)));
    append(
        new PTXLIRStmt.AssignStmt(
            predicate, new PTXBinary.Expr(PTXBinaryOp.SETP_EQ, kind, andResult, zeroConstant)));
    append(
        new PTXLIRStmt.AssignStmt(
            result,
            new PTXTernary.Expr(
  ...
  @Override
  protected void emitForeignCallOp(
  ...
  @Override
  public void emitStrategySwitch(
    ...
    Logger.traceBuildLIR(
        Logger.BACKEND.PTX,
        "emitStrategySwitch: strategy=%s key=%s defaultTarget=%s",
        strategy,
        key,
        defaultTarget);
  ...
  @Override
  protected void emitRangeTableSwitch(
      int lowKey, LabelRef defaultTarget, LabelRef[] targets, AllocatableValue key) {}

  @Override
  protected void emitHashTableSwitch(
      JavaConstant[] keys,
      LabelRef defaultTarget,
      LabelRef[] targets,
  ...
  public Variable newReturnVariable(ValueKind<?> lirKind) {
    ...
    Logger.traceBuildLIR(
        Logger.BACKEND.PTX,
        "newReturnVariable: %s <- %s (%s)",
        variable.toString(),
        lirKind.toString(),
        lirKind.getClass().getName());
  ...
  public void emitParameterAlloc() {
    ...
    Variable kernelContextPointer =
        newVariable(LIRKind.value(PTXArchitecture.KERNEL_CONTEXT.getLirKind()));
    parameterAllocations.put(PTXArchitecture.KERNEL_CONTEXT.getName(), kernelContextPointer);
    append(
        new PTXLIRStmt.LoadStmt(
            new PTXUnary.MemoryAccess(PTXAssemblerConstants.KERNEL_CONTEXT_NAME),
            kernelContextPointer,
            PTXNullaryOp.LD));
  }

  public void emitConditionalBranch(
      LabelRef ref, Variable predicate, boolean isNegated, boolean isLoopEdgeBack) {
    append(
        new PTXLIRStmt.ConditionalStatement(
            new PTXControlFlow.Branch(ref, true, isLoopEdgeBack), predicate, isNegated));
  }
