...
public class PTXFPUnaryIntrinsicNode extends UnaryNode
    implements ArithmeticLIRLowerable, MarkFloatingPointIntrinsicsNode {

  public static final NodeClass<PTXFPUnaryIntrinsicNode> TYPE =
      NodeClass.create(PTXFPUnaryIntrinsicNode.class);
  ...
  public void generate(NodeLIRBuilderTool builder, ArithmeticLIRGeneratorTool lirGen) {
    Logger.traceBuildLIR(
        Logger.BACKEND.PTX, "emitPTXFPUnaryIntrinsic: op=%s, x=%s", operation, getValue());
    ...
    if (shouldConvertInput(initialInput)) {
      ...
      auxValue =
          builder.getLIRGeneratorTool().append(new AssignStmt(auxVar, initialInput)).getResult();
    }
      ...
      case COSPI:
        result =
            gen.genFloatCosPI(
                auxValue,
                createConstantForPI(),
                builder.getLIRGeneratorTool().newVariable(LIRKind.value(PTXKind.F32)),
                builder.getLIRGeneratorTool());
        break;
      case SINPI:
        result =
            gen.genFloatSinPI(
    ...
    if (shouldConvertInput(initialInput)) {
      ...
      auxVar =
          builder.getLIRGeneratorTool().newVariable(LIRKind.value(initialInput.getPlatformKind()));
  ...
  private void generateTan(
      NodeLIRBuilderTool builder, PTXArithmeticTool lirGen, PTXBuiltinTool gen, Value x) {
    Value auxValue = x;
    Variable auxVar;
    ...
    Value sin =
        builder
            .getLIRGeneratorTool()
            .append(new AssignStmt(sinVar, gen.genFloatSin(auxValue)))
            .getResult();
    Value cos =
        builder
            .getLIRGeneratorTool()
            .append(new AssignStmt(cosVar, gen.genFloatCos(auxValue)))
            .getResult();
  ...
  /**
   * Generates the instructions to compute a signum function in Java: signum(x), where x can be a
   * NaN number or a positive or negative value of type float or double.
   *
   * <p>The intrinsic follows the implementation in Java and calculates first if the input value is
   * zero or a NaN value. If that condition is true, the result of this intrinsic will take the
   * input value. Otherwise, it takes the value of the copySign function.
   *
   * <p>return (x == 0.0f || Float.isNaN(x))? x : copySign(1.0f, x);
   */
  public void generateSign(NodeLIRBuilderTool builder, PTXArithmeticTool lirGen, Value x) {
    ...
    ptxlirGenerator.append(
        new AssignStmt(
            equalZeroPred,
            new PTXBinary.Expr(
                PTXAssembler.PTXBinaryOp.SETP_EQ,
                LIRKind.value(PTXKind.F32),
                x,
                new ConstantValue(LIRKind.value(PTXKind.F32), PrimitiveConstant.FLOAT_0))));
    ...
    ptxlirGenerator.append(
        new PTXLIRStmt.AssignStmt(
            isNanPred,
            new PTXUnary.Expr(
                PTXAssembler.PTXUnaryOp.TESTP_NOTANUMBER, LIRKind.value(x.getPlatformKind()), x)));
    ...
    if (((PTXKind) x.getPlatformKind()).isF32()) {
      ...
      shouldNotReachHere(
          "The kind of the input parameter in the signum method is not float or double.");
    }
    ...
    ptxlirGenerator.append(
        new PTXLIRStmt.AssignStmt(
            result,
            new PTXTernary.Expr(
                PTXAssembler.PTXTernaryOp.SELP,
                LIRKind.value(x.getPlatformKind()),
                x,
                copySign,
                orPred)));
  ...
  /**
   * Generates the instructions to compute exponential function in Java: e ^ a, where e is Euler's
   * constant and a is an arbitrary number.
   *
   * <p>Because PTX cannot perform this computation directly, we use the functions available to
   * obtain the result. b = e ^ a = 2^(a * log2(e))
   *
   * <p>Because the log function only operates on single precision FPU, we must convert the input
   * and output to and from double precision FPU, if necessary.
   */
  public void generateExp(
    ...
    Value log2e =
        new ConstantValue(
            LIRKind.value(PTXKind.F32),
            JavaConstant.forFloat((float) (Math.log10(Math.exp(1)) / Math.log10(2))));
  ...
  /**
   * Generates the instructions to compute logarithmic function in Java: log_e(a), where e is
   * Euler's constant and a is an arbitrary number.
   *
   * <p>Because PTX cannot perform this computation directly, we use the log_2 function to obtain
   * the result. b = log_e(a) = log_2(a) / log_2(e)
   *
   * <p>Because the log function only operates on single precision FPU, we must convert the input
   * and output to and from double precision FPU, if necessary.
   */
  public void generateLog(
    ...
    Value nominator =
        builder
            .getLIRGeneratorTool()
            .append(new AssignStmt(var, gen.genFloatLog2(auxValue)))
            .getResult();
    Value denominator =
        new ConstantValue(
            LIRKind.value(PTXKind.F32),
  ...
  private boolean shouldConvertInput(Value input) {
    return (operation() == Operation.TAN
            || operation() == Operation.TANH
            || operation() == Operation.COS
            || operation() == Operation.COSPI
            || operation() == Operation.SIN
            || operation() == Operation.SINPI
  ...
  /**
   * Returns a {@link ConstantValue} that corresponds to the (pi/180) value which is represented by
   * {@value
   * uk.ac.manchester.tornado.drivers.ptx.graal.asm.PTXAssemblerConstants#DEGREES_TO_RADIANS}. This
   * value is used as the first parameter for the multiplication (mul.rn.f32) with a float value in
   * order to convert an angle measured in degrees to an approximately equivalent angle measured in
   * radians.
   */
  public Value getConstantValueForRadians(Value x) {
    ...
    if (((PTXKind) x.getPlatformKind()).isF32()) {
      constantValue =
          new ConstantValue(
              LIRKind.value(PTXKind.F32),
              JavaConstant.forFloat(PTXAssemblerConstants.DEGREES_TO_RADIANS));
    } else if (((PTXKind) x.getPlatformKind()).isF64()) {
      constantValue =
          new ConstantValue(
              LIRKind.value(PTXKind.F64),
              JavaConstant.forDouble(PTXAssemblerConstants.DEGREES_TO_RADIANS));
    } else {
  ...
  public Value createConstantForPI() {
    return new ConstantValue(
        LIRKind.value(PTXKind.F32), JavaConstant.forFloat(PTXAssemblerConstants.PI));
  }
