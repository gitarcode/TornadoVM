...
public class PTXDevice implements TornadoTargetDevice {
  ...
  public PTXDevice(int deviceIndex) {
    ...
    constantBufferSize =
        cuDeviceGetAttribute(cuDevice, PTXDeviceAttribute.TOTAL_CONSTANT_MEMORY.value());
    totalDeviceMemory = cuDeviceTotalMem(cuDevice);
    localMemorySize =
        cuDeviceGetAttribute(cuDevice, PTXDeviceAttribute.MAX_SHARED_MEMORY_PER_BLOCK.value());
    numComputeUnits =
        cuDeviceGetAttribute(cuDevice, PTXDeviceAttribute.MULTIPROCESSOR_COUNT.value());
    ...
    maxThreadsPerBlock =
        cuDeviceGetAttribute(cuDevice, PTXDeviceAttribute.MAX_THREADS_PER_BLOCK.value());
  ...
  private long[] initMaxWorkItemSizes() {
    ...
    maxWorkItemSizes[0] =
        cuDeviceGetAttribute(cuDevice, PTXDeviceAttribute.MAX_BLOCK_DIM_X.value());
    maxWorkItemSizes[1] =
        cuDeviceGetAttribute(cuDevice, PTXDeviceAttribute.MAX_BLOCK_DIM_Y.value());
    maxWorkItemSizes[2] =
        cuDeviceGetAttribute(cuDevice, PTXDeviceAttribute.MAX_BLOCK_DIM_Z.value());
