...
public class PTXNodeLIRBuilder extends NodeLIRBuilder {
  ...
  public PTXNodeLIRBuilder(
      StructuredGraph graph, LIRGeneratorTool lirGen, PTXNodeMatchRules ptxNodeMatchRules) {
  ...
  public void emitInvoke(Invoke x) {
    ...
    if (callTarget instanceof DirectCallTargetNode) {
      emitDirectCall(
          (DirectCallTargetNode) callTarget, result, parameters, AllocatableValue.NONE, callState);
    } else if (callTarget instanceof IndirectCallTargetNode) {
      emitIndirectCall(
          (IndirectCallTargetNode) callTarget,
          result,
          parameters,
          AllocatableValue.NONE,
          callState);
    } else {
      shouldNotReachHere();
    }
  ...
  @Override
  protected void emitDirectCall(
      DirectCallTargetNode callTarget,
      Value result,
      Value[] parameters,
      Value[] temps,
    ...
    Logger.traceBuildLIR(
        Logger.BACKEND.PTX,
        "emitDirectCall: callTarget=%s result=%s callState=%s",
        callTarget,
        result,
        callState);
    if (isLegal(result) && ((PTXKind) result.getPlatformKind()).isVector()) {
      ...
      ConstantValue paramSize =
          new ConstantValue(
              LIRKind.value(PTXKind.S32), JavaConstant.forInt(resultKind.getSizeInBytes()));
      PTXBinary.Expr declaration =
          new PTXBinary.Expr(
              PTXAssembler.PTXBinaryTemplate.NEW_ALIGNED_PARAM_BYTE_ARRAY,
              LIRKind.value(PTXKind.B8),
              returnBuffer,
              paramSize);
      ...
      append(
          new PTXLIRStmt.VectorLoadStmt(
              (Variable) result,
              new PTXUnary.MemoryAccess(PTXArchitecture.paramSpace, returnBuffer, null)));
      return;
    }
  ...
  @Override
  protected void emitIndirectCall(
      IndirectCallTargetNode callTarget,
      Value result,
      Value[] parameters,
      Value[] temps,
        ...
        getGen()
            .append(
                new PTXLIRStmt.AssignStmt(
                    result, new PTXNullary.Parameter(locals[param.index()].getName(), lirKind)));
  ...
  public void doBlock(
      HIRBlock block, StructuredGraph graph, BlockMap<List<Node>> blockMap, boolean isKernel) {
  ...
  private void emitLoopExit(LoopExitNode node) {
    if (gen.getCurrentBlock().getSuccessorCount() != 0) {
      append(
          new PTXControlFlow.LoopBreakOp(
  ...
  private void emitShortCircuitOrNode(ShortCircuitOrNode node) {
    ...
    Variable result =
        tool.emitBinaryAssign(PTXBinaryOp.BITWISE_OR, LIRKind.value(PTXKind.PRED), x, y);
    setResult(node, result);
  }

  /**
   * TODO A possible optimization is to perform the loop condition check once before the first
   * iteration and then for the rest of the iterations, have the condition check before the loop
   * back edge. This prevents an "useless" jump when the loop condition no longer holds.
   *
   * <p>Currently, with this method, we will jump back to the loop header and perform the loop
   * initialization on every iteration, instead of jumping to the first block in the loop body.
   */
  ...
  public void emitIf(final IfNode x) {
    Logger.traceBuildLIR(
        Logger.BACKEND.PTX, "emitIf: %s, condition=%s\n", x, x.condition().getClass().getName());
    ...
    if (isLoop) {
      getGen()
          .emitConditionalBranch(
              isNegated ? getLIRBlock(x.trueSuccessor()) : getLIRBlock(x.falseSuccessor()),
              predicate,
              !isNegated,
              false);
    } else {
  ...
  private Variable emitLogicNode(final LogicNode node) {
    ...
    if (node instanceof FloatEqualsNode) {
      ...
      append(
          new AssignStmt(
              ...
              new PTXBinary.Expr(
                  ...
                  boolLirKind,
                  andRes,
                  new ConstantValue(boolLirKind, PrimitiveConstant.INT_0))));
    } else if (node instanceof ShortCircuitOrNode) {
      ...
      throw new TornadoRuntimeException(
          String.format("logic node (class=%s)", node.getClass().getName()));
    }
    setResult(node, pred);
    return pred;
  }
  ...
  public void visitEndNode(final AbstractEndNode end) {
    ...
    for (ValuePhiNode phi : merge.valuePhis()) {
      ...
      if (!phi.isLoopPhi() && phi.singleValueOrThis() == phi
          || (value instanceof PhiNode && !((PhiNode) value).isLoopPhi())) {
        final AllocatableValue result = gen.asAllocatable(operandForPhi(phi));
        if (value instanceof FixedArrayNode) {
          // if a FixedArrayNode instance is assigned to a ValuePhiNode we have a conditional
          // copy-by-reference
    ...
    while (beginNode != null
        && beginNode.predecessor() != null
        && !(beginNode instanceof BeginNode)) {
  ...
  private void emitBranch(IfNode dominator) {
    ...
    boolean hasElse =
        dominator.trueSuccessor() instanceof BeginNode
            && dominator.falseSuccessor() instanceof BeginNode;
   ...
   * variables have been assigned previously. We do this because PTX only allows for the built-ins
   * to be used in <b>mov</b> and <b>cvt</b> instructions.
   */
