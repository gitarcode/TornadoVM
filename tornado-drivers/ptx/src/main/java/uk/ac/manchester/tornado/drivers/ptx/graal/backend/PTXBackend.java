...
public class PTXBackend extends XPUBackend<PTXProviders>
  ...
  public PTXBackend(
      ...
      PTXDeviceContext deviceContext,
      PTXTargetDescription target,
      PTXCodeProvider codeCache,
  ...
  public FrameMapBuilder newFrameMapBuilder(RegisterConfig registerConfig) {
    RegisterConfig nonNullRegisterConfig =
        (registerConfig == null) ? getCodeCache().getRegisterConfig() : registerConfig;
    return new PTXFrameMapBuilder(
        newFrameMap(nonNullRegisterConfig), getCodeCache(), registerConfig);
  }
  ...
  public PTXCompilationResultBuilder newCompilationResultBuilder(
    ...
    PTXCompilationResultBuilder crb =
        new PTXCompilationResultBuilder(
            getProviders(),
            frameMap,
            asm,
            dataBuilder,
  ...
  private void emitPrologue(
      PTXCompilationResultBuilder crb,
      PTXAssembler asm,
      PTXLIRGenerationResult lirGenRes,
      ResolvedJavaMethod method) {
      ...
      if (isKernel) {
        if (locals[i].getType().getJavaKind().isPrimitive() || isHalfFloat(locals[i].getType())) {
          ...
          if (locals[i].getType().toJavaName().equals(KernelContext.class.getName())) {
            asm.emit(", ");
            asm.emit(
                ".param .u64 .ptr .global .align 8 %s",
                PTXAssemblerConstants.KERNEL_CONTEXT_ARGUMENT_NAME);
            continue;
          }
          ...
          asm.emit(
              ".param .u64 .ptr .global .align %s %s", kind.getSizeInBytes(), locals[i].getName());
        }
      } else {
        final AllocatableValue param = incomingArguments.getArgument(i);
        ...
        if (locals[i].getType().getJavaKind().isObject()) {
          PTXKind tmpKind =
              PTXKind.resolveToVectorKind(locals[i].getType().resolve(method.getDeclaringClass()));
  ...
  private void emitFunctionHeader(
      PTXAssembler asm, ResolvedJavaMethod method, PTXLIRGenerationResult lirGenRes) {
    ...
    if (returnKind == JavaKind.Void) {
      ...
      PTXKind returnPtxKind =
          (returnType.getAnnotation(Vector.class) == null)
              ? getTarget().getPTXKind(returnKind)
              : PTXKind.fromResolvedJavaType(returnType);
      if (returnPtxKind.isVector()) {
        asm.emit(
            ".func (.param .align 8 .b8 %s[%d]) %s (",
            "retVar", returnPtxKind.getSizeInBytes(), methodName);
      } else {
  ...
  private void emitVariableDefs(PTXAssembler asm, PTXLIRGenerationResult lirGenRes) {
    ...
    Map<PTXKind, Set<PTXLIRGenerationResult.VariableData>> kindToVariable =
        lirGenRes.getVariableTable();
            ...
            IntStream.range(0, vectorSplitData.vectorNames.length)
                .forEach(
                    i ->
                        asm.emitLine(
                            "\t.reg .%s %s;",
                            type.getElementKind(), vectorSplitData.vectorNames[i]));
          } else {
            IntStream.range(0, vectorSplitData.vectorNames.length)
                .forEach(
                    i ->
