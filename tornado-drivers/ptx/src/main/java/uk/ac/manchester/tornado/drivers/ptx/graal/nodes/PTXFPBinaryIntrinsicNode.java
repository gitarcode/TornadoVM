...
public class PTXFPBinaryIntrinsicNode extends BinaryNode
    implements ArithmeticLIRLowerable, MarkFloatingPointIntrinsicsNode {

  public static final NodeClass<PTXFPBinaryIntrinsicNode> TYPE =
      NodeClass.create(PTXFPBinaryIntrinsicNode.class);
  ...
  public void generate(NodeLIRBuilderTool builder, ArithmeticLIRGeneratorTool lirGen) {
    Logger.traceBuildLIR(
        Logger.BACKEND.PTX, "emitPTXFPBinaryIntrinsic: op=%s, x=%s, y=%s", operation, x, y);
   ...
   * obtain the result. The pseudocode below illustrates what we do to compute z = x ^ y : if (x <
   * 0) x = -x; z = 2^(y * log2(x)); if (x < 0 && y % 2 == 1) z = -z;
   *
   * <p>Because the log function only operates on single precision FPU , we must convert the inputs
   * and output to and from double precision FPU, if necessary.
   *
   * <p>This snipet generates the following code: <code>
  ...
  private void generatePow(
      NodeLIRBuilderTool builder, PTXArithmeticTool lirGen, PTXBuiltinTool gen, Value x, Value y) {
    ...
    genTool.append(
        new AssignStmt(
            signPred,
            new PTXBinary.Expr(
                PTXAssembler.PTXBinaryOp.SETP_LT,
    ...
    genTool.append(
        new PTXLIRStmt.ConditionalStatement(
            new AssignStmt(
                auxVar,
                new PTXBinary.Expr(
                    PTXAssembler.PTXBinaryOp.MUL,
                    LIRKind.value(PTXKind.F32),
                    a,
                    new ConstantValue(LIRKind.value(PTXKind.F32), JavaConstant.forFloat(-1)))),
            signPred,
            false));
    ...
    Value aMulLog2e =
        genTool
            .append(
                new AssignStmt(
                    auxVar,
                    lirGen.genBinaryExpr(
                        PTXAssembler.PTXBinaryOp.MUL, LIRKind.value(PTXKind.F32), b, log2x)))
            .getResult();
    ...
    genTool.append(
        new AssignStmt(
            auxInt,
            new PTXBinary.Expr(
                PTXAssembler.PTXBinaryOp.REM,
                LIRKind.value(PTXKind.S32),
                auxInt,
                new ConstantValue(LIRKind.value(PTXKind.S32), PrimitiveConstant.INT_2))));
    genTool.append(
        new AssignStmt(
            remPred,
            new PTXBinary.Expr(
                ...
                LIRKind.value(PTXKind.S32),
                auxInt,
                new ConstantValue(LIRKind.value(PTXKind.S32), PrimitiveConstant.INT_1))));
    genTool.append(
        new AssignStmt(
            signPred,
            new PTXBinary.Expr(
                PTXAssembler.PTXBinaryOp.BITWISE_AND,
                LIRKind.value(PTXKind.PRED),
                signPred,
                remPred)));
    ...
    genTool.append(
        new PTXLIRStmt.ConditionalStatement(
            ...
            false));
    genTool.append(
        new PTXLIRStmt.ConditionalStatement(new AssignStmt(auxVar, auxVar), signPred, true));
