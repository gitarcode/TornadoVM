...
public class PTXGenTool {
  ...
  public Value emitParameterLoad(Local local, ParameterNode paramNode) {
    Logger.traceBuildLIR(
        Logger.BACKEND.PTX, "emitParameterLoad: stamp=%s", paramNode.stamp(NodeView.DEFAULT));
    ...
    Variable result =
        (kind.isVector())
            ? gen.newVariable(LIRKind.value(target.getPTXKind(JavaKind.Object)))
            : gen.newVariable(lirKind);
    gen.append(
        new PTXLIRStmt.LoadStmt(
            new PTXUnary.MemoryAccess(local.getName()), result, PTXAssembler.PTXNullaryOp.LD));
