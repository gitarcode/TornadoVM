...
/**
 * Applies partial unroll on counted loops of more than 128 elements. By default, the unroll factor
 * is set to 2 except if the user explicitly passes a different value power of two.
 *
...
public class TornadoPartialLoopUnroll extends BasePhase<MidTierContext> {
  ...
  private static int getUnrollFactor() {
    return (isPowerOfTwo(TornadoOptions.UNROLL_FACTOR) && TornadoOptions.UNROLL_FACTOR <= 32)
        ? TornadoOptions.UNROLL_FACTOR
        : LOOP_UNROLL_FACTOR_DEFAULT;
  }
