...
public class TornadoPrivateArrayPiRemoval extends BasePhase<TornadoHighTierContext> {
  ...
  protected void run(StructuredGraph graph, TornadoHighTierContext context) {
    for (NewArrayNonVirtualizableNode fixedArray :
        graph.getNodes().filter(NewArrayNonVirtualizableNode.class)) {
      if (fixedArray.successors().filter(PanamaPrivateMemoryNode.class).isNotEmpty()) {
        ...
        PanamaPrivateMemoryNode panamaPrivateMemoryNode =
            fixedArray.successors().filter(PanamaPrivateMemoryNode.class).first();
