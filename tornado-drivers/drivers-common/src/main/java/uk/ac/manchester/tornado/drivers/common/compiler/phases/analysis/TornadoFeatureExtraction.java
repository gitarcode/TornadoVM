...
public class TornadoFeatureExtraction extends Phase {
  ...
  private LinkedHashMap<ProfilerCodeFeatures, Integer> extractFeatures(
      StructuredGraph graph, LinkedHashMap<ProfilerCodeFeatures, Integer> initMap) {
    ...
    for (Node node : graph.getNodes().snapshot()) {
      if (node instanceof MulNode
        ...
        irFeatures.put(
            ProfilerCodeFeatures.CASE,
            (countCases + ((IntegerSwitchNode) node).getSuccessorCount()));
        ...
        updateCounter(irFeatures, ProfilerCodeFeatures.PARALLEL_LOOPS);
          ...
          || node instanceof SignExtendNode) {
      ...
      } else if (node instanceof MarkFloatingPointIntrinsicsNode
  ...
  private void updateCounter(
      LinkedHashMap<ProfilerCodeFeatures, Integer> irFeatures, ProfilerCodeFeatures feature) {
  ...
  private void updateWithType(LinkedHashMap<ProfilerCodeFeatures, Integer> irFeatures, Node node) {
    ...
    if (opType == (JavaKind.Boolean)
        || (opType == JavaKind.Char)
        || (opType == JavaKind.Int)
        || (opType == JavaKind.Short)
        || (opType == JavaKind.Long)) {
  ...
  private void updateMemoryAccesses(
      LinkedHashMap<ProfilerCodeFeatures, Integer> irFeatures, Node node, boolean isLoad) {
