...
public class CodeUtil {
  ...
  public static CallingConvention getCallingConvention(
      CodeCacheProvider codeCache, CallingConvention.Type type, ResolvedJavaMethod method) {
  ...
  private static CallingConvention getCallingConvention(
    ...
    for (int i = 0; i < argTypes.length; i++, variableIndex++) {
      if (isHalfFloat(argTypes[i])) {
        ...
        inputParameters[i] =
            new Variable(LIRKind.value(target.arch.getPlatformKind(JavaKind.Short)), variableIndex);
        continue;
      }

      var javaKind = convertJavaKind(argTypes[i]);
      inputParameters[i] =
          new Variable(LIRKind.value(target.arch.getPlatformKind(javaKind)), variableIndex);
    }
  ...
  /**
   * Convert a {@link JavaType} to a {@link JavaKind}, all wrappers for primitive types are
   * converted to the corresponding {@link JavaKind} of the primitive type.
   *
   * @param type
   * @return
   */
