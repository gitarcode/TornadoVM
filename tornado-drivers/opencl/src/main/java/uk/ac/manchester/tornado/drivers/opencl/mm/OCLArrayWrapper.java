...
public abstract class OCLArrayWrapper<T> implements XPUBuffer {
  ...
  protected OCLArrayWrapper(
      final T array, final OCLDeviceContext device, final JavaKind kind, long batchSize) {
   ...
   */
  protected abstract int enqueueReadArrayData(
      long executionPlanId,
      long bufferId,
      long offset,
      long bytes,
    ...
    if (batchSize <= 0) {
      headerEvent =
          buildArrayHeader(Array.getLength(array))
              .enqueueWrite(executionPlanId, (useDeps) ? events : null);
    } else {
      headerEvent =
          buildArrayHeaderBatch(batchSize).enqueueWrite(executionPlanId, (useDeps) ? events : null);
    }
    final int returnEvent =
   ...
   * @return Event information
   */
  protected abstract int enqueueWriteArrayData(
      long executionPlanId,
      long bufferId,
      long offset,
      long bytes,
  ...
  private OCLByteBuffer getArrayHeader() {
    final OCLByteBuffer header =
        new OCLByteBuffer(deviceContext, bufferId, bufferOffset, arrayHeaderSize);
    header.buffer.clear();
    return header;
  }
   ...
   *
   * @param value in which the data are copied
   * @param hostOffset offset, in bytes, from the input value in which to perform the read.
   * @param events list of pending events.
   * @param useDeps flag to indicate dependencies should be carried for the next operation.
   */
  @Override
  public int read(
      long executionPlanId,
    ...
    final long numBytes =
        getSizeSubRegionSize() > 0 ? getSizeSubRegionSize() : (bufferSize - arrayHeaderSize);
  ...
  protected abstract int readArrayData(
      long executionPlanId,
      long bufferId,
      long offset,
      long bytes,
  ...
  protected abstract void writeArrayData(
      long executionPlanId,
      long bufferId,
      long offset,
      long bytes,
