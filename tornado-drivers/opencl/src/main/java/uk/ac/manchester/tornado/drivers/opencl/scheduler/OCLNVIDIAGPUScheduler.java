...
public class OCLNVIDIAGPUScheduler extends OCLKernelScheduler {
  ...
  public void calculateLocalWork(final TaskDataContext meta) {
    ...
    switch (meta.getDims()) {
      case 3:
        localWork[2] =
            calculateGroupSize(
                calculateEffectiveMaxWorkItemSizes(meta)[2], meta.getGlobalWork()[2], 3);
        localWork[1] =
            calculateGroupSize(
                calculateEffectiveMaxWorkItemSizes(meta)[1], meta.getGlobalWork()[1], 3);
        localWork[0] =
            calculateGroupSize(
                calculateEffectiveMaxWorkItemSizes(meta)[0], meta.getGlobalWork()[0], 3);
        break;
      case 2:
        localWork[1] =
            calculateGroupSize(
                calculateEffectiveMaxWorkItemSizes(meta)[1], meta.getGlobalWork()[1], 2);
        localWork[0] =
            calculateGroupSize(
                calculateEffectiveMaxWorkItemSizes(meta)[0], meta.getGlobalWork()[0], 2);
        break;
      case 1:
        localWork[0] =
            calculateGroupSize(
                calculateEffectiveMaxWorkItemSizes(meta)[0], meta.getGlobalWork()[0], 1);
        break;
      default:
        break;
    }
  ...
  /**
   * Checks if the selected local work group does not exceed the maximum work group size permitted
   * by the driver. If it does, it uses a heuristic to set the local work group.
   *
