...
public class OCLLIRGenerator extends LIRGenerator {
  ...
  public OCLLIRGenerator(CodeGenProviders providers, LIRGenerationResult res) {
    super(
        new OCLLIRKindTool((OCLTargetDescription) providers.getCodeCache().getTarget()),
        new OCLArithmeticTool(),
        new OCLBarrierSetLIRGenerator() {},
        new OCLMoveFactory(),
        providers,
        res);
  ...
  public LIRKind getLIRKind(Stamp stamp) {
    return (stamp instanceof OCLStamp)
        ? LIRKind.value(((OCLStamp) stamp).getOCLKind())
  ...
  public Variable emitConditionalMove(
    ...
    final OCLTernary.Select selectExpr =
        new OCLTernary.Select(
            LIRKind.combine(trueValue, falseValue), condExpr, trueValue, falseValue);
  ...
  public void emitDeoptimize(Value actionAndReason, Value speculation, LIRFrameState state) {
    ...
    Logger.traceBuildLIR(
        Logger.BACKEND.OpenCL,
        "emitDeoptimize: id=%d, reason=%s, action=%s",
        debugId,
        reason,
        action);
  ...
  public void emitIntegerTestBranch(
      Value value, Value value1, LabelRef lr, LabelRef lr1, double d) {
    unimplemented();
  }
  ...
  public Variable emitIntegerTestMove(Value left, Value right, Value trueValue, Value falseValue) {
    Logger.traceBuildLIR(
        ...
        "emitIntegerTestMove: "
            ...
            + " : "
            + falseValue);
    assert left.getPlatformKind() == right.getPlatformKind()
        && ((OCLKind) left.getPlatformKind()).isInteger();
    ...
    final OCLTernary.Select selectExpr =
        new OCLTernary.Select(
            LIRKind.combine(trueValue, falseValue), condExpr, falseValue, trueValue);
