...
public class OCLJIT {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      System.out.printf(
          "method: name=%s, signature=%s\n",
          resolvedMethod.getName(), resolvedMethod.getSignature());

      final OCLBackend backend =
          getTornadoRuntime().getBackend(OCLBackendImpl.class).getDefaultBackend();

      TaskDataContext meta = TaskDataContext.create(new ScheduleContext("s0"), methodName, method);

      OCLCompilationResult result =
          OCLCompiler.compileCodeForDevice(
              resolvedMethod,
              new Object[] {},
              meta,
              (OCLProviders) backend.getProviders(),
              backend,
              new EmptyProfiler());
