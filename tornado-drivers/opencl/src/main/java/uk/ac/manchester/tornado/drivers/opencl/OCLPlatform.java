...
public class OCLPlatform implements TornadoPlatformInterface {
  ...
  public OCLPlatform(int index, long platformPointers) {
    ...
    if (isVendor(Vendor.XILINX)) {
      deviceCount =
          clGetDeviceCount(platformPointers, OCLDeviceType.CL_DEVICE_TYPE_ACCELERATOR.getValue());
  ...
  public String toString() {
    String sb =
        String.format("name=%s, num. devices=%d, ", getName(), devices.size())
            + String.format("version=%s", getVersion());
    return sb.trim();
  }
