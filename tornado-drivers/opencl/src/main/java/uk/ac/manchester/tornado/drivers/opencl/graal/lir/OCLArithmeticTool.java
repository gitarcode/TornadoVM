...
public class OCLArithmeticTool extends ArithmeticLIRGenerator {
  ...
  public Value emitZeroExtend(Value value, int fromBits, int toBits) {
    Logger.traceBuildLIR(
        Logger.BACKEND.OpenCL, "emitZeroExtend: %s (from %d to %d)", value, fromBits, toBits);
  ...
  private boolean shouldEmitIntegerIndexes(OCLAddressCast cast) {
    return cast.getMemorySpace().name() == OCLAssemblerConstants.LOCAL_MEM_MODIFIER
        || cast.getMemorySpace().name() == OCLAssemblerConstants.PRIVATE_MEM_MODIFIER;
  }

  public void emitVectorLoad(
  ...
  public Variable emitLoad(
    ...
    Logger.traceBuildLIR(
        Logger.BACKEND.OpenCL, "emitLoad: %s <- %s\nstate:%s", kind, address, state);
    ...
    if (oclKind.isVector()) {
      ...
      emitVectorLoad(
          result,
          intrinsic,
          getOffsetValue(oclKind, (MemoryAccess) address),
          cast,
          (MemoryAccess) address);
    } else {
  ...
  public void emitStore(
      ...
      MemoryOrderMode memoryOrder) {
    Logger.traceBuildLIR(
        Logger.BACKEND.OpenCL, "emitStore: kind=%s, address=%s, input=%s", kind, address, input);
    guarantee(kind.getPlatformKind() instanceof OCLKind, "invalid LIRKind: %s", kind);
    OCLKind oclKind = (OCLKind) kind.getPlatformKind();
    ...
    if (oclKind.isVector()) {
      ...
      OCLAddressCast cast =
          new OCLAddressCast(memAccess.getBase(), LIRKind.value(oclKind.getElementKind()));
      getGen()
          .append(
              new VectorStoreStmt(
                  intrinsic, getOffsetValue(oclKind, memAccess), cast, memAccess, input));
    } else {
  ...
  /**
   * It calculates and returns the offset for vstore/vload operations as a Value object.
   *
   * @param oclKind the kind for getting the size of the element type in a vector
   * @param memoryAccess the object that holds the index of an element in a vector
   * @return {@link Value }
   */
  private Value getPrivateOffsetValue(OCLKind oclKind, MemoryAccess memoryAccess) {
    ...
    int index =
        Integer.parseInt(OCLAssembler.getAbsoluteIndexFromValue(memoryAccess.getIndex()))
            / oclKind.getVectorLength();
    return new ConstantValue(LIRKind.value(OCLKind.INT), JavaConstant.forInt(index));
  }

  private Value getOffsetValue(OCLKind oclKind, MemoryAccess memoryAccess) {
    if (memoryAccess.getBase().getMemorySpace()
        == OCLMemorySpace.GLOBAL.getBase().getMemorySpace()) {
    ...
    getGen()
        .append(
            new OCLLIRStmt.AssignStmt(
                result, new OCLTernary.Expr(operation, resultKind, op1, op2, op3)));
    return result;
  }
  ...
  public Value emitRSQRT(Value op) {
    ...
    getGen()
        .append(
            new OCLLIRStmt.AssignStmt(
                result, new OCLUnary.Intrinsic(RSQRT, LIRKind.value(op.getPlatformKind()), op)));
    return result;
  }
}
