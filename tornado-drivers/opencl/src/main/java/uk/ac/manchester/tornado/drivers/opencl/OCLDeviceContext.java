...
public class OCLDeviceContext implements OCLDeviceContextInterface {
  ...
  /**
   * Table to represent {@link uk.ac.manchester.tornado.api.TornadoExecutionPlan} -> {@link
   * OCLCommandQueueTable}
   */
  ...
  public int enqueueBarrier(long executionPlanId) {
    ...
    return (commandQueue.getOpenclVersion() < 120)
        ? -1
  ...
  public int enqueueMarker(long executionPlanId) {
    ...
    return commandQueue.getOpenclVersion() < 120
        ? -1
        : eventPool.registerEvent(oclEvent, EventDescriptor.DESC_SYNC_MARKER, commandQueue);
  }
  ...
  public int enqueueNDRangeKernel(
  ...
  public int enqueueWriteBuffer(
    ...
    OCLEventPool eventPool = getOCLEventPool(executionPlanId);
    return eventPool.registerEvent(
        commandQueue.enqueueWrite(
            bufferId,
            OpenCLBlocking.FALSE,
            offset,
            bytes,
            array,
            hostOffset,
    ...
    return eventPool.registerEvent(
        commandQueue.enqueueRead(
            ...
            eventPool.serialiseEvents(waitEvents, commandQueue)
                ? eventPool.waitEventsBuffer
                : null),
        EventDescriptor.DESC_READ_SEGMENT,
        commandQueue);
  }
    ...
    eventPool.registerEvent(
        commandQueue.enqueueWrite(
            ...
            eventPool.serialiseEvents(waitEvents, commandQueue)
                ? eventPool.waitEventsBuffer
                : null),
        EventDescriptor.DESC_WRITE_SEGMENT,
        commandQueue);
  }
  ...
  public int enqueueBarrier(long executionPlanId, int[] events) {
    ...
    long oclEvent =
        commandQueue.enqueueBarrier(
            eventPool.serialiseEvents(events, commandQueue) ? eventPool.waitEventsBuffer : null);
    return commandQueue.getOpenclVersion() < 120
        ? -1
        : eventPool.registerEvent(oclEvent, EventDescriptor.DESC_SYNC_BARRIER, commandQueue);
  }
  ...
  public void dumpEvents() {
    ...
    for (Long id : executionPlanIds) {
      ...
      events.sort(
          Comparator.comparingLong(OCLEvent::getCLSubmitTime)
              .thenComparingLong(OCLEvent::getCLStartTime));
      long base = events.getFirst().getCLSubmitTime();
      System.out.println("event: device,type,info,queued,submitted,start,end,status");
      events.forEach(
          event ->
              System.out.printf(
                  "event: %s,%s,%s,0x%x,%d,%d,%d,%s\n",
                  deviceName,
                  event.getName(),
                  event.getOclEventID(),
                  event.getCLQueuedTime() - base,
                  event.getCLSubmitTime() - base,
                  event.getCLStartTime() - base,
                  event.getCLEndTime() - base,
                  event.getStatus()));
    }
  }
  ...
  public boolean isPlatformFPGA() {
    return this.getDevice().getDeviceType() == OCLDeviceType.CL_DEVICE_TYPE_ACCELERATOR
        && (getPlatformContext().getPlatform().getName().toLowerCase().contains("fpga")
  ...
  public Event resolveEvent(long executionPlanId, int event) {
    ...
    OCLEventPool eventPool = getOCLEventPool(executionPlanId);
    return new OCLEvent(
        eventPool.getDescriptor(event).getNameDescription(),
        commandQueue,
        event,
        eventPool.getOCLEvent(event));
  }
  ...
  @Override
  public OCLInstalledCode installCode(
      TaskDataContext meta, String id, String entryPoint, byte[] code) {
  ...
  @Override
  public OCLInstalledCode installCode(
      String id, String entryPoint, byte[] code, boolean printKernel) {
