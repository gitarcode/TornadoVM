...
public class TornadoAtomicIntegerNode extends FixedWithNextNode implements LIRLowerable {
  ...
  public static final NodeClass<TornadoAtomicIntegerNode> TYPE =
      NodeClass.create(TornadoAtomicIntegerNode.class);
  ...
  public static HashMap<ResolvedJavaMethod, HashMap<Integer, Integer>> globalAtomicsParameters =
      new HashMap<>();

  private static final int DEFAULT_VALUE = -1;

  @Input ValueNode initialValue;
  ...
  private void generateExpressionForOpenCL2_0(NodeLIRBuilderTool gen) {
    ...
    tool.append(
        new OCLLIRStmt.RelocatedExpressionStmt(
            new OCLUnary.IntrinsicAtomicDeclaration(
                OCLAssembler.OCLUnaryIntrinsic.ATOMIC_VAR_INIT,
                result,
                gen.operand(initialValue))));
  ...
  /**
   * Method to reserve a position in the atomic-int global buffer and map the parameter index with
   * the assigned position. The mapping-table is obtained at runtime for streaming in and out data
   * in the right positions of the atomic buffer.
   *
   * @param paramIndex Object parameter index taken from {@link
