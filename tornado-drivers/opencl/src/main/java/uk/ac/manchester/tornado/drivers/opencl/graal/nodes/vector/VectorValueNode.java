...
public class VectorValueNode extends FloatingNode implements LIRLowerable {
  ...
  private Value getParam(NodeLIRBuilderTool gen, LIRGeneratorTool tool, int index) {
    ...
    return (valueNode == null)
        ? new ConstantValue(LIRKind.value(kind), kind.getDefaultValue())
      ...
      case 8:
        {
          ...
          assignExpr =
              new OCLVectorAssign.Assign8Expr(op8, getOCLKind(), s0, s1, s2, s3, s4, s5, s6, s7);
          break;
        }
      case 16:
        {
          ...
          assignExpr =
              new OCLVectorAssign.Assign16Expr(
                  op16,
                  getOCLKind(),
                  s0,
                  s1,
  ...
  public boolean allLanesSet() {
    return (values.count() == 1 && values.first() instanceof VectorValueNode)
        || (values.filter(VectorLoadElementNode.class).count() == kind.getVectorLength());
  }
  ...
  public void deleteUnusedLoads() {
    usages()
        .filter(VectorLoadElementNode.class)
        .forEach(
            node -> {
              if (node.hasNoUsages()) {
                values.remove(node);
