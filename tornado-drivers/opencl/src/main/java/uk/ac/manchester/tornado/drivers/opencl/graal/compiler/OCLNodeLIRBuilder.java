...
public class OCLNodeLIRBuilder extends NodeLIRBuilder {
  ...
  public OCLNodeLIRBuilder(
      final StructuredGraph graph, final LIRGeneratorTool gen, NodeMatchRules nodeMatchRules) {
          ...
          if (!hasOperand(valueNode)) {
            if (!peephole(valueNode)) {
              try {
                ...
                if (e
                    instanceof
                    TornadoDeviceFP64NotSupported tornadoDeviceFP64NotSupportedException) {
  ...
  private void platformPatch(boolean isKernel) {
    final List<LIRInstruction> insns =
        getLIRGeneratorTool().getResult().getLIR().getLIRforBlock(gen.getCurrentBlock());
    ...
    if (op instanceof ExprStmt exprStmtOp) {
      ...
      if (expr.getExpr() instanceof OCLUnary.Expr
          && ((OCLUnary.Expr) expr.getExpr()).getOpcode().equals(OCLUnaryOp.RETURN)) {
        OCLUnary.Expr returnExpr = (OCLUnary.Expr) expr.getExpr();
        append(
            new ExprStmt(new OCLNullary.Expr(OCLNullaryOp.RETURN, LIRKind.value(OCLKind.ILLEGAL))));
        insns.remove(index);
  ...
  private Value emitNegatedLogicNode(final LogicNode node) {
    ...
    if (node instanceof LogicalEqualsNode) {
      ...
      result =
          getGen()
              .getArithmetic()
              .genBinaryExpr(OCLBinaryIntrinsicCmp.FLOAT_IS_NOT_EQUAL, intLirKind, x, y);
      ...
      result =
          getGen()
              .getArithmetic()
              .genBinaryExpr(OCLBinaryIntrinsicCmp.FLOAT_IS_GREATEREQUAL, intLirKind, x, y);
    } else if (node instanceof IntegerBelowNode) {
      final IntegerBelowNode condition = (IntegerBelowNode) node;
      final Value x = operand(condition.getX());
      final Value y = operand(condition.getY());
      result =
          getGen().getArithmetic().genBinaryExpr(OCLBinaryOp.RELATIONAL_GTE, boolLirKind, x, y);
      ...
      result =
          getGen()
              .getArithmetic()
              .genTestNegateBinaryExpr(OCLBinaryOp.BITWISE_AND, boolLirKind, x, y);
    } else {
  ...
  private OCLLIROp emitLogicNode(final LogicNode node) {
    ...
    if (node instanceof LogicalEqualsNode) {
      ...
      result =
          getGen()
              .getArithmetic()
              .genBinaryExpr(OCLBinaryIntrinsicCmp.FLOAT_IS_EQUAL, intLirKind, x, y);
    } else if (node instanceof FloatLessThanNode) {
      final FloatLessThanNode condition = (FloatLessThanNode) node;
      final Value x = operand(condition.getX());
      final Value y = operand(condition.getY());
      result =
          getGen()
              .getArithmetic()
              .genBinaryExpr(OCLBinaryIntrinsicCmp.FLOAT_IS_LESS, intLirKind, x, y);
    } else if (node instanceof IntegerBelowNode) {
      final IntegerBelowNode condition = (IntegerBelowNode) node;
      final Value x = operand(condition.getX());
      final Value y = operand(condition.getY());
      ...
      final Value y = operand(testNode.getY());
      result =
          getGen().getArithmetic().genTestBinaryExpr(OCLBinaryOp.BITWISE_AND, boolLirKind, x, y);
    } else {
      throw new TornadoRuntimeException(
          String.format("logic node (class=%s)", node.getClass().getName()));
    }
  ...
  protected void emitDirectCall(
      final DirectCallTargetNode callTarget,
      final Value result,
  ...
  protected void emitIndirectCall(
      final IndirectCallTargetNode arg0,
      final Value arg1,
      final Value[] arg2,
      final Value[] arg3,
  ...
  public void emitIf(final IfNode x) {
    Logger.traceBuildLIR(
        Logger.BACKEND.OpenCL, "emitIf: %s, condition=%s\n", x, x.condition().getClass().getName());
    ...
    final Value condition =
        (invertedLoop) ? emitNegatedLogicNode(x.condition()) : emitLogicNode(x.condition());
  ...
  protected void emitNode(final ValueNode node) {
    ...
    } else if (node instanceof IntelUnrollPragmaNode
        || node instanceof XilinxPipeliningPragmaNode
        || node instanceof FPGAWorkGroupSizeNode) {
      // ignore emit-action
    } else {
      super.emitNode(node);
    }
  ...
  private void emitOCLFPGAPragmas(HIRBlock block) {
    for (ValueNode tempDomBlockNode : block.getNodes()) {
      if (tempDomBlockNode instanceof IntelUnrollPragmaNode
          || tempDomBlockNode instanceof XilinxPipeliningPragmaNode) {
