...
public class OCLCommandQueue extends CommandQueue {
  ...
  /**
   * Small buffer for querying properties regarding the command queue. This is useful for debugging.
   */
  ...
  static native void clGetCommandQueueInfo(long queueId, int info, byte[] buffer)
      throws OCLException;

  /**
   * Dispatch an OpenCL kernel via a JNI call.
   *
   * @param queueId OpenCL command queue object
   * @param kernelId OpenCL kernel ID object
   * @param dim Dimensions of the Kernel (1D, 2D or 3D)
   * @param global_work_offset Offset within global access
   * @param global_work_size Total number of threads to launch
   * @param local_work_size Local work group size
   * @param events List of events
   * @return Returns an event's ID
   * @throws OCLException OpenCL Exception
   */
  static native long clEnqueueNDRangeKernel(
      long queueId,
      long kernelId,
      int dim,
  ...
  static native long writeArrayToDevice(
      long queueId,
      byte[] buffer,
      long hostOffset,
      boolean blocking,
  ...
  static native long readArrayFromDevice(
      long queueId,
      byte[] buffer,
      long hostOffset,
      boolean blocking,
  ...
  static native long readArrayFromDeviceOffHeap(
      long queueId,
      long hostPointer,
      long hostOffset,
      boolean blocking,
  ...
  public long enqueueNDRangeKernel(
      OCLKernel kernel,
      int dim,
    ...
    try {
      return clEnqueueNDRangeKernel(
          commandQueuePtr,
          kernel.getOclKernelID(),
          dim,
          (openclVersion > 100) ? globalWorkOffset : null,
          globalWorkSize,
          localWorkSize,
          waitEvents);
    } catch (OCLException e) {
  ...
  public long enqueueWrite(
    ...
    try {
      return writeArrayToDevice(
          commandQueuePtr, array, hostOffset, blocking, offset, bytes, devicePtr, waitEvents);
    } catch (OCLException e) {
      logger.error(e.getMessage());
      throw new TornadoBailoutRuntimeException(e.getMessage());
    }
  ...
  public long enqueueWrite(
    ...
    try {
      return writeArrayToDevice(
          commandQueuePtr, hostPointer, hostOffset, blocking, offset, bytes, devicePtr, waitEvents);
    } catch (OCLException e) {
      logger.error(e.getMessage());
      throw new TornadoBailoutRuntimeException(e.getMessage());
    }
  ...
  public long enqueueRead(
    ...
    try {
      return readArrayFromDeviceOffHeap(
          commandQueuePtr, hostPointer, hostOffset, blocking, offset, bytes, devicePtr, waitEvents);
    } catch (OCLException e) {
      logger.error(e.getMessage());
      throw new TornadoBailoutRuntimeException(e.getMessage());
  ...
  public long enqueueBarrier(long[] waitEvents) {
    return (openclVersion < 120)
        ? enqueueBarrier_OCLv1_1(waitEvents)
        : enqueueBarrier_OCLv1_2(waitEvents);
  }
