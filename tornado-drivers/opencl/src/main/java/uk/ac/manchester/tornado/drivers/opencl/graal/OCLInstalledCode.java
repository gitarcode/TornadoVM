...
public class OCLInstalledCode extends InstalledCode implements TornadoInstalledCode {
   ...
   *
   * @param kernelArgs OpenCL kernel parameters {@link OCLByteBuffer}
   * @param meta task metadata {@link TaskDataContext}
   */
  private void setKernelArgs(
      final OCLKernelStackFrame kernelArgs, final XPUBuffer atomicSpace, TaskDataContext meta) {
    int index = 0;
    ...
    if (meta != null && meta.getLocalSize() > 0) {
      logger.info(
          "\tallocating %s of local memory",
          RuntimeUtilities.humanReadableByteCount(meta.getLocalSize(), true));
  ...
  private void printDebugLaunchInfo(final TaskDataContext meta) {
    ...
    if (meta.getXPUDevice() instanceof OCLTornadoDevice) {
      System.out.println(
          "\tDevice  : "
              + ((OCLTornadoDevice) meta.getXPUDevice()).getPhysicalDevice().getDeviceName());
    }
  }

  public int submitWithEvents(
      long executionPlanId,
      final OCLKernelStackFrame kernelArgs,
      final XPUBuffer atomicSpace,
    ...
    if (DEBUG) {
      logger.info(
          "kernel submitted: id=0x%x, method = %s, device =%s",
          kernel.getOclKernelID(), kernel.getName(), deviceContext.getDevice().getDeviceName());
    }
    ...
    if (meta == null) {
      task =
          deviceContext.enqueueNDRangeKernel(
              executionPlanId,
              kernel,
              1,
              null,
              singleThreadGlobalWorkSize,
              singleThreadLocalWorkSize,
              waitEvents);
    } else {
      if (meta.isParallel()) {
  ...
  private int submitSequential(long executionPlanId, final TaskDataContext meta) {
    ...
    if ((meta.getGlobalWork() == null) || (meta.getGlobalWork().length == 0)) {
      ...
      task =
          deviceContext.enqueueNDRangeKernel(
              executionPlanId, kernel, 1, null, meta.getGlobalWork(), meta.getLocalWork(), null);
    }
  ...
  private void launchKernel(
      long executionPlanId,
      final OCLKernelStackFrame callWrapper,
      final TaskDataContext meta,
      long batchThreads) {
  ...
  private void checkKernelNotNull() {
    if (kernel == null) {
      throw new TornadoRuntimeException(
          "[ERROR] Generated Kernel is NULL. \n"
              + "Please report this issue to https://github.com/beehive-lab/TornadoVM");
    }
  }

  private void submitWithoutEvents(
      long executionPlanId,
      final OCLKernelStackFrame oclKernelStackFrame,
      final XPUBuffer atomicSpace,
      final TaskDataContext meta,
      long batchThreads) {
    checkKernelNotNull();
    ...
    updateProfilerKernelContextWrite(
        executionPlanId, kernelContextWriteEventId, meta, oclKernelStackFrame);
  ...
  private void updateProfilerKernelContextWrite(
      ...
      OCLKernelStackFrame callWrapper) {
    if (TornadoOptions.isProfilerEnabled()) {
  ...
  public int launchWithoutDependencies(
      ...
      long batchThreads) {
    submitWithoutEvents(
        executionPlanId, (OCLKernelStackFrame) callWrapper, atomicSpace, meta, batchThreads);
    return -1;
  }
}
