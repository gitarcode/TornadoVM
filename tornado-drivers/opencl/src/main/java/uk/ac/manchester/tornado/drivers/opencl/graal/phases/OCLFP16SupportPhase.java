...
/**
 * This compiler phase examines if the execution device supports half precision types and throws a
 * {@link TornadoDeviceFP16NotSupported} exception if it does not.
 */
  ...
  protected void run(StructuredGraph graph) {
    ...
    for (ReadNode readNode : graph.getNodes().filter(ReadNode.class)) {
      if (readNode.getLocationIdentity().toString().contains("VectorHalf") && !fp16Support) {
        throw new TornadoDeviceFP16NotSupported(
