...
public class TornadoTaskSpecialisation extends BasePhase<TornadoHighTierContext> {
  ...
  private static final String WARNING_GRID_SCHEDULER_DYNAMIC_LOOP_BOUNDS =
      "[TornadoVM] Warning: The loop bounds will be configured by the GridScheduler. Check the grid"
          + " by using the flag --threadInfo.";
  ...
  private ConstantNode lookupPrimField(
      StructuredGraph graph, Node node, Object obj, String field, JavaKind kind) {
    final Class<?> type = obj.getClass();
    final Field f = lookupField(type, field);
  ...
  private void evaluate(final StructuredGraph graph, final Node node, final Object value) {
    if (node instanceof ArrayLengthNode arrayLength) {
      ...
      /**
       * This condition covers the case that loop bounds should be taken based on the grid size
       * given by {@link GridScheduler}. This allows the loop bounds to be dynamically configured,
       * without requiring recompilation.
       *
       * <p>This condition will disable the FPGA HLS loop optimizations, because the loop bound is
       * not retrievable at compile time.
       */
      if (gridScheduling && isParameterInvolvedInParallelLoopBound(node)) {
        ...
        OCLKernelContextAccessNode kernelContextAccessNode =
            graph.addOrUnique(new OCLKernelContextAccessNode(constantValue));
        node.replaceAtUsages(kernelContextAccessNode);
        index++;
      } else {
        final ConstantNode constant =
            (batchThreads <= 0)
                ? ConstantNode.forInt(length)
                : ConstantNode.forInt((int) batchThreads);
      ...
      if (field.getType().getJavaKind().isPrimitive()) {
        ...
        throw new TornadoBailoutRuntimeException(
            "Non-final objects introduced via scope are not supported");
      }
  ...
  private ConstantNode createPrimitiveConstantFromObjectParameter(
      Object obj, StructuredGraph graph) {
    return switch (obj) {
  ...
  private void propagateParameters(
      StructuredGraph graph, ParameterNode parameterNode, Object[] args) {
    if (args[parameterNode.index()] != null
      ...
      if (gridScheduling && isParameterInvolvedInParallelLoopBound(parameterNode)) {
        ...
        // So every primitive wrapper bound to have UnboxNode that can also be optionally protected
        // by FixedGuardNode that checks that parameter is not null.
        // This pair is removed for constant values.
        // Otherwise there will be problems during Graph canonicalization.
        graph
            .getNodes()
            .filter(n -> n instanceof PiNode piNode && piNode.object() == primitiveConstant)
            .snapshot()
            .forEach(
                node -> {
                      ...
                      n -> {
                        if (n instanceof UnboxNode unboxNode) {
                          ...
                          if (prev instanceof FixedGuardNode fixedGuardNode) {
                            if (fixedGuardNode.condition() instanceof IsNullNode isNullNode
                                && isNullNode.getValue() == primitiveConstant) {
  ...
  protected void run(StructuredGraph graph, TornadoHighTierContext context) {
    ...
    while (hasWork) {
      ...
      graph
          ...
          .forEach(
              pi -> {
                if (pi.stamp(NodeView.DEFAULT) instanceof ObjectStamp
                    && pi.object().stamp(NodeView.DEFAULT) instanceof ObjectStamp) {
      ...
      getDebugContext()
          .dump(
              DebugContext.INFO_LEVEL, graph, "After TaskSpecialisation iteration = " + iterations);

      hasWork =
          (lastNodeCount != graph.getNodeCount()
                  || graph.getNewNodes(mark).isNotEmpty()
                  || hasPanamaArraySizeNode(graph))
    ...
    graph
        .getNodes()
        .filter(ParallelRangeNode.class)
        .forEach(
            range -> {
              if (range.value() instanceof PhiNode phiNode) {
                ...
                for (Node usage : usages) {
                  if (usage instanceof IntegerLessThanNode) {
                    ...
                    if (constant != null) {
                      getDebugContext()
                          .dump(DebugContext.INFO_LEVEL, graph, "Before Swapping Constant-Phi");
                      ParallelRangeNode pr =
                          new ParallelRangeNode(
                              range.index(), constant, range.offset(), range.stride());
                      ...
                      less.safeDelete();
                      getDebugContext()
                          .dump(DebugContext.INFO_LEVEL, graph, "After Swapping Constant-Phi");
                    }
                  }
                }
              }
