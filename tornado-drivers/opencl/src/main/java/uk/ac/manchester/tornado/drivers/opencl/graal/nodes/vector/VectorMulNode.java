...
public class VectorMulNode extends FloatingNode implements LIRLowerable, VectorOp {
  ...
  protected VectorMulNode(
      NodeClass<? extends VectorMulNode> c, OCLKind kind, ValueNode x, ValueNode y) {
    ...
    gen.getLIRGeneratorTool()
        .append(
            new AssignStmt(
                result,
                new OCLBinary.Expr(
                    OCLBinaryOp.MUL, gen.getLIRGeneratorTool().getLIRKind(stamp), input1, input2)));
