...
public enum OCLKind implements PlatformKind {
  ...
  public static OCLKind fromResolvedJavaType(ResolvedJavaType type) {
    if (!type.isArray()) {
      for (OCLKind k : OCLKind.values()) {
        if (k.javaClass != null
            && (k.javaClass.getSimpleName().equalsIgnoreCase(type.getJavaKind().name())
                || k.javaClass.getSimpleName().equals(type.getUnqualifiedName()))) {
          return k;
        }
      }
    }
  ...
  public static OCLAssembler.OCLBinaryTemplate resolvePrivatePointerTemplate(
      ResolvedJavaType type) {
    return resolvePrivatePointerTemplate(type.getJavaKind());
  }

  public static OCLAssembler.OCLBinaryTemplate resolvePrivatePointerCopyTemplate(
      ResolvedJavaType type) {
