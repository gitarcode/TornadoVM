...
public class OCLCompilationResultBuilder extends CompilationResultBuilder {
  ...
  public OCLCompilationResultBuilder(
  ...
  /**
   * Checks if the {@link OCLNodeLIRBuilder(LoopBeginNode)} has been called right before {@link
   * OCLNodeLIRBuilder#emitIf}. In other words, that there is no data flow/control flow between the
   * {@link LoopBeginNode} and the corresponding {@link IfNode} loop condition.
   *
   ...
   */
  private static boolean isLoopConditionRightAfterHeader(
      int loopCondIndex, int loopPostOpIndex, int loopInitOpIndex) {
    return (loopCondIndex - 1 == loopPostOpIndex) && (loopCondIndex - 2 == loopInitOpIndex);
  }

  /**
   * Checks if there are any LIR instructions between the loop condition and the {@link LoopInitOp}
   * and {@link LoopPostOp}. If there are no instructions, it is possible to move the loop condition
   * to the loop header.
   *
  ...
  private static boolean isLoopDependencyNode(LIRInstruction op) {
    return ((op instanceof OCLControlFlow.LoopInitOp
        || op instanceof OCLControlFlow.LoopConditionOp
        || op instanceof OCLControlFlow.LoopPostOp));
  }
  ...
  private static boolean isLoopBlock(HIRBlock block, HIRBlock loopHeader) {
    ...
    while (!stack.isEmpty()) {
      ...
      if (b.getId() < loopHeader.getId()) {
        ...
        HIRBlock[] successors =
            IntStream.range(0, b.getSuccessorCount())
                .mapToObj(b::getSuccessorAt)
  ...
  void emitBlock(HIRBlock block) {
    ...
    if (breakInst != null) {
      try {
        ...
        throw e.addContext(
            "lir instruction", block + "@" + breakInst.id() + " " + breakInst + "\n");
      }
    }
  }
  ...
  private void rescheduleBasicBlock(
      HIRBlock basicHIRBlock,
      OCLBlockVisitor visitor,
      HashSet<HIRBlock> visited,
  ...
  private boolean isFalseSuccessorWithLoopEnd(IfNode ifNode, HIRBlock basicHIRBlock) {
    return isCurrentHIRBlockAFalseBranch(ifNode, basicHIRBlock)
        && basicHIRBlock.getEndNode() instanceof LoopEndNode;
  }
  ...
  private boolean isTrueBranchWithEndNodeOrNotControlSplit(HIRBlock blockTrueBranch) {
    return ((blockTrueBranch.getEndNode() instanceof AbstractEndNode)
        || !(blockTrueBranch.getEndNode() instanceof ControlSplitNode));
  }

  /**
   * From Graal 22.1.0 the graph traversal was changed. This method reschedules the current basic
   * block to generate always the true condition before the false condition only if we have a
   * LoopEndNode node in the false branch, or we have a LoopExit in the true branch contains a
   * {@link LoopExitNode} or it is not a control Split (due to nested control-flow).
   *
   ...
   */
  private void rescheduleTrueBranchConditionsIfNeeded(
      HIRBlock basicBlock,
      OCLBlockVisitor visitor,
      HashSet<HIRBlock> visited,
      HashMap<HIRBlock, HIRBlock> pending) {
    if (!basicBlock.isLoopHeader()
        && basicBlock.getDominator() != null
        && basicBlock.getDominator().getEndNode() instanceof IfNode) {
  ...
  private void traverseControlFlowGraph(
      HIRBlock basicBlock,
      OCLBlockVisitor visitor,
      HashSet<HIRBlock> visited,
      HashMap<HIRBlock, HIRBlock> pending) {
    ...
    if (basicBlock.isLoopHeader()) {
      HIRBlock[] successors =
          IntStream.range(0, basicBlock.getSuccessorCount())
              .mapToObj(basicBlock::getSuccessorAt)
              .toArray(HIRBlock[]::new);
      ...
      for (HIRBlock block : successors) {
        ...
        if (!isInnerLoop) {
          assert ifNode != null;
          if (ifNode.trueSuccessor() == block.getBeginNode()
  ...
  private static class DepFinder implements InstructionValueProcedure {
    ...
    @Override
    public Value doValue(
        LIRInstruction instruction, Value value, OperandMode mode, EnumSet<OperandFlag> flags) {
