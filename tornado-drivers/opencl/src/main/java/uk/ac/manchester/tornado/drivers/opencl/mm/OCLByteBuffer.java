...
/**
 * A buffer for inspecting data within an OpenCL device. It is not backed by any userspace object.
 */
public class OCLByteBuffer {
  ...
  public OCLByteBuffer(
      OCLDeviceContext deviceContext,
      final long oclBufferId,
      final long offset,
      final long numBytes) {
  ...
  public int read(long executionPlanId, long fromBuffer, final int[] toArray) {
    return deviceContext.readBuffer(
        executionPlanId, fromBuffer, 0, toArray.length * 4, toArray, 0, null);
  }
  ...
  public int enqueueRead(long executionPlanId, final int[] events) {
    return deviceContext.enqueueReadBuffer(
        executionPlanId, toBuffer(), offset, bytes, buffer.array(), 0, events);
  }
   ...
   *
   * @param fromBuffer buffer to enqueue
   * @param array integer array to copy to the device.
  ...
  public int enqueueWrite(
      ...
      final int[] events) {
    return deviceContext.enqueueWriteBuffer(
        executionPlanId, fromBuffer, offset, BYTES_PER_INTEGER * array.length, array, 0, events);
  }
