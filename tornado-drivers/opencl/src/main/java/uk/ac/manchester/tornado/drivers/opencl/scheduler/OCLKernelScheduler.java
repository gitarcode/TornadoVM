...
public abstract class OCLKernelScheduler {
  ...
  public final String WARNING_FPGA_THREAD_LOCAL =
      "[TornadoVM OCL] Warning: TornadoVM changed the user-defined local size to: "
          + ((getDefaultLocalWorkGroup() != null)
              ? Arrays.toString(getDefaultLocalWorkGroup())
              : "null")
          + ".";

  public static final String WARNING_THREAD_LOCAL =
      "[TornadoVM OCL] Warning: TornadoVM changed the user-defined local size to null. Now, the"
          + " OpenCL driver will select the best configuration.";
  ...
  public int submit(
      long executionPlanId, final OCLKernel kernel, final TaskDataContext meta, long batchThreads) {
  ...
  public int launch(
    ...
    if (meta.isWorkerGridAvailable()) {
      ...
      return deviceContext.enqueueNDRangeKernel(
          executionPlanId, kernel, grid.dimension(), offset, global, local, waitEvents);
    } else {
      return deviceContext.enqueueNDRangeKernel(
          executionPlanId,
          kernel,
          ...
          meta.getGlobalWork(),
          (meta.shouldUseOpenCLDriverScheduling() ? null : meta.getLocalWork()),
          waitEvents);
    }
  }

  /**
   * Checks if the selected local work group fits on the target device. If it does not fit, it sets
   * the local work group to null, so the OpenCL driver chooses a default value. In this case, the
   * threads configured in the local work sizes depends on each OpenCL driver.
   *
