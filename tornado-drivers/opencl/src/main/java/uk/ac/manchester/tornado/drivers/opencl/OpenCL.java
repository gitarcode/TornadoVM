...
public class OpenCL {
    ...
    if (VIRTUAL_DEVICE_ENABLED) {
      ...
      try {
        ...
        throw new TornadoRuntimeException(
            "[ERROR] Initialization of the OpenCL platform is not correct");
      }
   ...
   *
   * @param tornadoDevice OpenCL device to run the application.
   * @param openCLCode OpenCL code to run.
   * @param taskMeta TaskMetadata.
   * @param accesses Access of each parameter
  ...
  public static void run(
    ...
    KernelStackFrame callWrapper =
        tornadoDevice.createKernelStackFrame(executionContextId, numArgs);
    callWrapper.reset();
    ...
    for (int platformIndex = 0; platformIndex < platforms.size(); platformIndex++) {
      ...
      for (int deviceIndex = 0; deviceIndex < context.getNumDevices(); deviceIndex++) {
        OCLDeviceContext deviceContext =
            (OCLDeviceContext) context.createDeviceContext(deviceIndex);
        System.out.printf(
  ...
  public static TornadoTargetDevice getDevice(int platformIndex, int deviceIndex) {
    final TornadoPlatformInterface platform = platforms.get(platformIndex);
    OCLDeviceContext deviceContext =
        (OCLDeviceContext) platform.createContext().createDeviceContext(deviceIndex);
