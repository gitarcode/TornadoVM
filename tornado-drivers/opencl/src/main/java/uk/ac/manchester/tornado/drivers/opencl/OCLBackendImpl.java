...
public final class OCLBackendImpl implements TornadoAcceleratorBackend {
  ...
  private static final List<OCLDeviceType> DEVICE_TYPE_LIST =
      Arrays.asList( //
  ...
  public OCLBackendImpl(
      final OptionValues options,
      final HotSpotJVMCIRuntime vmRuntime,
      TornadoVMConfigAccess vmConfig) {
    final int numPlatforms = OpenCL.getNumPlatforms();

    if (numPlatforms < 1) {
      throw new TornadoBailoutRuntimeException(
          "[WARNING] No OpenCL platforms found. Deoptimizing to sequential execution.");
    }
  ...
  private OCLBackend[] orderFlattenBackends() {
    ...
    for (OCLBackend backend : flatBackends) {
      ...
      List<OCLBackend> backendListForDeviceType =
          deviceTypeMap.computeIfAbsent(deviceType, k -> new ArrayList<>());
    ...
    Map<OCLDeviceType, List<OCLBackend>> groupedByDeviceType =
        backendList.stream()
            .collect(
                Collectors.groupingBy(
                    backend -> backend.getDeviceContext().getDevice().getDeviceType()));

    // Sort each sublist by size in descending order
    groupedByDeviceType.forEach(
        (deviceType, sublist) ->
            Collections.sort(
                sublist,
                (backend1, backend2) -> {
  ...
  public TornadoXPUDevice getDevice(int index) {
    if (index < flatBackends.length) {
      ...
      throw new TornadoDeviceNotFound(
          "[ERROR] device required not found: " + index + " - Max: " + flatBackends.length);
    }
  }
  ...
  private OCLBackend createOCLJITCompiler(
      final OptionValues options,
      final HotSpotJVMCIRuntime jvmciRuntime,
      TornadoVMConfigAccess vmConfig,
      final OCLContextInterface context,
      final int deviceIndex) {
    final OCLTargetDevice device = context.devices().get(deviceIndex);
    logger.info("Creating backend for %s", device.getDeviceName());
    return OCLHotSpotBackendFactory.createJITCompiler(
        options, jvmciRuntime, vmConfig, context, device);
  }

  private void installDevices(
      int platformIndex,
      TornadoPlatformInterface platform,
      final OptionValues options,
      final HotSpotJVMCIRuntime vmRuntime,
      TornadoVMConfigAccess vmConfig) {
    ...
    for (int deviceIndex = 0; deviceIndex < numDevices; deviceIndex++) {
      ...
      backends[platformIndex][deviceIndex] =
          createOCLJITCompiler(options, vmRuntime, vmConfig, context, deviceIndex);
    }
  }
  ...
  private void discoverDevices(
      ...
      final HotSpotJVMCIRuntime vmRuntime,
      TornadoVMConfigAccess vmConfig) {
    IntStream.range(0, OpenCL.getNumPlatforms())
        .forEach(
            i -> {
