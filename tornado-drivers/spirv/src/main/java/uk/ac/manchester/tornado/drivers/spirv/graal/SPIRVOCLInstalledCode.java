...
public class SPIRVOCLInstalledCode extends SPIRVInstalledCode {
  ...
  private void setKernelArgs(long executionPlanId, final SPIRVKernelStackFrame callWrapper) {
    ...
    int status =
        dispatcher.clSetKernelArg(
            kernelPointer, 0, Sizeof.LONG.getNumBytes(), callWrapper.toBuffer());
    ...
    for (int argIndex = 0; argIndex < callWrapper.getCallArguments().size(); argIndex++) {
      ...
      if (arg.getValue() instanceof KernelStackFrame.KernelContextArgument) {
        status =
            dispatcher.clSetKernelArg(
                kernelPointer, kernelParamIndex, Sizeof.LONG.getNumBytes(), callWrapper.toBuffer());
        checkStatus(status, "clSetKernelArg");
        continue;
      }
      ...
      if (RuntimeUtilities.isBoxedPrimitive(arg.getValue())
        ...
        if (!arg.isReferenceType()) {
          // In OpenCL, we need to set the argument. But it is set as buffer pointer. So we add the
          // kernelContext as a dummy one.
          status =
              dispatcher.clSetKernelArg(
                  kernelPointer,
                  kernelParamIndex,
  ...
  public void submit(
      long executionPlanId, long kernelPointer, final TaskDataContext meta, long[] waitEvents) {
    ...
    if (status != OCLErrorCode.CL_SUCCESS) {
      switch (status) {
        case OCLErrorCode.CL_INVALID_KERNEL_ARGS ->
            System.err.println("[OCL Error] Invalid Kernel Args");
    ...
    int value =
        eventPool.registerEvent(kernelEvent[0], EventDescriptor.DESC_PARALLEL_KERNEL, commandQueue);
  ...
  public int launch(
    ...
    if (meta.isWorkerGridAvailable()) {
      ...
      return dispatcher.clEnqueueNDRangeKernel(
          queuePointer,
          kernelPointer, //
          workerGrid.dimension(),
          workerGrid.getGlobalOffset(),
          workerGrid.getGlobalWork(),
          workerGrid.getLocalWork(), //
          waitEvents,
          kernelEvent); //
    } else {
  ...
  private void updateProfiler(
    ...
    if (TornadoOptions.isProfilerEnabled()) {
      ...
      meta.getProfiler()
          .setTaskPowerUsage(
              ProfilerType.POWER_USAGE_mW, meta.getId(), deviceContext.getPowerUsage());
    }
  }
