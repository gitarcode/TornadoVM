...
public class SPIRVUnary {
  ...
  protected static class UnaryConsumer extends SPIRVLIROp {
    ...
    @Opcode protected final SPIRVUnaryOp opcode;

    @Use protected Value value;

    @Def protected Variable result;
  ...
  public static class LoadFromStackFrameExpr extends Expr {
    ...
    public LoadFromStackFrameExpr(
        LIRKind lirKind, SPIRVKind type, int indexFromStackFrame, int parameterIndex) {
     ...
     *
     * <p>The equivalent in OpenCL is as follows: <code>
     * ulong_0 = (ulong) _frame[STACK_INDEX];
     * </code>
     *
     * <p>
     *
     * <p>This an example of the target code to generate in SPIR-V: <code>
    ...
    public void emit(SPIRVCompilationResultBuilder crb, SPIRVAssembler asm) {
      ...
      asm.currentBlockScope()
          .add(
              new SPIRVOpInBoundsPtrAccessChain( //
                  ptrCrossWorkGroupULong, //
                  accessPTR, //
                  loadID, //
                  index, //
  ...
  public static class AssignLoadFromInputFrame extends Expr {
     ...
     * <p>ul_0 = (ulong) a;
     *
     * <p>Equivalent generated code: <code>
     * %46 = OpLoad %_ptr_CrossWorkgroup_uchar %a_addr Aligned 8
     * %47 = OpConvertPtrToU %ulong %46
     * OpStore %ul_0 %47 Aligned 8
     * </code>
     */
    public AssignLoadFromInputFrame(
        LIRKind lirKind, SPIRVKind type, int indexFromKernelContext, int parameterIndex) {
      super(null, null, lirKind, null);
      this.type = type;
    ...
    public void emit(SPIRVCompilationResultBuilder crb, SPIRVAssembler asm) {
      Logger.traceCodeGen(
          Logger.BACKEND.SPIRV, "µIns AssignLoadFromInputFrame with Index: " + parameterIndex);
  ...
  public abstract static class AbstractMemoryAccess extends UnaryConsumer {
    ...
    protected AbstractMemoryAccess(
        SPIRVUnaryOp opcode, Variable result, LIRKind valueKind, Value value) {
  ...
  public static class MemoryIndexedAccess extends AbstractMemoryAccess {
    ...
    private void emitPrivateMemoryIndexedAccess(
      ...
      if (index instanceof ConstantValue) {
        indexId =
            asm.lookUpConstant(
                ((ConstantValue) index).getConstant().toValueString(),
      ...
      SPIRVId type =
          asm.primitives.getPtrOpTypePointerWithStorage(spirvKind, SPIRVStorageClass.Function());
      asm.currentBlockScope()
          .add(
              new SPIRVOpInBoundsPtrAccessChain(
                  type, arrayAccessId, baseId, baseIndex, new SPIRVMultipleOperands(indexId)));
      asm.registerLIRInstructionValue(this, arrayAccessId);
    }

    private void emitLocalMemoryIndexedAccess(
        SPIRVCompilationResultBuilder crb, SPIRVAssembler asm) {
      ...
      SPIRVKind spirvKind = (SPIRVKind) getValue().getPlatformKind();
      SPIRVId type =
          asm.primitives.getPtrOpTypePointerWithStorage(spirvKind, SPIRVStorageClass.Workgroup());
      asm.currentBlockScope()
          .add(
              new SPIRVOpInBoundsPtrAccessChain(
                  type, arrayAccessId, baseId, baseIndex, new SPIRVMultipleOperands(indexId)));
      ...
      if (getMemoryRegion().getMemorySpace() == SPIRVMemorySpace.LOCAL) {
        type =
            asm.primitives.getPtrOpTypePointerWithStorage(
                (SPIRVKind) getValue().getPlatformKind(), SPIRVStorageClass.Workgroup());
      } else if (getMemoryRegion().getMemorySpace() == SPIRVMemorySpace.PRIVATE) {
        type =
            asm.primitives.getPtrToTypeFunctionPrimitive((SPIRVKind) getValue().getPlatformKind());
      } else {
        throw new RuntimeException(
            "Memory access not valid for a SPIRVOpInBoundsPtrAccessChain instruction");
      }
      ...
      asm.currentBlockScope()
          .add(
              new SPIRVOpInBoundsPtrAccessChain(
                  ...
                  baseId,
                  baseIndex,
                  new SPIRVMultipleOperands(indexId)));
  ...
  public static class SPIRVAddressCast extends UnaryConsumer {
    ...
    /**
     * Generates the following SPIR-V code: <code>
    ...
    public void emit(SPIRVCompilationResultBuilder crb, SPIRVAssembler asm) {
      Logger.traceCodeGen(
          ...
          "emit SPIRVAddressCast with LIRKIND: " + getLIRKind().getPlatformKind());
      ...
      SPIRVId ptrCrossGroup =
          asm.primitives.getPtrToCrossWorkGroupPrimitive(
              (SPIRVKind) getLIRKind().getPlatformKind());
  ...
  public static class ThreadBuiltinCallForSPIRV extends UnaryConsumer {
    ...
    public ThreadBuiltinCallForSPIRV(
        SPIRVThreadBuiltIn builtIn, Variable result, LIRKind valueKind, Value dimension) {
     ...
     *
     * <p>Example for get_global_id: <code>
    ...
    public void emit(SPIRVCompilationResultBuilder crb, SPIRVAssembler asm) {
      ...
      if (dimension instanceof ConstantValue) {
        dimensionValue =
            Integer.parseInt(((ConstantValue) dimension).getConstant().toValueString());
      } else {
  ...
  public abstract static class AbstractExtend extends UnaryConsumer {
    ...
    protected SPIRVId loadConvertIfNeeded(
        SPIRVCompilationResultBuilder crb, SPIRVAssembler asm, SPIRVId type, SPIRVKind spirvKind) {
  ...
  public static class SignExtend extends AbstractExtend {
    ...
    public void emit(SPIRVCompilationResultBuilder crb, SPIRVAssembler asm) {
      ...
      Logger.traceCodeGen(
          Logger.BACKEND.SPIRV, "emit SPIRVOpSConvert : " + fromBits + " -> " + toBits);
  ...
  public static class SignNarrowValue extends AbstractExtend {
    ...
    /**
     * Following this: {@url
     * https://www.khronos.org/registry/spir-v/specs/unified1/SPIRV.html#OpSConvert} <code>
     * Convert signed width. This is either a truncate or a sign extend.
     * </code>
     *
     * <p>OpSConvert can be used for sign extend as well as truncate. The "S" symbol represents
     * signed format.
     *
     * @param crb {@link SPIRVCompilationResultBuilder}
     * @param asm {@link SPIRVAssembler}
  ...
  public static class CastFloatDouble extends CastOperations {
    ...
    public void emit(SPIRVCompilationResultBuilder crb, SPIRVAssembler asm) {
      ...
      Logger.traceCodeGen(
          Logger.BACKEND.SPIRV,
          "emit SPIRVOpFConvert from " + value.getPlatformKind() + " -> " + toType);
  ...
  /**
   * OpenCL Extended Instruction Set Intrinsics. As specified in the SPIR-V 1.0 standard, the
   * following intrinsics in SPIR-V represents builtin functions from the OpenCL standard.
   *
   * <p>For obtaining the correct Int-Reference of the function:
   *
   * <p><url>https://www.khronos.org/registry/spir-v/specs/1.0/OpenCL.ExtendedInstructionSet.100.html</url>
   */
  public static class Intrinsic extends UnaryConsumer {
    ...
    public void emit(SPIRVCompilationResultBuilder crb, SPIRVAssembler asm) {
      ...
      SPIRVLiteralExtInstInteger intrinsic =
          new SPIRVLiteralExtInstInteger(builtIn.value, builtIn.name);
      asm.currentBlockScope()
          .add(
              new SPIRVOpExtInst(
                  type, result, set, intrinsic, new SPIRVMultipleOperands<>(loadParam)));
      asm.registerLIRInstructionValue(this, result);
    }
  ...
  public static class Negate extends UnaryConsumer {
    ...
    public void emit(SPIRVCompilationResultBuilder crb, SPIRVAssembler asm) {
      Logger.traceCodeGen(
          Logger.BACKEND.SPIRV,
          "emit " + nameDebugInstruction + getValue() + " with type: " + getSPIRVPlatformKind());
  ...
  public static class Barrier extends UnaryConsumer {
    ...
    public void emit(SPIRVCompilationResultBuilder crb, SPIRVAssembler asm) {
      Logger.traceCodeGen(
          Logger.BACKEND.SPIRV,
          "emit Barrier with FLAGS"
              + flags.toString()
              + " SEMANTICS: "
              + flags.getMemorySemantics());
      SPIRVId constant2 = asm.lookUpConstant("2", SPIRVKind.OP_TYPE_INT_32);
      SPIRVId constantSemantics =
          asm.lookUpConstant(
              Integer.toString(flags.getMemorySemantics()), SPIRVKind.OP_TYPE_INT_32);
      asm.currentBlockScope()
          .add(new SPIRVOpControlBarrier(constant2, constant2, constantSemantics));
    }
  }
  ...
  public static class ReturnWithValue extends SPIRVLIROp {
    ...
    public void emit(SPIRVCompilationResultBuilder crb, SPIRVAssembler asm) {
      ...
      Logger.traceCodeGen(
          Logger.BACKEND.SPIRV,
          "emit SPIRVOpReturnValue : " + currentBlock.toString() + " with value: " + input);
