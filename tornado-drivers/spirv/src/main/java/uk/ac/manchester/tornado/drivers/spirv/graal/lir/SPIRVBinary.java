...
public class SPIRVBinary {
  ...
  protected static class BinaryConsumer extends SPIRVLIROp {
    ...
    @Opcode protected SPIRVBinaryOp binaryOperation;

    @Use protected Value x;

    @Use protected Value y;

    @Use protected Variable result;

    protected BinaryConsumer(
        SPIRVBinaryOp instruction, Variable result, LIRKind valueKind, Value x, Value y) {
      super(valueKind);
    ...
    private SPIRVId genTypeConversionIfNeeded(
        SPIRVAssembler asm,
        SPIRVKind spirvKind,
        SPIRVKind convertionKind,
        SPIRVId instructionToConvert) {
      if (convertionKind != null && convertionKind != spirvKind) {
        ...
        asm.currentBlockScope()
            .add(new SPIRVOpSConvert(idConversionType, resultConversion, instructionToConvert));
        return resultConversion;
      }
      return instructionToConvert;
    }

    private SPIRVId getParameterWithTypeConversionIfNeeded(
        SPIRVAssembler asm,
        SPIRVKind spirvKind,
        SPIRVKind conversionKind,
        SPIRVId instructionToConvert) {
      if (conversionKind != null && conversionKind != spirvKind) {
        SPIRVId resultConversion = asm.module.getNextId();
        SPIRVId idConversionType = asm.primitives.getTypePrimitive(conversionKind);
        asm.currentBlockScope()
            .add(new SPIRVOpUConvert(idConversionType, resultConversion, instructionToConvert));
        return resultConversion;
      }
      return instructionToConvert;
    }

    protected SPIRVId getId(
        Value inputValue, SPIRVAssembler asm, SPIRVKind spirvKind, SPIRVKind convertionKind) {
      if (inputValue instanceof ConstantValue) {
        ...
        if (param == null) {
          throw new RuntimeException(
              "LOADING PARAMETER: " + inputValue + " with NULL VALUE in SPIR-V Table");
        }
        ...
        Logger.traceCodeGen(
            Logger.BACKEND.SPIRV, "emit LOAD Variable: " + inputValue + " ::: " + param);
    ...
    public void emit(SPIRVCompilationResultBuilder crb, SPIRVAssembler asm) {
      ...
      Logger.traceCodeGen(
          ...
          "emitBinaryOperation "
              + binaryOperation.getInstruction()
              + ":  "
              + x
              + " "
              + binaryOperation.getOpcode()
              + " "
              + y
              + "  Result Kind: "
              + resultKind);
      ...
      SPIRVInstruction instructionOperation =
          binaryOperation.generateInstruction(typeResultOperationId, operationId, a, b);
  ...
  public static class PrivateArrayAllocation extends BinaryConsumer {
    ...
    @LIRInstruction.Def private AllocatableValue resultArray;
  ...
  public static class LocalArrayAllocation extends BinaryConsumer {
    ...
    private SPIRVId addSPIRVIdLocalArrayInPreamble(SPIRVAssembler asm) {
      ...
      asm.module.add(
          new SPIRVOpDecorate(
              id, SPIRVDecoration.Alignment(new SPIRVLiteralInteger(kind.getSizeInBytes()))));
      return id;
    }
    ...
    public void emit(SPIRVCompilationResultBuilder crb, SPIRVAssembler asm) {
      Logger.traceCodeGen(
          Logger.BACKEND.SPIRV, "emit ArrayDeclaration: " + resultArray + "[" + length + "]");

      SPIRVId idResult = addSPIRVIdLocalArrayInPreamble(asm);

      SPIRVId primitiveType =
          asm.primitives.getTypePrimitive((SPIRVKind) lirKind.getPlatformKind());
      ...
      SPIRVId resultArrayId =
          asm.declareArray((SPIRVKind) lirKind.getPlatformKind(), primitiveType, lengthId);
      SPIRVId functionPTR = asm.getFunctionPtrToLocalArray(resultArrayId);

      // Registration of the variable in the module level
      asm.module.add(
          new SPIRVOpVariable(
              functionPTR, idResult, SPIRVStorageClass.Workgroup(), new SPIRVOptionalOperand<>()));
  ...
  public static class Intrinsic extends BinaryConsumer {
    ...
    public Intrinsic(
        SPIRVUnary.Intrinsic.OpenCLExtendedIntrinsic builtIn, LIRKind valueKind, Value x, Value y) {
    ...
    public void emit(SPIRVCompilationResultBuilder crb, SPIRVAssembler asm) {
      ...
      Logger.traceCodeGen(
          Logger.BACKEND.SPIRV,
          "emit SPIRVLiteralExtInstInteger: " + builtIn.getName() + " (" + x + "," + y + ")");
      ...
      asm.currentBlockScope()
          .add(
              new SPIRVOpExtInst(
                  typeOperation, result, set, intrinsic, new SPIRVMultipleOperands<>(a, b)));
  ...
  public static class VectorOperation extends BinaryConsumer {
    ...
    public void emit(SPIRVCompilationResultBuilder crb, SPIRVAssembler asm) {
      ...
      LIRKind lirKind = getLIRKind();
      SPIRVKind spirvKind = (SPIRVKind) lirKind.getPlatformKind();
      SPIRVId typeOperation =
          asm.primitives.getTypePrimitive(
              spirvKind.getElementKind()); // / Vector Selection -> Element Kind
      ...
      SPIRVInstruction instruction =
          binaryOperation.generateInstruction(
              typeOperation, binaryVectorOperationResult, resultSelect1, resultSelect2);
  ...
  public static class TernaryCondition extends BinaryConsumer {
    ...
    @Use private Value leftVal;
    private Condition cond;

    @Use private Value right;

    @Use private Value trueValue;

    @Use private Value falseValue;
    ...
    public void emit(SPIRVCompilationResultBuilder crb, SPIRVAssembler asm) {
      ...
      switch (cond) {
        case EQ:
          asm.currentBlockScope()
              .add(new SPIRVOpIEqual(typeBoolean, comparisonResult, idLeftVar, idRightVar));
          break;
        case NE:
          asm.currentBlockScope()
              .add(new SPIRVOpINotEqual(typeBoolean, comparisonResult, idLeftVar, idRightVar));
          break;
        case LT:
          asm.currentBlockScope()
              .add(new SPIRVOpSLessThan(typeBoolean, comparisonResult, idLeftVar, idRightVar));
          break;
        case GT:
          asm.currentBlockScope()
              .add(new SPIRVOpSGreaterThan(typeBoolean, comparisonResult, idLeftVar, idRightVar));
          break;
        default:
      ...
      asm.currentBlockScope()
          .add(
              new SPIRVOpSelect(
                  resultType, resultSelectId, comparisonResult, trueValueId, falseValueId));
  ...
  public static class IntegerTestNode extends BinaryConsumer {
    ...
    public void emit(SPIRVCompilationResultBuilder crb, SPIRVAssembler asm) {
      Logger.traceCodeGen(
          Logger.BACKEND.SPIRV, "emit IntegerTestNode: (" + x + " & " + y + ") ==  0");
      ...
      SPIRVInstruction instruction =
          binaryOperation.generateInstruction(typeOperation, bitWiseAnd, a, b);
      asm.currentBlockScope().add(instruction);
      ...
      asm.currentBlockScope()
          .add(new SPIRVOpIEqual(booleanType, compEqual, bitWiseAnd, zeroConstant));
  ...
  public static class IntegerTestMoveNode extends BinaryConsumer {
    ...
    public void emit(SPIRVCompilationResultBuilder crb, SPIRVAssembler asm) {
      Logger.traceCodeGen(
          Logger.BACKEND.SPIRV,
          "emit IntegerTestMoveNode: (condition from IntegerTestNode)" + " ? " + x + " : " + y);
      ...
      asm.currentBlockScope()
          .add(
              new SPIRVOpSelect(
                  asm.primitives.getTypePrimitive(SPIRVKind.OP_TYPE_INT_32),
                  resultID,
                  comparisonID,
                  trueValueId,
                  falseValueId));
