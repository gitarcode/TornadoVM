...
/**
 * This phase modifies the functionality of the originally FloatingRead Phase from Graal. The
 * original phase reschedules and replaces Read-Nodes with {@link FloatingReadNode} nodes in order
 * to move loads closer to the actual usage. An extra check is performed through
 * shouldBeFloatingRead to check the node origin of the access to prevent read associated with
 * private memory to be replaced by floating reads and schedule outside the scope of the parallel
 * loop.
 *
 * <p>{@link org.graalvm.compiler.phases.common.FloatingReadPhase}
 */
public class TornadoFloatingReadReplacement extends PostRunCanonicalizationPhase<CoreProviders> {
  ...
  public TornadoFloatingReadReplacement(
      boolean createMemoryMapNodes, CanonicalizerPhase canonicalizer) {
    super(canonicalizer);
  ...
  private static void processIdentity(
      EconomicSet<LocationIdentity> currentState, LocationIdentity identity) {
  ...
  public static TornadoFloatingReadReplacement.MemoryMapImpl mergeMemoryMaps(
      AbstractMergeNode merge, List<? extends MemoryMap> states) {
    TornadoFloatingReadReplacement.MemoryMapImpl newState =
        new TornadoFloatingReadReplacement.MemoryMapImpl();
  ...
  private EconomicSet<LocationIdentity> processLoop(
      HIRLoop loop, EconomicMap<LoopBeginNode, EconomicSet<LocationIdentity>> modifiedInLoops) {
  ...
  protected void run(StructuredGraph graph, CoreProviders context) {
    ...
    EconomicSetNodeEventListener listener =
        new EconomicSetNodeEventListener(EnumSet.of(NODE_ADDED, ZERO_USAGES));
    try (Graph.NodeEventScope nes = graph.trackNodeEvents(listener)) {
      ReentrantNodeIterator.apply(
          new FloatingReadPhase.FloatingReadClosure(
              modifiedInLoops, true, createMemoryMapNodes, initMemory),
          graph.start(),
  ...
  public static class FloatingReadClosure
      extends ReentrantNodeIterator.NodeIteratorClosure<
          TornadoFloatingReadReplacement.MemoryMapImpl> {
    ...
    public FloatingReadClosure(
        EconomicMap<LoopBeginNode, EconomicSet<LocationIdentity>> modifiedInLoops,
    ...
    /**
     * Improve the memory graph by re-wiring all usages of a {@link MemoryAnchorNode} to the real
     * last access location.
     */
    private static void processAnchor(
        MemoryAnchorNode anchor, TornadoFloatingReadReplacement.MemoryMapImpl state) {
      for (Node node : anchor.usages().snapshot()) {
        if (node instanceof MemoryAccess) {
          MemoryAccess access = (MemoryAccess) node;
          if (access.getLastLocationAccess() == anchor) {
            MemoryKill lastLocationAccess =
                state.getLastLocationAccess(access.getLocationIdentity());
            assert lastLocationAccess != null;
    ...
    private static void processAccess(
        MemoryAccess access, TornadoFloatingReadReplacement.MemoryMapImpl state) {
    ...
    /**
     * @param accessNode is a {@link FixedNode} that will be replaced by a {@link FloatingNode}.
     *     This method checks if the node that is going to be replaced has an {@link
     *     SPIRVBarrierNode} as next.
     */
    ...
    /**
     * @param nextNode is a {@link FixedNode} that will be replaced by a {@link FloatingNode}. This
     *     method removes the redundant {@link SPIRVBarrierNode}.
     */
    ...
    private static boolean shouldBeFloatingRead(FloatableAccessNode accessNode) {
      ...
      boolean hasPrivateArrays =
          accessNode.graph().getNodes().filter(FixedArrayNode.class).isNotEmpty();
    ...
    @SuppressWarnings("try")
    private static void createMemoryPhi(
        LoopBeginNode loop,
        TornadoFloatingReadReplacement.MemoryMapImpl initialState,
        EconomicMap<LocationIdentity, MemoryPhiNode> phis,
    ...
    @Override
    protected TornadoFloatingReadReplacement.MemoryMapImpl processNode(
        FixedNode node, TornadoFloatingReadReplacement.MemoryMapImpl state) {

      if (node instanceof LoopExitNode) {
        final LoopExitNode loopExitNode = (LoopExitNode) node;
        final EconomicSet<LocationIdentity> modifiedInLoop =
            modifiedInLoops.get(loopExitNode.loopBegin());
        final boolean anyModified = modifiedInLoop.contains(LocationIdentity.any());
        state
            .getMap()
            .replaceAll(
                (locationIdentity, memoryNode) ->
                    (anyModified || modifiedInLoop.contains(locationIdentity))
      ...
      if (MemoryKill.isSingleMemoryKill(node)) {
        assert !MemoryKill.isMultiMemoryKill(node)
            : "Node cannot be single and multi kill concurrently " + node;
      ...
      if (createMemoryMapNodes && node instanceof MemoryMapControlSinkNode) {
        try (DebugCloseable position = node.withNodeSourcePosition()) {
          ((MemoryMapControlSinkNode) node)
              .setMemoryMap(node.graph().unique(new MemoryMapNode(state.getMap())));
        }
      }
      return state;
    }
    ...
    private void processCheckpoint(
        SingleMemoryKill checkpoint, TornadoFloatingReadReplacement.MemoryMapImpl state) {
      processIdentity(checkpoint.getKilledLocationIdentity(), checkpoint, state);
    }

    private void processCheckpoint(
        MultiMemoryKill checkpoint, TornadoFloatingReadReplacement.MemoryMapImpl state) {
    ...
    private void processIdentity(
        ...
        MemoryKill checkpoint,
        TornadoFloatingReadReplacement.MemoryMapImpl state) {
      if (identity.isAny()) {
        state.getMap().clear();
      }
      if (identity.isMutable()) {
    ...
    @SuppressWarnings("try")
    private void processFloatable(
        FloatableAccessNode accessNode, TornadoFloatingReadReplacement.MemoryMapImpl state) {
      ...
      if (accessNode.canFloat() && shouldBeFloatingRead(accessNode)) {
        ...
        GraalError.guarantee(
            !initMemory.contains(accessNode.getAddress().getBase()),
            "base used for init cannot be used for other accesses: %s",
            accessNode);
    ...
    @Override
    protected TornadoFloatingReadReplacement.MemoryMapImpl merge(
        AbstractMergeNode merge, List<TornadoFloatingReadReplacement.MemoryMapImpl> states) {
      return mergeMemoryMaps(merge, states);
    }

    @Override
    protected TornadoFloatingReadReplacement.MemoryMapImpl afterSplit(
        AbstractBeginNode node, TornadoFloatingReadReplacement.MemoryMapImpl oldState) {
      return new TornadoFloatingReadReplacement.MemoryMapImpl(oldState);
    }

    @Override
    protected EconomicMap<LoopExitNode, TornadoFloatingReadReplacement.MemoryMapImpl> processLoop(
        LoopBeginNode loop, TornadoFloatingReadReplacement.MemoryMapImpl initialState) {
      ...
      ReentrantNodeIterator.LoopInfo<TornadoFloatingReadReplacement.MemoryMapImpl> loopInfo =
          ReentrantNodeIterator.processLoop(this, loop, initialState);

      UnmodifiableMapCursor<LoopEndNode, TornadoFloatingReadReplacement.MemoryMapImpl>
          endStateCursor = loopInfo.endStates.getEntries();
      while (endStateCursor.advance()) {
        ...
        while (phiCursor.advance()) {
          ...
          phi.initializeValueAt(
              endIndex,
              ValueNodeInterface.asNode(endStateCursor.getValue().getLastLocationAccess(key)));
        }
      }
