...
public class SPIRVLevelZeroInstalledCode extends SPIRVInstalledCode {
  ...
  public static final String WARNING_THREAD_LOCAL =
      "[TornadoVM SPIR-V] Warning: TornadoVM changed the user-defined local thread sizes to the"
          + " suggested values by the driver.";
  ...
  public SPIRVLevelZeroInstalledCode(
      String name, SPIRVModule spirvModule, SPIRVDeviceContext deviceContext) {
  ...
  private void setKernelArgs(long executionPlanId, final SPIRVKernelStackFrame callWrapper) {
    ...
    int result =
        levelZeroKernel.zeKernelSetArgumentValue(
            kernel.getPtrZeKernelHandle(), 0, Sizeof.LONG.getNumBytes(), callWrapper.toBuffer());
    ...
    for (int argIndex = 0; argIndex < callWrapper.getCallArguments().size(); argIndex++) {
      ...
      if (arg.getValue() instanceof KernelStackFrame.KernelContextArgument) {
        result =
            levelZeroKernel.zeKernelSetArgumentValue(
                kernel.getPtrZeKernelHandle(),
                kernelParamIndex,
                Sizeof.LONG.getNumBytes(),
                callWrapper.toBuffer());
        LevelZeroUtils.errorLog("zeKernelSetArgumentValue", result);
        continue;
      }

      if (RuntimeUtilities.isBoxedPrimitive(arg.getValue())
          || arg.getValue().getClass().isPrimitive()) {
  ...
  private DeviceThreadScheduling calculateGlobalAndLocalBlockOfThreads(
      TaskDataContext meta, long batchThreads) {
  ...
  private int setThreadSuggestionFromLevelZero(
      LevelZeroKernel levelZeroKernel,
      ZeKernelHandle kernel,
      int[] groupSizeX,
      int[] groupSizeY,
      int[] groupSizeZ) {
    int result =
        levelZeroKernel.zeKernelSuggestGroupSize(
            kernel.getPtrZeKernelHandle(),
            (int) deviceThreadScheduling.globalWork[0],
            (int) deviceThreadScheduling.globalWork[1],
            (int) deviceThreadScheduling.globalWork[2],
            groupSizeX,
            groupSizeY,
            groupSizeZ);
    LevelZeroUtils.errorLog("zeKernelSuggestGroupSize", result);
    result =
        levelZeroKernel.zeKernelSetGroupSize(
            kernel.getPtrZeKernelHandle(), groupSizeX, groupSizeY, groupSizeZ);
  ...
  private ThreadBlockDispatcher suggestThreadSchedulingToLevelZeroDriver(
    ...
    if (!meta.isWorkerGridAvailable()) {
      int result =
          levelZeroKernel.zeKernelSuggestGroupSize(
              kernel.getPtrZeKernelHandle(),
              (int) threadScheduling.globalWork[0],
              (int) threadScheduling.globalWork[1],
              (int) threadScheduling.globalWork[2],
              groupSizeX,
              groupSizeY,
              groupSizeZ);
      LevelZeroUtils.errorLog("zeKernelSuggestGroupSize", result);
  ...
  private void launchKernelWithLevelZero(
      long executionPlanId,
      ZeKernelHandle kernel,
      DeviceThreadScheduling threadScheduling,
      ThreadBlockDispatcher dispatcher) {
    ...
    SPIRVLevelZeroCommandQueue commandQueue =
        (SPIRVLevelZeroCommandQueue)
            deviceContext
                .getSpirvContext()
                .getCommandQueueForDevice(executionPlanId, deviceContext.getDeviceIndex());
    ...
    ZeEventHandle kernelEventTimer =
        kernelTimeStamp != null ? kernelTimeStamp.getKernelEventTimer() : null;

    // Launch the kernel on the Intel Integrated GPU
    int result =
        commandList.zeCommandListAppendLaunchKernel(
    ...
    result =
        commandList.zeCommandListAppendBarrier(
            commandList.getCommandListHandlerPtr(), null, 0, null);
    LevelZeroUtils.errorLog("zeCommandListAppendBarrier", result);
  }

  private boolean computeThreadBlock(TaskDataContext meta) {
    return meta.shouldResetThreadsBlock()
        || deviceThreadScheduling == null
        || threadBlockDispatcher == null
        || meta.isWorkerGridAvailable();
  }
  ...
  public int launchWithoutDependencies(
    ...
    if (computeThreadBlock(meta)) {
      ...
      if (TornadoOptions.USE_LEVELZERO_THREAD_DISPATCHER_SUGGESTIONS) {
        threadBlockDispatcher =
            suggestThreadSchedulingToLevelZeroDriver(
                deviceThreadScheduling, levelZeroKernel, kernel, meta);
      } else {
    ...
    launchKernelWithLevelZero(
        executionPlanId, kernel, deviceThreadScheduling, threadBlockDispatcher);
