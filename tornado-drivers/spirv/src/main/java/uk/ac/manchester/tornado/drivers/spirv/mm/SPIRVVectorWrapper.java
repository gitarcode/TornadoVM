...
public class SPIRVVectorWrapper implements XPUBuffer {
  ...
  public SPIRVVectorWrapper(final SPIRVDeviceContext device, final Object object, long batchSize) {
    TornadoInternalError.guarantee(
        object instanceof PrimitiveStorage,
        "Expecting a PrimitiveStorage type, but found: " + object.getClass());
  ...
  private int enqueueReadArrayData(
    ...
    if (kind == JavaKind.Int) {
      return deviceContext.enqueueReadBuffer(
          executionPlanId, bufferId, offset, bytes, (int[]) value, hostOffset, waitEvents);
    } else if (kind == JavaKind.Float) {
      return deviceContext.enqueueReadBuffer(
          executionPlanId, bufferId, offset, bytes, (float[]) value, hostOffset, waitEvents);
    } else if (kind == JavaKind.Double) {
      return deviceContext.enqueueReadBuffer(
          executionPlanId, bufferId, offset, bytes, (double[]) value, hostOffset, waitEvents);
    } else if (kind == JavaKind.Long) {
      return deviceContext.enqueueReadBuffer(
          executionPlanId, bufferId, offset, bytes, (long[]) value, hostOffset, waitEvents);
    } else if (kind == JavaKind.Short) {
      return deviceContext.enqueueReadBuffer(
          executionPlanId, bufferId, offset, bytes, (short[]) value, hostOffset, waitEvents);
    } else if (kind == JavaKind.Byte) {
      return deviceContext.enqueueReadBuffer(
          executionPlanId, bufferId, offset, bytes, (byte[]) value, hostOffset, waitEvents);
      ...
      if (value instanceof TornadoNativeArray tornadoNativeArray) {
        return deviceContext.enqueueReadBuffer(
            ...
            offset,
            bytes,
            tornadoNativeArray.getSegmentWithHeader().address(),
            hostOffset,
            waitEvents);
      } else {
    ...
    if (kind == JavaKind.Int) {
      ...
      if (value instanceof TornadoNativeArray nativeArray) {
        return deviceContext.enqueueWriteBuffer(
            ...
            offset,
            bytes,
            nativeArray.getSegmentWithHeader().address(),
            hostOffset,
            waitEvents);
      } else {
        throw new TornadoRuntimeException("Type not supported: " + value.getClass());
