...
public class SPIRVObjectWrapper implements XPUBuffer {
  ...
  public SPIRVObjectWrapper(final SPIRVDeviceContext deviceContext, Object object) {
    ...
    resolvedType =
        (HotSpotResolvedJavaType)
            getVMRuntime().getHostJVMCIBackend().getMetaAccess().lookupJavaType(objectType);
    ...
    for (int index = 0; index < fields.length; index++) {
      ...
      if (type.isArray()) {
        ...
        // lambda expression
        wrappedField =
            new SPIRVObjectWrapper(
                deviceContext, TornadoUtils.getObjectFromField(reflectedField, object));
      }
  ...
  public void write(long executionPlanId, Object object) {
    ...
    deviceContext.writeBuffer(
        executionPlanId, toBuffer(), bufferOffset, getObjectSize(), buffer.array(), 0, null);
    for (int i = 0; i < fields.length; i++) {
