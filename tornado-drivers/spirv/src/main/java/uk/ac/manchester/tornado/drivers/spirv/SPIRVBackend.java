...
public class SPIRVBackend extends XPUBackend<SPIRVProviders>
  ...
  public SPIRVBackend(
      ...
      SPIRVCodeProvider codeProvider,
  ...
  @Override
  protected CompiledCode createCompiledCode(
  ...
  public String toString() {
    return String.format(
        "Backend: arch=%s, device=%s",
        spirvArchitecture.getName(), deviceContext.getDevice().getDeviceName());
  }
  ...
  public FrameMapBuilder newFrameMapBuilder(RegisterConfig registerConfig) {
    RegisterConfig registerConfigNonNull =
        registerConfig == null ? getCodeCache().getRegisterConfig() : registerConfig;
    return new SPIRVFrameMapBuilder(
        newFrameMap(registerConfigNonNull), getCodeCache(), registerConfig);
  }

  @Override
  public LIRGenerationResult newLIRGenerationResult(
      CompilationIdentifier compilationId,
      LIR lir,
      FrameMapBuilder frameMapBuilder,
      RegisterAllocationConfig registerAllocationConfig) {
    return new SPIRVLIRGenerationResult(
        compilationId,
        lir,
        frameMapBuilder,
  ...
  public LIRGeneratorTool newLIRGenerator(LIRGenerationResult lirGenRes) {
    ...
    return new SPIRVLIRGenerator(
        getProviders(), lirGenRes, spirvlirGenerationResult.getMethodIndex());
  }
  ...
  public SPIRVCompilationResultBuilder newCompilationResultBuilder(
      FrameMap frameMap,
    ...
    SPIRVCompilationResultBuilder crb =
        new SPIRVCompilationResultBuilder(
            getProviders(),
            frameMap,
            asm,
            dataBuilder,
  ...
  public void emitCode(
      CompilationResultBuilder resultBuilder,
      LIR lir,
      ResolvedJavaMethod method,
      ...
      asm.module =
          new SPIRVModule( //
              new SPIRVHeader( //
                  ...
                  SPIRV_HEADER_VALUES
                      .SPIRV_INITIAL_BOUND, // The bound will be filled once the code-gen is
                                            // finished
    ...
    profiler.sum(
        ProfilerType.TOTAL_CODE_GENERATION_TIME,
        profiler.getTaskTimer(ProfilerType.TASK_CODE_GENERATION_TIME, taskMetaData.getId()));
  }
  ...
  private void emitSPIRVCapabilities(SPIRVModule module) {
    // Emit Capabilities
    module.add(
        new SPIRVOpCapability(
            SPIRVCapability
                .Addresses())); // Uses physical addressing, non-logical addressing modes.
    module.add(
        new SPIRVOpCapability(
            SPIRVCapability
                .Linkage())); // Uses partially linked modules and libraries. (e.g., OpenCL)
  ...
  private void emitOpenCLAddressingMode(SPIRVModule module) {
    ...
    module.add(
        new SPIRVOpMemoryModel(SPIRVAddressingModel.Physical64(), SPIRVMemoryModel.OpenCL()));
  }
  ...
  private SPIRVLiteralContextDependentNumber buildLiteralContextNumber(
      SPIRVKind kind, Constant value) {
    if (kind == SPIRVKind.OP_TYPE_FLOAT_16) {
      ...
      return new SPIRVContextDependentInt(
          BigInteger.valueOf(Integer.parseInt(value.toValueString())));
    } else if (kind == SPIRVKind.OP_TYPE_INT_64) {
      return new SPIRVContextDependentLong(
          BigInteger.valueOf(Long.parseLong(value.toValueString())));
      ...
      throw new TornadoRuntimeException(
          "SPIRV - SPIRVLiteralContextDependentNumber Type not supported");
    }
  }
      ...
      for (Variable var : kindToVariable.get(spirvKind)) {
        ...
        if (!TornadoOptions.OPTIMIZE_LOAD_STORE_SPIRV) {
          ...
          asm.module.add(
              new SPIRVOpDecorate(
                  variable,
                  SPIRVDecoration.Alignment(new SPIRVLiteralInteger(spirvKind.getByteCount()))));
  ...
  private void registerParallelIntrinsics(
      ControlFlowGraph cfg, SPIRVAssembler asm, SPIRVModule module) {
    ...
    if (cfg.graph
        .getNodes()
        .filter(SPIRVThreadBuiltIn.GLOBAL_THREAD_ID.getNodeClass())
        .isNotEmpty()) {
    ...
    if (cfg.graph.getNodes().filter(SPIRVThreadBuiltIn.LOCAL_THREAD_ID.getNodeClass()).isNotEmpty()
        || cfg.graph
            .getNodes()
            .filter(SPIRVThreadBuiltIn.LOCAL_THREAD_ID.getOptionalNodeClass())
            .isNotEmpty()) {
    ...
    if (cfg.graph.getNodes().filter(SPIRVThreadBuiltIn.WORKGROUP_SIZE.getNodeClass()).isNotEmpty()
        || cfg.graph
            .getNodes()
            .filter(SPIRVThreadBuiltIn.WORKGROUP_SIZE.getOptionalNodeClass())
            .isNotEmpty()) {
  ...
  private void emitPrologueForMainKernel(
      SPIRVCompilationResultBuilder crb,
      SPIRVAssembler asm,
      ResolvedJavaMethod method,
      LIR lir,
    ...
    kernelContextParameter.typeId =
        asm.primitives.getPtrToCrossWorkGroupPrimitive(SPIRVKind.OP_TYPE_INT_64);
    ...
    SPIRVInstScope functionScope =
        asm.emitOpFunction(
            returnId, methodId, methodSignatureId, SPIRVFunctionControl.DontInline());
    ...
    for (int i = 0; i < localParameters.size(); i++) {
      ...
      asm.module.add(
          new SPIRVOpDecorate(
              id,
              SPIRVDecoration.Alignment(
                  new SPIRVLiteralInteger(localParameter.kind.getSizeInBytes()))));
      ...
      asm.module.add(
          new SPIRVOpDecorate(
              idPtr,
              SPIRVDecoration.Alignment(
                  new SPIRVLiteralInteger(SPIRVKind.OP_TYPE_INT_64.getSizeInBytes()))));
    ...
    if (!TornadoOptions.OPTIMIZE_LOAD_STORE_SPIRV) {
      ...
      for (Tuple2<SPIRVId, SPIRVKind> id : idTable.list) {
        ...
        SPIRVId resultType =
            asm.primitives.getPtrOpTypePointerWithStorage(kind, SPIRVStorageClass.Function());
        blockScope.add(
            new SPIRVOpVariable(
                resultType, id.first, SPIRVStorageClass.Function(), new SPIRVOptionalOperand<>()));
      }
    }
    ...
    for (Tuple2<SPIRVId, SPIRVKind> t2 : ptrParameters) {
      ...
      blockScope.add(
          new SPIRVOpVariable(
              resultType, ptrToParam, SPIRVStorageClass.Function(), new SPIRVOptionalOperand<>()));
    }
    for (int i = 0; i < ptrParameters.size(); i++) {
      blockScope.add(
          new SPIRVOpStore(
              ptrParameters.get(i).first, //
              parameters.get(i), //
              new SPIRVOptionalOperand<>(SPIRVMemoryAccess.Aligned(new SPIRVLiteralInteger(8)))));
    }
  }

  private void emitPrologueForNonMainKernel(
      SPIRVCompilationResultBuilder crb, SPIRVAssembler asm, ResolvedJavaMethod method, LIR lir) {
    ...
    final ResolvedJavaType returnType =
        method.getSignature().getReturnType(null).resolve(method.getDeclaringClass());
    ...
    for (int i = index; i < locals.length; i++, j++) {
      ...
      if (l.getType().getJavaKind() == JavaKind.Object) {
        ...
        if (javaName.startsWith(SPIRVKind.COLLECTION_PATH)
            || javaName.startsWith(SPIRVKind.VECTOR_COLLECTION_PATH)) {
  ...
  /**
   * This method emits the global variables (Storage Class set to Input) for OpenCL thread access,
   * such as get_global_id, global_size, etc.
   *
   * <p>This is due to if the kernel is parallel, we need to declare a vector 3 elements
   * (ThreadID-0, ThreadID-1, ThreadID-2) that will be used in the OCL builtins for thread id and
   * global sizes.
   *
   * <p>Example: <code>
  ...
  public void emitBuiltinVariables(SPIRVAssembler asm) {
    SPIRVId ptrV3ulong =
        asm.primitives.getPtrOpTypePointerWithStorage(
            SPIRVKind.OP_TYPE_VECTOR3_INT_64, SPIRVStorageClass.Input());
  ...
  private void emitPrologueForMainKernelEntry(
      SPIRVCompilationResultBuilder crb,
      SPIRVAssembler asm,
      ResolvedJavaMethod method,
      LIR lir,
    ...
    if (fp64CapabilityEnabled && !supportsFP64) {
      throw new TornadoDeviceFP64NotSupported(
          "Error - The current SPIR-V device does not support FP64");
    }
    asm.emitEntryPointMainKernel(
        cfg.graph, method.getName(), fp64CapabilityEnabled, fp16CapabilityEnabled);
  ...
  private void emitPrologue(
    ...
    Logger.traceCodeGen(
        Logger.BACKEND.SPIRV,
        "[SPIR-V CodeGen] Generating SPIR-V Preamble for method: %s",
        methodName);
    if (crb.isKernel()) {
  ...
  private static class TypeConstant {
    ...
    public TypeConstant(
        SPIRVId typeID,
        SPIRVLiteralContextDependentNumber literal,
        String valueString,
        SPIRVKind kind) {
      this.typeID = typeID;
  ...
  private static class IDTable {
    ...
    public IDTable(
        List<Tuple2<SPIRVId, SPIRVKind>> list,
        Map<SPIRVKind, Set<Variable>> kindToVariable,
        ArrayList<AllocatableValue> resultArrays) {
      this.list = list;
  ...
  private static class ArrayGen {
    ...
    private static SPIRVId addSPIRVIdInPreamble(SPIRVAssembler asm, AllocatableValue resultArray) {
      ...
      asm.module.add(
          new SPIRVOpName(idArrayVariable, new SPIRVLiteralString(resultArray.toString())));
      SPIRVKind kind = (SPIRVKind) resultArray.getPlatformKind();
      asm.module.add(
          new SPIRVOpDecorate(
              idArrayVariable,
              SPIRVDecoration.Alignment(new SPIRVLiteralInteger(kind.getSizeInBytes()))));
      asm.registerLIRInstructionValue(resultArray.toString(), idArrayVariable);
      return idArrayVariable;
    }

    public static void emit(
        SPIRVAssembler asm, AllocatableValue resultArray, ValueKind<?> lirKind) {
      SPIRVId idResult = addSPIRVIdInPreamble(asm, resultArray);
      Value length = ((ArrayVariable) resultArray).getLength();
      SPIRVId primitiveTypeId =
          asm.primitives.getTypePrimitive((SPIRVKind) lirKind.getPlatformKind());
      SPIRVId elementsId;
      if (length instanceof ConstantValue) {
        elementsId =
            asm.lookUpConstant(
                ((ConstantValue) length).getConstant().toValueString(), SPIRVKind.OP_TYPE_INT_32);
      } else {
        throw new RuntimeException("Constant expected");
      }

      // Array declaration avoiding duplications
      SPIRVId resultArrayId =
          asm.declareArray((SPIRVKind) lirKind.getPlatformKind(), primitiveTypeId, elementsId);
