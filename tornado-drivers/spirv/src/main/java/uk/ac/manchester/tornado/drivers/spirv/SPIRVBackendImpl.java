...
public final class SPIRVBackendImpl implements TornadoAcceleratorBackend {
  ...
  /**
   * Matrix of backend instances. Each row has a driver implementation (e.g., Level Zero). Each
   * column represents a device within that backend.
   */
  ...
  /** Total number of devices available (include all backends platform and devices). */
  private int backendCounter;

  private List<TornadoDevice> devices;

  public SPIRVBackendImpl(
      OptionValues options, HotSpotJVMCIRuntime vmRuntime, TornadoVMConfigAccess vmConfigAccess) {
    ...
    if (numPlatforms < 1) {
      throw new TornadoBailoutRuntimeException(
          "[Warning] No SPIR-V platforms found. Deoptimizing to sequential execution");
    }
  ...
  private void discoverDevices(
      ...
      HotSpotJVMCIRuntime vmRuntime,
      TornadoVMConfigAccess vmCon,
      int numPlatforms) {
    for (int platformIndex = 0; platformIndex < numPlatforms; platformIndex++) {
      ...
      // Since a platform can have more than one device and not all of them must be SPIR-V
      // supported,
      ...
      for (int deviceIndex = 0; deviceIndex < numDevices; deviceIndex++) {
        ...
        if (device.isSPIRVSupported()) {
          SPIRVBackend backend =
              createSPIRVJITCompilerBackend(
                  options, vmRuntime, vmCon, device, context, device.getSPIRVRuntime());
  ...
  private SPIRVBackend createSPIRVJITCompilerBackend(
      OptionValues options,
      HotSpotJVMCIRuntime vmRuntime,
    ...
    return SPIRVHotSpotBackendFactory.createJITCompiler(
        options, vmRuntime, vmConfig, device, context, spirvRuntime);
  }
  ...
  public TornadoXPUDevice getDevice(int index) {
    if (index < flatBackends.length) {
      ...
      throw new TornadoDeviceNotFound(
          "[ERROR]-[SPIRV-DRIVER] Device required not found: "
              + index
              + " - Max: "
              + spirvBackends.length);
    }
  }
