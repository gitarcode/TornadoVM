...
public class SPIRVArithmeticTool extends ArithmeticLIRGenerator {
  ...
  public SPIRVLIROp genBinaryExpr(
      Variable result, SPIRVBinaryOp op, LIRKind lirKind, Value x, Value y) {
    ...
    getGen()
        .append(
            new SPIRVLIRStmt.AssignStmt(
                result, new SPIRVBinary.VectorOperation(op, lirKind, x, y)));
    return result;
  }
  ...
  protected Variable emitAdd(LIRKind resultKind, Value a, Value b, boolean setFlags) {
    Logger.traceBuildLIR(
        ...
        "[µInstructions] emitAdd: %s + %s -- RESULT KIND: %s",
        a,
        b,
        resultKind);
  ...
  public Value emitNarrow(Value inputVal, int bits) {
    ...
    final Variable result = getGen().newVariable(lirKind);
    SPIRVUnary.SignNarrowValue signNarrowValue =
        new SPIRVUnary.SignNarrowValue(lirKind, result, inputVal, bits);
  ...
  public Value emitSignExtend(Value inputVal, int fromBits, int toBits) {
    Logger.traceBuildLIR(
        Logger.BACKEND.SPIRV, "signExtend: %s , from %s to %s", inputVal, fromBits, toBits);
    LIRKind lirKind = getGen().getLIRKindTool().getIntegerKind(toBits);
    final Variable result = getGen().newVariable(lirKind);
    SPIRVUnary.SignExtend signExtend =
        new SPIRVUnary.SignExtend(lirKind, result, inputVal, fromBits, toBits);
  ...
  public Value emitZeroExtend(Value inputVal, int fromBits, int toBits) {
    Logger.traceBuildLIR(
        Logger.BACKEND.SPIRV, "emitZeroExtend: %s (from %d to %d)", inputVal, fromBits, toBits);
  ...
  public Variable emitLoad(
    ...
    Logger.traceBuildLIR(
        Logger.BACKEND.SPIRV, "emitLoad: %s <- %s with state:%s", kind, address, state);
    ...
    if (address instanceof SPIRVUnary.MemoryIndexedAccess) {
      ...
      if (spirvKind.isVector()
          && indexedAccess.getMemoryRegion().getMemorySpace() == SPIRVMemorySpace.PRIVATE) {
        Logger.traceBuildLIR(
            Logger.BACKEND.SPIRV,
            "~~ emit IndexedLoadMemCollectionAccess in address: "
                + address
                + "[ "
                + indexedAccess.getIndex()
                + "]");
        Value offset = getOffsetValue(spirvKind, indexedAccess);
        getGen()
            .append(new SPIRVLIRStmt.IndexedLoadMemCollectionAccess(indexedAccess, result, offset));
      } else {
        Logger.traceBuildLIR(
            Logger.BACKEND.SPIRV,
  ...
  public void emitStore(
      ...
      MemoryOrderMode memoryOrder) {
    Logger.traceBuildLIR(
        Logger.BACKEND.SPIRV, "emitStore: kind=%s, address=%s, input=%s", kind, address, input);
    ...
    if (spirvKind.isVector()) {
      if (address instanceof SPIRVUnary.MemoryIndexedAccess) {
        ...
        SPIRVUnary.MemoryIndexedAccess indexedAccess = (SPIRVUnary.MemoryIndexedAccess) address;
        SPIRVAddressCast cast =
            new SPIRVAddressCast(
                indexedAccess.getValue(),
                indexedAccess.getMemoryRegion(),
                LIRKind.value(spirvKind));
        Value offset = getOffsetValue(spirvKind, indexedAccess);
        Logger.traceBuildLIR(
            Logger.BACKEND.SPIRV,
            "~~ emit StoreVectorCollectionStmt in address: "
                + address
                + "[ "
                + indexedAccess.getIndex()
                + "]");
        getGen()
            .append(new SPIRVLIRStmt.StoreVectorCollectionStmt(cast, indexedAccess, input, offset));
      } else {
        SPIRVAddressCast cast =
            new SPIRVAddressCast(
                memAccess.getValue(), memAccess.getMemoryRegion(), LIRKind.value(spirvKind));
        Logger.traceBuildLIR(
            Logger.BACKEND.SPIRV, "~~ emit StoreVectorStmt in address: " + address);
      ...
      if (memAccess != null) {
        if (address instanceof MemoryAccess) {
          ...
          Logger.traceBuildLIR(
              Logger.BACKEND.SPIRV, "~~ emit SPIRVAddressCast in address: " + address);
          SPIRVAddressCast cast =
              new SPIRVAddressCast(
                  memAccess.getValue(), memoryAccess2.getMemoryRegion(), LIRKind.value(spirvKind));
          ...
          Logger.traceBuildLIR(
              Logger.BACKEND.SPIRV,
              "~~ emit StoreIndexedMemAccess in address: "
                  + address
                  + "[ "
                  + indexedAccess.getIndex()
                  + "]");
  ...
  private void emitLoad(AllocatableValue result, SPIRVAddressCast cast, MemoryAccess address) {
    Logger.traceBuildLIR(
        ...
        "emitLoad STMT: %s = (%s) %s",
        result.toString(),
        result.getPlatformKind().toString(),
        address.toString());
    getGen().append(new SPIRVLIRStmt.LoadStmt(result, cast, address));
  }

  private void emitLoadVectorType(
      AllocatableValue result, SPIRVAddressCast cast, MemoryAccess address) {
    Logger.traceBuildLIR(
        Logger.BACKEND.SPIRV,
        "emitLoadVector STMT: %s = (%s) %s",
        result.toString(),
        result.getPlatformKind().toString(),
        address.toString());
  ...
  private Value getPrivateOffsetValue(
      SPIRVKind spirvKind, SPIRVUnary.MemoryIndexedAccess memoryAccess) {
    ...
    if (memoryAccess.getIndex() instanceof ConstantValue) {
      ...
      privateOffsetValue =
          new ConstantValue(LIRKind.value(SPIRVKind.OP_TYPE_INT_64), JavaConstant.forInt(index));
    }
  ...
  public Value emitFMAInstruction(Value op1, Value op2, Value op3) {
    ...
    if ((op1 instanceof SPIRVVectorElementSelect)
        || (op2 instanceof SPIRVVectorElementSelect)
        || (op3 instanceof SPIRVVectorElementSelect)) {
    ...
    SPIRVUnary.Intrinsic.OpenCLExtendedIntrinsic operation =
        SPIRVUnary.Intrinsic.OpenCLExtendedIntrinsic.FMA;
    ...
    getGen()
        .append(
            new SPIRVLIRStmt.AssignStmt(
                result,
                new SPIRVTernary.TernaryIntrinsic(result, operation, resultKind, op1, op2, op3)));
    return result;
  }
  ...
  public Value emitRSQRT(Value op) {
    ...
    getGen()
        .append(
            new SPIRVLIRStmt.AssignStmt(
                result,
                new SPIRVUnary.Intrinsic(
                    SPIRVUnary.Intrinsic.OpenCLExtendedIntrinsic.RSQRT,
                    LIRKind.value(op.getPlatformKind()),
                    op)));
    return result;
  }
}
