...
public class SPIRVVectorValueNode extends FloatingNode
    implements LIRLowerable, MarkVectorValueNode {

  public static final NodeClass<SPIRVVectorValueNode> TYPE =
      NodeClass.create(SPIRVVectorValueNode.class);
  ...
  public void initialiseToDefaultValues(StructuredGraph graph) {
    final ConstantNode defaultValue =
        ConstantNode.forPrimitive(spirvKind.getElementKind().getDefaultValue(), graph);
    ...
    if (origin instanceof InvokeNode) {
      ...
      final AllocatableValue result =
          (gen.hasOperand(this))
              ...
              : tool.newVariable(LIRKind.value(getSPIRVKind()));
  ...
  private Value getParam(NodeLIRBuilderTool gen, LIRGeneratorTool tool, int index) {
    ...
    return (valueNode == null)
        ? new ConstantValue(LIRKind.value(spirvKind), spirvKind.getDefaultValue())
  ...
  private void generateVectorAssign(
    ...
    switch (spirvKind.getVectorLength()) {
      ...
      case 8:
        ...
        lirKind = gen.getLIRGeneratorTool().getLIRKind(stamp);
        assignExpr =
            new SPIRVVectorAssign.AssignVectorExpr(lirKind, s0, s1, s2, s3, s4, s5, s6, s7);
        break;
      case 16:
        {
          s1 = getParam(gen, tool, 1);
          ...
          lirKind = gen.getLIRGeneratorTool().getLIRKind(stamp);
          assignExpr =
              new SPIRVVectorAssign.AssignVectorExpr(
                  lirKind, s0, s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, s13, s14, s15);
          break;
        }
      default:
