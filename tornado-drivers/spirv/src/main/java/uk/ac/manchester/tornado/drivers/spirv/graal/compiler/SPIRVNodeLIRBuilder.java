...
/**
 * It traverses the HIR instructions from the Graal CFP and it generates LIR for the SPIR-V backend.
 *
 * <p>SPIR-V Visitor from HIR to LIR.
 */
public class SPIRVNodeLIRBuilder extends NodeLIRBuilder {
  ...
  public SPIRVNodeLIRBuilder(
      StructuredGraph graph, LIRGeneratorTool gen, NodeMatchRules nodeMatchRules) {
  ...
  protected void emitDirectCall(
    ...
    Logger.traceBuildLIR(
        ...
        result,
        callState);
    final SPIRVDirectCall spirvDirectCall =
        new SPIRVDirectCall(callTarget, result, parameters, callState);
  ...
  public void emitInvoke(Invoke x) {
    ...
    if (callTargetNode instanceof DirectCallTargetNode) {
      emitDirectCall(
          (DirectCallTargetNode) callTargetNode,
          result,
          parameters,
  ...
  protected void emitPrologue(final StructuredGraph graph, boolean isKernel) {
    if (isKernel) {
      ...
      for (ParameterNode param : graph.getNodes(ParameterNode.TYPE)) {
        setResult(
            ...
            getGen()
                .getSPIRVGenTool()
                .emitParameterLoad(
                    param, param.index() + 1)); // + 1 to account for the kernel context
      }
    } else {
      ...
      for (final ParameterNode param : graph.getNodes(ParameterNode.TYPE)) {
        ...
        Logger.traceBuildLIR(
            Logger.BACKEND.SPIRV, "Generating LoadParameter : " + locals[param.index()].getName());
        Variable result = getGen().newVariable(lirKind);
        getGen()
            .append(
                new SPIRVLIRStmt.StoreFunctionParameter(
                    result,
                    new SPIRVUnary.LoadParameter(locals[param.index()], lirKind, param.index())));
  ...
  public void visitEndNode(final AbstractEndNode end) {
    Logger.traceBuildLIR(
        Logger.BACKEND.SPIRV, "[µInst visitEnd (SPIRVNodeLIRBuilder#visitEndNode)]: " + end);
  ...
  protected LIRKind getPhiKind(PhiNode phi) {
    ...
    if (stamp.isEmpty()) {
      ...
      SPIRVKind kind =
          SPIRVKind.fromResolvedJavaTypeToVectorKind(oStamp.javaType(gen.getMetaAccess()));
  ...
  private Variable emitLogicNode(final LogicNode node) {
    ...
    if (node instanceof IntegerLessThanNode) {
      ...
      append(
          new SPIRVLIRStmt.IgnorableAssignStmt(
              result, new SPIRVBinary.Expr(result, op, boolLIRKind, x, y)));
      ...
      append(
          new SPIRVLIRStmt.IgnorableAssignStmt(
              result, new SPIRVBinary.IntegerTestNode(op, intLIRKind, x, y)));
  ...
  public void emitIf(final IfNode x) {
    Logger.traceBuildLIR(
        Logger.BACKEND.SPIRV, "emitIf: %s, condition=%s", x, x.condition().getClass().getName());
    ...
    if (isLoop) {
      getGen()
          .emitConditionalBranch(
              condition, getLIRBlock(x.trueSuccessor()), getLIRBlock(x.falseSuccessor()));
    } else {
      getGen()
          .emitConditionalBranch(
              condition, getLIRBlock(x.trueSuccessor()), getLIRBlock(x.falseSuccessor()));
    ...
    for (ValuePhiNode phi : mergeNode.valuePhis()) {
      ...
      if (valuePhi != phi) {
        ...
        append(new SPIRVLIRStmt.AssignStmtWithLoad(dest, src));
          ...
          && isPhiValueInPhiTraceTableOrConstant(phi)) {
  ...
  private void generateOpPhiInstruction(
      LIRPhiVars phiVars, HIRBlock dependentPhiValueBlock, final HIRBlock predBlock) {
    ...
    for (LIRPhiVars.PhiMeta meta : phiVars.getPhiVars()) {
      ...
      append(
          new SPIRVLIRStmt.OpPhiStmt(
              meta.getResultPhi(),
              meta.getValue(),
              dependentPhiValueBlock.toString(),
              predBlock.toString(),
              phiMap,
              phiTrace));
    }
  ...
  protected void emitNode(final ValueNode node) {
    Logger.traceBuildLIR(
        Logger.BACKEND.SPIRV, " [emitNode (SPIRVNodeLIRBuilder#emitNode)] visiting: %s", node);
