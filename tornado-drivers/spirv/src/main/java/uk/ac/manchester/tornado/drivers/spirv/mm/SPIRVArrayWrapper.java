...
public abstract class SPIRVArrayWrapper<T> implements XPUBuffer {
  ...
  protected SPIRVArrayWrapper(
      final T array, final SPIRVDeviceContext device, final JavaKind kind, long batchSize) {
  ...
  private SPIRVByteBuffer getArrayHeader() {
    final SPIRVByteBuffer header =
        new SPIRVByteBuffer(deviceContext, bufferId, bufferOffset, arrayHeaderSize);
    header.buffer.clear();
    return header;
  }
