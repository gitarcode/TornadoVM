...
public class VectorElementOpNode extends FloatingNode
  ...
  @Input protected ValueNode lane;

  public VectorElementOpNode(
      NodeClass<? extends FloatingNode> c, SPIRVKind kind, ValueNode vector, ValueNode lane) {
    ...
    if (vectorStamp instanceof SPIRVStamp) {
      ...
      if (objectStamp.type() != null) {
        ...
        guarantee(
            ...
            vectorKind);
        guarantee(
            vectorKind.getVectorLength() >= laneId(), "Invalid lane %d on type %s", laneId(), kind);
      }
    } else {
      shouldNotReachHere(
  ...
  public void generate(NodeLIRBuilderTool gen) {
    ...
    Logger.traceBuildLIR(
        ...
        "emitVectorElementOp SELECT: targetVector=%s, laneId=%d",
        targetVector,
        laneId());
    ...
    final SPIRVVectorElementSelect element =
        new SPIRVVectorElementSelect(
            lirKind,
            LIRKind.value(targetVector.getPlatformKind()),
