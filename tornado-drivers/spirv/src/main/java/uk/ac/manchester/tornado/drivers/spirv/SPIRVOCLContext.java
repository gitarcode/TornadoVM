...
public class SPIRVOCLContext extends SPIRVContext {
  ...
  public SPIRVOCLContext(
      SPIRVPlatform platform, List<SPIRVDevice> devices, OCLContextInterface context) {
  ...
  public OCLCommandQueue getCommandQueue(long executionPlanId, int deviceIndex) {
    ...
    return commmandQueueTable
        .get(executionPlanId)
        .get((SPIRVOCLDevice) devices.get(deviceIndex), (OCLContext) oclContext);
  }
  ...
  public int readBuffer(
      ...
      long hostOffset,
      int[] waitEvents,
      ProfilerTransfer profilerTransferp) {
  ...
  public int enqueueWriteBuffer(
    ...
    return eventPool.registerEvent(
        commandQueue.enqueueWrite(
            ...
            value,
            hostOffset,
            eventPool.serialiseEvents(waitEvent, commandQueue) ? eventPool.waitEventsBuffer : null),
        EventDescriptor.DESC_WRITE_BYTE,
        commandQueue);
  }
