...
public class SPIRVLevelZeroContext extends SPIRVContext {
  ...
  private static final String BUFFER_NOT_FOUND_ERROR_MESSAGE =
      "Should always have a buffer created at this point.";
  ...
  public SPIRVLevelZeroContext(
      SPIRVPlatform platform, List<SPIRVDevice> devices, LevelZeroContext levelZeroContext) {
  ...
  @Override
  public SPIRVLevelZeroCommandQueue getCommandQueueForDevice(
      long executionPlanId, int deviceIndex) {
    if (!commmandQueueTable.containsKey(executionPlanId)) {
      SPIRVDevice device = devices.get(deviceIndex);
      SPIRVLevelZeroCommandQueueTable spirvCommandQueueTable =
          new SPIRVLevelZeroCommandQueueTable();
  ...
  private void registerTimeStamp(
      LevelZeroCommandList commandList, TimeStamp startT, TimeStamp stopT) {
  ...
  public long allocateMemory(int deviceIndex, long numBytes) {
    ...
    if (TornadoOptions.LEVEL_ZERO_EXTENDED_MEMORY_MODE) {
      ...
      relaxedAllocationLimitsExpDescriptor.setFlags(
          ZeRelaxedAllocationLimitsFlags.ZE_RELAXED_ALLOCATION_LIMITS_EXP_FLAG_MAX_SIZE);
    ...
    if (TornadoOptions.LEVEL_ZERO_SHARED_MEMORY) {
      ...
      int result =
          levelZeroContext.zeMemAllocShared( //
      ...
      int result =
          levelZeroContext.zeMemAllocDevice( //
              levelZeroContext.getDefaultContextPtr(), //
              deviceMemAllocDesc, //
              numBytes, //
              TornadoOptions.LEVEL_ZERO_BUFFER_ALIGNMENT, //
  ...
  public int readBuffer(
    ...
    SPIRVLevelZeroCommandQueue spirvCommandQueue =
        getCommandQueueForDevice(executionPlanId, deviceIndex);
    ...
    int result =
        commandList.zeCommandListAppendMemoryCopyWithOffset(
  ...
  public void flush(long executionPlanId, int deviceIndex) {
    ...
    result =
        commandQueue.zeCommandQueueExecuteCommandLists(
            commandQueue.getCommandQueueHandlerPtr(), 1, commandList.getCommandListHandler(), null);
    LevelZeroUtils.errorLog("zeCommandQueueExecuteCommandLists", result);

    // Synchronize
    result =
        commandQueue.zeCommandQueueSynchronize(
            commandQueue.getCommandQueueHandlerPtr(), Long.MAX_VALUE);
