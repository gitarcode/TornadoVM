...
public class SPIRVLevelZeroCommandQueueTable {
  ...
  public SPIRVLevelZeroCommandQueue get(SPIRVDevice device, LevelZeroContext levelZeroContext) {
    ...
    return deviceCommandMap
        .get(device)
        .get(Thread.currentThread().threadId(), device, levelZeroContext);
  }
  ...
  private static class ThreadCommandQueueTable {
    ...
    public SPIRVLevelZeroCommandQueue get(
        long threadId, SPIRVDevice device, LevelZeroContext levelZeroContext) {
      if (!commandQueueMap.containsKey(threadId)) {
        ...
        SPIRVLevelZeroCommandQueue spirvLevelZeroCommandQueue =
            new SPIRVLevelZeroCommandQueue(
                commandQueue, commandList, (LevelZeroDevice) device.getDeviceRuntime());
        commandQueueMap.put(threadId, spirvLevelZeroCommandQueue);
      }
      return commandQueueMap.get(threadId);
    }

    private LevelZeroCommandQueue createCommandQueue(
        LevelZeroContext context, SPIRVDevice spirvDevice) {
      ...
      int result =
          context.zeCommandQueueCreate(
              context.getContextHandle().getContextPtr()[0],
    ...
    private LevelZeroCommandList createCommandList(
        LevelZeroContext context, SPIRVDevice spirvDevice) {
      LevelZeroDevice device = (LevelZeroDevice) spirvDevice.getDeviceRuntime();
    ...
    private int getCommandQueueOrdinal(LevelZeroDevice device) {
      ...
      int result =
          device.zeDeviceGetCommandQueueGroupProperties(
              device.getDeviceHandlerPtr(), numQueueGroups, null);
      LevelZeroUtils.errorLog("zeDeviceGetCommandQueueGroupProperties", result);

      if (numQueueGroups[0] == 0) {
        throw new RuntimeException(
            "Number of Queue Groups is 0 for device: " + device.getDeviceProperties().getName());
      }
      int ordinal = numQueueGroups[0];

      if (device.getCommandQueueGroupProperties() == null) {
        ZeCommandQueueGroupProperties[] commandQueueGroupProperties =
            new ZeCommandQueueGroupProperties[numQueueGroups[0]];
        result =
            device.zeDeviceGetCommandQueueGroupProperties(
                device.getDeviceHandlerPtr(), numQueueGroups, commandQueueGroupProperties);
        LevelZeroUtils.errorLog("zeDeviceGetCommandQueueGroupProperties", result);
      }

      for (int i = 0; i < numQueueGroups[0]; i++) {
        if ((device.getCommandQueueGroupProperties(i).getFlags()
