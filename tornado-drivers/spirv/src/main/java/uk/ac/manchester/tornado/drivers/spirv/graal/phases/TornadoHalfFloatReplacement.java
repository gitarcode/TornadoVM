...
public class TornadoHalfFloatReplacement extends BasePhase<TornadoHighTierContext> {
  ...
  protected void run(StructuredGraph graph, TornadoHighTierContext context) {
    ...
    for (JavaWriteNode javaWrite : graph.getNodes().filter(JavaWriteNode.class)) {
      if (isWriteHalfFloat(javaWrite)) {
        ...
        if (javaWrite.predecessor() instanceof NewHalfFloatInstance) {
          ...
          NewHalfFloatInstance newHalfFloatInstance =
              (NewHalfFloatInstance) javaWrite.predecessor();
        ...
        WriteHalfFloatNode writeHalfFloatNode =
            new WriteHalfFloatNode(writingAddress, writingValue);
    ...
    for (LoadIndexedVectorNode loadIndexedVectorNode :
        graph.getNodes().filter(LoadIndexedVectorNode.class)) {
    ...
    for (SPIRVVectorValueNode vectorValueNode :
        graph.getNodes().filter(SPIRVVectorValueNode.class)) {
      if (vectorValueNode.getSPIRVKind().isHalf()) {
        for (Node vectorElement : vectorValueNode.inputs()) {
          if (vectorElement instanceof VectorLoadElementNode) {
            ...
            VectorLoadElementNode vectorLoadShort =
                new VectorLoadElementNode(
                    SPIRVKind.OP_TYPE_FLOAT_16, vectorLoad.getVector(), vectorLoad.getLaneId());
            graph.addWithoutUnique(vectorLoadShort);
            vectorLoad.replaceAtUsages(vectorLoadShort);
            vectorLoad.safeDelete();
          } else if (vectorElement instanceof ConstantNode constantNode
              && constantNode.getValue().toValueString().contains("null")) {
  ...
  private static ValueNode replaceAdd(AddHalfFloatNode addHalfFloatNode, StructuredGraph graph) {
    ...
    if (addX instanceof VectorLoadElementNode
        || addY instanceof VectorLoadElementNode
        || addX instanceof VectorAddHalfNode
        || addY instanceof VectorAddHalfNode) {
  ...
  private static ValueNode replaceSub(SubHalfFloatNode subHalfFloatNode, StructuredGraph graph) {
    ...
    if (subX instanceof VectorLoadElementNode
        || subY instanceof VectorLoadElementNode
        || subX instanceof VectorSubHalfNode
        || subY instanceof VectorSubHalfNode) {
  ...
  private static boolean isVectorOp(Node x, Node y) {
    if (x instanceof VectorLoadElementNode
        || y instanceof VectorLoadElementNode
        || x instanceof VectorMultHalfNode
        || y instanceof VectorMultHalfNode) {
      return true;
    } else {
      return false;
    }
  ...
  private static ValueNode replaceMult(MultHalfFloatNode multHalfFloatNode, StructuredGraph graph) {
    ...
    if (multX instanceof VectorLoadElementNode
        || multY instanceof VectorLoadElementNode
        || multX instanceof VectorMultHalfNode
        || multY instanceof VectorMultHalfNode) {
  ...
  private static ValueNode getHalfOperand(
      ValueNode operand, StructuredGraph graph, boolean isVectorOperation) {
    ...
    if (operand instanceof VectorLoadElementNode) {
      ...
      halfOperand =
          new VectorLoadElementNode(
              SPIRVKind.OP_TYPE_INT_16, loadElementNodeX.getVector(), loadElementNodeX.getLaneId());
      ...
      graph.addWithoutUnique(halfOperand);
        ...
        instanceof
        MultHalfFloatNode) { // if operand is another half operator, replace this first to avoid
