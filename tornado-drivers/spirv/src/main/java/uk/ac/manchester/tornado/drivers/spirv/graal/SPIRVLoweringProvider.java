...
public class SPIRVLoweringProvider extends DefaultJavaLoweringProvider {
  ...
  /**
   * {@link SPIRVLoweringProvider#gpuSnippet} is set during the lowering phase. Therefore, this
   * method must be called after a lowering phase in order to get the correct result.
   *
   * @return boolean
   */
  ...
  private void lowerReduceSnippets(Node node, LoweringTool tool) {
    ...
    GlobalThreadSizeNode spirvGlobalSize =
  ...
  private void lowerLocalNewArray(
    ...
    localArrayNode =
        graph.addWithoutUnique(
            new LocalArrayNode(
                SPIRVArchitecture.localSpace, newArray.elementType(), newLengthNode));
  ...
  private void lowerPrivateNewArray(
    ...
    fixedArrayNode =
        graph.addWithoutUnique(
            new FixedArrayNode(
                SPIRVArchitecture.privateSpace, newArray.elementType(), newLengthNode));
  ...
  /**
   * From Graal: it indicates the smallest width for comparing an integer value on the target
   * platform.
   */
  @Override
  ...
  private AbstractWriteNode createMemWriteNode(
    ...
    if (!(valueStamp instanceof SPIRVStamp)
        || !((SPIRVStamp) valueStamp).getSPIRVKind().isVector()) {
  ...
  public void lowerStoreIndexedNode(StoreIndexedNode storeIndexed, LoweringTool tool) {
    ...
    AbstractWriteNode memoryWrite =
        createMemWriteNode(elementKind, valueToStore, array, address, graph, storeIndexed);
