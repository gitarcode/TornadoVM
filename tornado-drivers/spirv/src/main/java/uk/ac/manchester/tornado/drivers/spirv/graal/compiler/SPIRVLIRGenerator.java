...
/**
 * It traverses the SPIR-V HIR and generates SPIR-V LIR from which the backend will emit the SPIR-V
 * code.
 */
public class SPIRVLIRGenerator extends LIRGenerator {
  ...
  public SPIRVLIRGenerator(
      CodeGenProviders providers, LIRGenerationResult lirGenRes, final int methodIndex) {
    super(
        new SPIRVLIRKindTool((SPIRVTargetDescription) providers.getCodeCache().getTarget()),
        new SPIRVArithmeticTool(),
        new SPIRVBarrierSetLIRGenerator(),
        new SPIRVMoveFactory(),
        providers,
        lirGenRes);
  ...
  public void emitReturn(JavaKind javaKind, Value input) {
    ...
    if (input != null) {
      ...
      append(
          new SPIRVLIRStmt.ExprStmt(new SPIRVUnary.ReturnWithValue(lirKind, input, currentBlock)));
    } else {
      append(
          new SPIRVLIRStmt.ExprStmt(
              new SPIRVNullary.ReturnNoOperands(LIRKind.Illegal, currentBlock)));
    }
  }
  ...
  public Variable emitConditionalMove(
    ...
    SPIRVBinary.TernaryCondition ternaryInstruction =
        new SPIRVBinary.TernaryCondition(
            LIRKind.combine(trueValue, falseValue), leftVal, cond, right, trueValue, falseValue);
  ...
  public Variable emitIntegerTestMove(
    ...
    assert leftVal.getPlatformKind() == right.getPlatformKind()
        && ((SPIRVKind) leftVal.getPlatformKind()).isInteger();
    ...
    SPIRVBinary.IntegerTestNode integerTestNode =
        new SPIRVBinary.IntegerTestNode(
            SPIRVAssembler.SPIRVBinaryOp.BITWISE_AND, kind, leftVal, right);
    SPIRVBinary.IntegerTestMoveNode moveNode =
        new SPIRVBinary.IntegerTestMoveNode(integerTestNode, result, kind, trueValue, falseValue);
  ...
  public Variable newVariable(ValueKind<?> valueKind) {
    ...
    Logger.traceBuildLIR(
        Logger.BACKEND.SPIRV,
        "[SPIR-V] newVariable: %s <- %s (%s)",
        variable.toString(),
        actualLIRKind.toString(),
        actualLIRKind.getClass().getName());
  ...
  public void emitJump(LabelRef label, boolean isLoopEdgeBack) {
    Logger.traceBuildLIR(
        Logger.BACKEND.SPIRV, "emitJump: label=%s isLoopEdgeBack=%b", label, isLoopEdgeBack);
