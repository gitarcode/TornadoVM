...
/**
 * Class to map a SPIR-V device (Device represented either in LevelZero or an OpenCL device) with an
 * SPIR-V Context.
 */
public abstract class SPIRVDeviceContext implements TornadoDeviceContext {
  ...
  private ProfilerTransfer createStartAndStopBufferTimers() {
    if (this instanceof SPIRVLevelZeroDeviceContext && TornadoOptions.isProfilerEnabled()) {
      LevelZeroTransferTimeStamp start =
          new LevelZeroTransferTimeStamp(spirvContext, (LevelZeroDevice) device.getDeviceRuntime());
  ...
  public int enqueueWriteBuffer(
    ...
    ProfilerTransfer profilerTransfer = createStartAndStopBufferTimers();
    executionIds.add(executionPlanId);
    spirvContext.enqueueWriteBuffer(
        executionPlanId,
  ...
  public TornadoInstalledCode installBinary(SPIRVCompilationResult result) {
    return installBinary(
        result.getMeta(), result.getId(), result.getName(), result.getSPIRVBinary());
  }

  public SPIRVInstalledCode installBinary(
  ...
  public Event resolveEvent(long executionPlanId, int eventId) {
    ...
    if (this instanceof SPIRVLevelZeroDeviceContext) {
      ...
      OCLCommandQueue commandQueue =
          context.getCommandQueue(executionPlanId, spirvoclDeviceContext.getDeviceIndex());
      OCLEventPool eventPool = context.getOCLEventPool(executionPlanId);
      return new OCLEvent(
          eventPool.getDescriptor(eventId).getNameDescription(),
          commandQueue,
          eventId,
          eventPool.getOCLEvent(eventId));
    } else {
