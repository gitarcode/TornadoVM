...
public class TestSPIRVJITCompiler {
  ...
  public static void main(String[] args) {
    System.out.println(
        "Running Native: uk.ac.manchester.tornado.drivers.spirv.tests.TestSPIRVJITCompiler");
  ...
  public MetaCompilation compileMethod(Class<?> klass, String methodName, Object... parameters) {
    ...
    SPIRVBackend spirvBackend =
        tornadoRuntime.getBackend(SPIRVBackendImpl.class).getDefaultBackend();
    ...
    SPIRVCompilationResult spirvCompilationResult =
        SPIRVCompiler.compileSketchForDevice(
            sketch,
            compilableTask,
            (SPIRVProviders) spirvBackend.getProviders(),
            spirvBackend,
            new EmptyProfiler());

    // 3. Install the SPIR-V code into the VM
    SPIRVDevice spirvDevice = (SPIRVDevice) device.getDeviceContext().getDevice();
    SPIRVInstalledCode spirvInstalledCode =
        (SPIRVInstalledCode) spirvDevice.getDeviceContext().installBinary(spirvCompilationResult);

    return new MetaCompilation(taskMeta, spirvInstalledCode);
  }

  public void run(
      SPIRVTornadoDevice spirvTornadoDevice,
      SPIRVInstalledCode installedCode,
    ...
    spirvTornadoDevice.allocateObjects(
        new Object[] {a, b, c},
        0,
  ...
  public void test() {
    ...
    MetaCompilation compileMethod =
        compileMethod(TestSPIRVJITCompiler.class, "methodToCompile", a, b, c);
    ...
    run(
        (SPIRVTornadoDevice) device,
        (SPIRVInstalledCode) compileMethod.getInstalledCode(),
