...
public class SPIRVCompiler {
  ...
  private static synchronized SPIRVCompilationResult compile(SPIRVCompilationRequest r) {
  ...
  private static void emitFrontEnd(
    ...
    try (DebugContext.Scope s =
            getDebugContext().scope("SPIRVFrontend", new DebugDumpScope("SPIRVFrontend"));
        DebugCloseable a = FrontEnd.start(getDebugContext())) {

      /*
       * Register metadata with all tornado phases
       */
      ((SPIRVCanonicalizer) suites.getHighTier().getCustomCanonicalizer())
          .setContext(providers.getMetaAccess(), installedCodeOwner, args, meta);
      ...
      final TornadoMidTierContext midTierContext =
          new TornadoMidTierContext(
              providers, backend, optimisticOpts, profilingInfo, installedCodeOwner, args, meta);
      ...
      getDebugContext()
          .dump(DebugContext.BASIC_LEVEL, graph.getLastSchedule(), "Final LIR schedule");
  ...
  private static void emitBackEnd(
      StructuredGraph graph,
      Object stub,
      ResolvedJavaMethod installedCodeOwner,
      SPIRVBackend backend,
    ...
    try (DebugContext.Scope s = getDebugContext().scope("SPIRVBackend", graph.getLastSchedule());
      ...
      try (DebugContext.Scope s2 =
          getDebugContext().scope("SPIRVCodeGen", lirGen, lirGen.getLIR())) {
  ...
  private static LIRGenerationResult emitLIR(
      SPIRVBackend backend,
      StructuredGraph graph,
      Object stub,
      RegisterConfig registerConfig,
  ...
  private static <T extends CompilationResult> LIRGenerationResult emitLIR0(
    ...
    try (DebugContext.Scope ds = getDebugContext().scope("EmitLIR");
      ...
      SPIRVLIRGenerationResult lirGenRes =
          (SPIRVLIRGenerationResult)
      ...
      // LIR generation
      SPIRVIRGenerationPhase.LIRGenerationContext context =
          new SPIRVIRGenerationPhase.LIRGenerationContext(
              lirGen, nodeLirGen, graph, schedule, isKernel);
      ...
      try (DebugContext.Scope s = getDebugContext().scope("LIRStages", nodeLirGen, lir)) {
        ...
        LIRGenerationResult result =
            emitLowLevel(
                backend.getTarget(), lirGenRes, lirGen, lirSuites, registerAllocationConfig);
  ...
  private static void emitCode(
    ...
    try (DebugCloseable a = EmitCode.start(getDebugContext())) {
      ...
      final SPIRVCompilationResultBuilder crb =
          backend.newCompilationResultBuilder(
              frameMap, compilationResult, isKernel, isParallel, lirGen.getLIR());
  ...
  public static SPIRVCompilationResult compileSketchForDevice(
      Sketch sketch,
      CompilableTask task,
    ...
    new TornadoLogger()
        .info(
            ...
            resolvedJavaMethod.getName(), backend.getDeviceContext().getDevice().getDeviceName());
    ...
    SPIRVCompilationResult kernelCompilationResult =
        new SPIRVCompilationResult(
            task.getId(), buildKernelName(resolvedJavaMethod.getName()), taskMeta);
    ...
    // @formatter:off
    SPIRVCompilationRequest kernelCompilationRequest =
        new SPIRVCompilationRequest(
            kernelGraph,
            resolvedJavaMethod,
            args,
            taskMeta,
    ...
    final Deque<ResolvedJavaMethod> workList =
        new ArrayDeque<>(kernelCompilationResult.getNonInlinedMethods());
    while (!workList.isEmpty()) {
      final ResolvedJavaMethod currentMethod = workList.pop();
      Sketch currentSketch =
          TornadoSketcher.lookup(
              currentMethod, task.meta().getBackendIndex(), taskMeta.getDeviceIndex());
      final StructuredGraph graph =
          (StructuredGraph) currentSketch.getGraph().copy(getDebugContext());

      final SPIRVCompilationResult compilationResult =
          new SPIRVCompilationResult(task.getId(), currentMethod.getName(), taskMeta);
      ...
      // @formatter:off
      SPIRVCompilationRequest methodCompilationRequest =
          new SPIRVCompilationRequest(
              graph,
              currentMethod,
              null,
  ...
  public static class SPIRVCompilationRequest {
    ...
    public SPIRVCompilationRequest(
        StructuredGraph graph,
        ResolvedJavaMethod installedCodeOwner,
        Object[] args,
        TaskDataContext meta,
