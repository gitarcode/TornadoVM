...
public class TornadoSPIRVIntrinsicsReplacements extends BasePhase<TornadoHighTierContext> {
    ...
    for (InvokeNode invoke : invokeNodes) {
      ...
      switch (methodName) {
        ...
        case "Direct#SPIRVOCLIntrinsics.localBarrier":
          {
            SPIRVBarrierNode barrierNode =
                graph.addOrUnique(new SPIRVBarrierNode(SPIRVBarrierNode.SPIRVMemFenceFlags.LOCAL));
        ...
        case "Direct#SPIRVOCLIntrinsics.globalBarrier":
          {
            SPIRVBarrierNode barrierNode =
                graph.addOrUnique(new SPIRVBarrierNode(SPIRVBarrierNode.SPIRVMemFenceFlags.GLOBAL));
            graph.replaceFixed(invoke, barrierNode);
            break;
          }
        case "Direct#SPIRVOCLIntrinsics.get_local_id":
          {
            ...
            LocalThreadIdFixedNode localIdNode =
                graph.addOrUnique(new LocalThreadIdFixedNode(dimension));
        ...
        case "Direct#SPIRVOCLIntrinsics.get_global_id":
          {
            ...
            GlobalThreadIdNode globalThreadIdNode =
                graph.addOrUnique(new GlobalThreadIdNode(dimension));
        ...
        case "Direct#SPIRVOCLIntrinsics.get_global_size":
          {
            ...
            GlobalThreadSizeNode globalThreadSizeNode =
                graph.addOrUnique(new GlobalThreadSizeNode(dimension));
  ...
  private void lowerLocalInvokeNodeNewArray(
      StructuredGraph graph, int length, JavaKind elementKind, InvokeNode invokeWithNewArray) {
    ...
    LocalArrayNode localArrayNode =
        graph.addOrUnique(
            new LocalArrayNode(SPIRVArchitecture.localSpace, elementType, newLengthNode));
    invokeWithNewArray.replaceAtUsages(localArrayNode);
  }

  private void lowerPrivateInvokeNodeNewArray(
      StructuredGraph graph, int size, JavaKind elementKind, InvokeNode invokeWithNewArray) {
    final ConstantNode newLenghNode = ConstantNode.forInt(size, graph);
    ResolvedJavaType elementType = metaAccessProvider.lookupJavaType(elementKind.toJavaClass());
    FixedArrayNode fixedArrayNode =
        graph.addOrUnique(
            new FixedArrayNode(SPIRVArchitecture.kernelContextSpace, elementType, newLenghNode));
    ...
    if (secondInput instanceof ConstantNode lengthNode) {
      if (lengthNode.getValue() instanceof PrimitiveConstant) {
        ...
        JavaKind elementKind =
            getJavaKindFromConstantNode((ConstantNode) callTarget.arguments().get(0));
