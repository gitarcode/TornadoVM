...
/** This class specifies how to load a parameter to the kernel from the TornadoVM Stack-Frame. */
public class SPIRVGenTool {
  ...
  public Value emitParameterLoad(ParameterNode paramNode, int index) {
    Logger.traceBuildLIR(
        Logger.BACKEND.SPIRV, "emitParameterLoad: stamp=%s", paramNode.stamp(NodeView.DEFAULT));
    ...
    Variable result =
        (spirvKind.isVector())
            ? generator.newVariable(LIRKind.value(target.getSPIRVKind(JavaKind.Object)))
            : generator.newVariable(lirKind);
  ...
  private void emitParameterLoad(AllocatableValue resultValue, int index) {
    ...
    SPIRVLIRStmt.ASSIGNParameter assignStmt =
        new SPIRVLIRStmt.ASSIGNParameter( //
            resultValue, //
  ...
  private void emitParameterLoadWithNoStore(AllocatableValue resultValue, int index) {
    ...
    SPIRVLIRStmt.ASSIGNParameterWithNoStore assignStmt =
        new SPIRVLIRStmt.ASSIGNParameterWithNoStore( //
            resultValue, //
            new SPIRVUnary.AssignLoadFromInputFrame( //
                lirKind, //
                SPIRVKind.OP_TYPE_INT_8, //
