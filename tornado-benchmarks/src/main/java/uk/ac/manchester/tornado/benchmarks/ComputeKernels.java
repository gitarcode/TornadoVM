...
public class ComputeKernels {
  ...
  /**
   * Parallel Implementation of the MonteCarlo computation: this is based on the Marawacc compiler
   * framework.
   *
   * @author Juan Fumero
   */
  ...
  public static void nBody(
      int numBodies, FloatArray refPos, FloatArray refVel, float delT, float espSqr) {
    for (@Parallel int i = 0; i < numBodies; i++) {
      ...
      for (int k = 0; k < 3; k++) {
        refPos.set(
            body + k,
            refPos.get(body + k) + refPos.get(body + k) * delT + 0.5f * acc[k] * delT * delT);
  ...
  public static void blackscholes(
      final FloatArray randArray, final FloatArray put, final FloatArray call) {
    for (@Parallel int gid = 0; gid < call.getSize(); gid++) {
      ...
      final float d1 =
          (TornadoMath.log(S / K) + ((R + ((sigmaVal * sigmaVal) / two)) * T)) / sigmaSqrtT;
  ...
  public static void computeDFT(
      DoubleArray inreal, DoubleArray inimag, DoubleArray outreal, DoubleArray outimag) {
  ...
  /**
   * Parallel Implementation of the Mandelbrot: this is based on the Marawacc compiler framework.
   *
   * @author Juan Fumero
   */
  ...
  public static void channelConvolution(
      IntArray rgbChannel,
      IntArray channelBlurred,
      final int numRows,
      final int numCols,
      FloatArray filter,
      ...
      for (@Parallel int c = 0; c < numCols; ++c) {
        ...
        for (int filter_r = -filterWidth / 2; filter_r <= filterWidth / 2; ++filter_r) {
          for (int filter_c = -filterWidth / 2; filter_c <= filterWidth / 2; ++filter_c) {
            ...
            float filter_value =
                filter.get((filter_r + filterWidth / 2) * filterWidth + filter_c + filterWidth / 2);
