...
public final class GraphicsKernels {
  ...
  public static void rotateImageStreams(ImageFloat3 output, Matrix4x4Float m, ImageFloat3 input) {
    IntStream.range(0, output.X() * output.Y())
        .parallel()
        .forEach(
            (int index) -> {
  ...
  public static void convolveImageArray(
      final FloatArray input,
      final FloatArray filter,
      final FloatArray output,
      final int iW,
      final int iH,
      final int fW,
      final int fH) {
          ...
          for (u = 0; u < fW; u++) {
            if ((((y - filterY2) + v) >= 0) && ((y + v) < iH)) {
              if ((((x - filterX2) + u) >= 0) && ((x + u) < iW)) {
                sum +=
                    filter.get((v * fW) + u)
                        * input.get((((y - filterY2) + v) * iW) + ((x - filterX2) + u));
              }
            }
          }
        }
  ...
  public static void convolveImage(
      final ImageFloat input, final ImageFloat filter, final ImageFloat output) {
    int u;
    int v;
  ...
  public static void convolveImageStreams(
      final ImageFloat input, final ImageFloat filter, final ImageFloat output) {
    final int filterX2 = filter.X() / 2;
    final int filterY2 = filter.Y() / 2;
    IntStream.range(0, output.X() * output.Y())
