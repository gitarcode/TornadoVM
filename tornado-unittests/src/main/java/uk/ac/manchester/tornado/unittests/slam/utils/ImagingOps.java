...
public class ImagingOps {
  ...
  public static void resizeImage6(
      ImageFloat dest, ImageFloat src, int scaleFactor, float eDelta, int radius) {
  ...
  public static void bilateralFilter(
      ImageFloat dest, ImageFloat src, FloatArray gaussian, float eDelta, int radius) {
          ...
          for (int yy = -radius; yy <= radius; yy++) {
            for (int xx = -radius; xx <= radius; xx++) {
              ...
              if (currentPixel > 0f) {
                ...
                final float factor =
                    (gaussian.get(xx + radius)
                        * gaussian.get(yy + radius)
                        * exp(-mod / eDSquared2));
