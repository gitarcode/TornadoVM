...
public class TestCase extends TornadoTestBase {
  ...
  private static void KMeansCalculateCentroids(
      ShortArray cache_dqsize,
      IntArray cache_dstart,
      IntArray cache_dqid,
      FloatArray cache_dqtfidf,
      FloatArray cache_kmeans,
      IntArray doc_group,
      IntArray sizes) {
    int N = sizes.get(0);
    int V = sizes.get(1);
  ...
  public void test() throws TornadoExecutionPlanException {
    ...
    TaskGraph taskGraph =
        new TaskGraph("foo") //
            ...
            .task(
                "bar",
                TestCase::KMeansCalculateCentroids,
                cache_dqsize,
                cache_dstart,
                cache_dqid,
                cache_dqtfidf,
