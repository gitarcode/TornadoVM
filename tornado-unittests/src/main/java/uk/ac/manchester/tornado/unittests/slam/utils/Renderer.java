...
public class Renderer {
  ...
  public static void renderLight(
      ImageByte4 output, ImageFloat3 verticies, ImageFloat3 normals, Float3 light, Float3 ambient) {
  ...
  public static void renderVolume(
    ...
    for (@Parallel int y = 0; y < output.Y(); y++) {
      for (@Parallel int x = 0; x < output.X(); x++) {
        final Float4 hit =
            raycastPoint(volume, volumeDims, x, y, view, nearPlane, farPlane, smallStep, largeStep);
        final Byte4 pixel;
        if (hit.getW() > 0) {
  ...
  public static void renderDepth(
      ImageByte4 output, ImageFloat depthMap, float nearPlane, float farPlane) {
