...
public class GraphicsMath {
    ...
    for (@Parallel int y = 0; y < normals.Y(); y++) {
      for (@Parallel int x = 0; x < normals.X(); x++) {
        ...
        boolean invalidNormal =
            left.getZ() == 0 || right.getZ() == 0 || up.getZ() == 0 || down.getZ() == 0;
        final Float3 normal;
        if (invalidNormal) {
  ...
  public static Float3 rigidTransform(Matrix4x4Float matrix, Float3 point) {
    ...
    final Float3 rotation =
        new Float3(
            dot(matrix.row(0).asFloat3(), point),
            dot(matrix.row(1).asFloat3(), point),
            dot(matrix.row(2).asFloat3(), point));
    return add(rotation, translation);
  }

  public static Float4 raycastPoint(
      final VolumeShort2 volume,
      final Float3 dim,
      final int x,
      final int y,
