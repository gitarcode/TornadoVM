...
/**
 * This abstract sealed class represents the common functionality of the TornadoVM custom native
 * arrays, (e.g., {@link ByteArray}, {@link IntArray}, etc.)
 *
 * <p>The class provides methods for retrieving the number of elements stored in the native arrays,
 * for obtaining the underlying memory segment, for clearing the data, for calculating the total
 * number of bytes occupied by the memory segment, and for getting the number of bytes, excluding
 * the array header size.
 *
 * <p>The constant {@link ARRAY_HEADER} represents the size of the header in bytes.
 */
...
permits ByteArray,
  ...
  public static final long ARRAY_HEADER =
      Long.parseLong(System.getProperty("tornado.panama.objectHeader", "24"));
  ...
  /**
   * Returns the underlying {@link MemorySegment} of the native array, without the Tornado Array
   * header.
   *
  ...
  /**
   * Returns the total number of bytes that the {@link MemorySegment} occupies, including the header
   * bytes.
   *
