...
/**
 * A {@link TaskGraph} is encapsulated in this class and all actions over a task graph are coded
 * from this class. For instance, execution.
 *
 * <p>This class does not allow a task-graph to mutate (e.g., add/remove tasks or data from the
 * graph itself). To mutate a graph, we need to create a new {@link ImmutableTaskGraph} object from
 * a mutated {@TaskGraph}. Note that, developers can mutate task-graph objects (of type
 * {@TaskGraph}) without affecting the execution of graphs encapsulated in {@link
 * ImmutableTaskGraph}.
 *
