...
/**
 * Data Range Class provides a set of utilities to perform partial copies between the accelerator
 * (e.g., a GPU) and the CPU.
 *
 * @since v1.0.1
 */
  ...
  public void materialize() {
    ...
    if ((partialSizeMaterialized + offsetMaterialized) > (totalSizeInBytes + arrayHeader)) {
      throw new TornadoRuntimeException(
          "[Error] Partial Copy size is larger than the array size. Check the value passed to the"
              + " `withSize` method");
    }
