...
/**
 * Object that stores all information related to profiling an executor. To be able to return all
 * timers, developers must enable the profiler from the command line using the option:
 *
 * <p><code>
 * --enableProfiler <console|silent>
 * </code>
 *
 * <p>Alternatively, the profiler can be enabled using the {@link
...
public class TornadoProfilerResult implements ProfilerInterface {
  ...
  /**
   * Returns Tornado JIT Compilation time, in ns, (from Java bc to final step in Graal LIR) for all
   * immutable task-graphs associated to the executor.
   *
   * @return long
   */
  @Override
  ...
  /**
   * Returns the compilation time (in ns)that took the device driver (e.g., OpenCL driver) to create
   * the device binary (e.g., from OpenCL C to binary, or from SPIR-V to binary).
   *
   * @return long
   */
  @Override
  ...
  /**
   * Returns the total data transfer time (in ns) for all immutable task-graphs to perform copies
  ...
  /**
   * Return the total time for all immutable task-graphs that took to send data to the device (host
   * -> device).
   *
   * @return long
   */
  ...
  /**
   * Return the total time (in ns) for all immutable task-graphs that took to receive data to the
   * host (device -> host).
   *
   * @return long
   */
  ...
  /**
   * Returns the total time (in ns) that took for all immutable task-graphs to dispatch the command
   * to send and receive data. This depends on the driver implementation.
   *
   * @return long
   */
  @Override
  ...
  /**
   * Returns the total time (in ns) that took all kernels to be dispatched by the driver. This is
  ...
  /**
   * Returns the total time (in ns) for all immutable task-graphs to run the kernel. This metric is
   * from the driver.
   *
   * @return long
   */
  ...
  /**
   * Returns the total number of bytes that were transferred to the hardware accelerator (host to
   * device) for the current execution of the execution plan.
   *
  ...
  /**
   * Returns the total number of bytes that were transferred to the host (device to host) for the
   * current execution of the execution plan.
   *
   * @return long Number of bytes
   */
  @Override
