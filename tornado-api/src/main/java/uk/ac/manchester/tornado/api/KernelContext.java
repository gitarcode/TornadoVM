...
/**
 * Context of TornadoVM execution to exploit kernel-parallel applications, in which the parallelism
 * is implicit.
 *
 * <p>The application can access thread-id for 1D, 2D and 3D dimensions. Additionally, the
 * application can access local memory (OpenCL terminology), or shared memory (CUDA terminology) as
 * well as synchronization primitives such as barriers.
 *
 * <p>
 *
 * <ul>
 *   <li>{@link KernelContext} is an object exposed by the TornadoVM API in order to leverage
 *       low-level programming features provided by heterogeneous frameworks (e.g. OpenCL, CUDA) to
 *       the developers, such as thread-id, access to local/shared memory and barriers.
 *   <li>{@link KernelContext} provides a Java API that is transparently translated to both OpenCL
 *       and PTX by the TornadoVM JIT compiler. The main difference with the {@link TaskGraph} API
 *       is that the tasks within a {@link TaskGraph} that use {@link KernelContext} must be {@link
 *       GridScheduler}.
 * </ul>
 */
public class KernelContext implements ExecutionContext {
   ...
   *
   * <p>OpenCL equivalent: get_global_id(0);
   *
   * <p>PTX equivalent: blockIdx.x * blockDim.x + threadIdx.x
   */
   ...
   *
   * <p>OpenCL equivalent: get_global_id(1);
   *
   * <p>PTX equivalent: blockIdx.y * blockDim.y + threadIdx.y
   */
   ...
   *
   * <p>OpenCL equivalent: get_global_id(2);
   *
   * <p>PTX equivalent: blockIdx.z * blockDim.z + threadIdx.z
   */
   ...
   *
   * <p>OpenCL equivalent: get_global_size();
   *
   * <p>PTX equivalent: gridDim * blockDim
   */
   ...
   * It returns the global group size of a particular dimension (e.g. X, Y, Z).
   *
   * <p>OpenCL equivalent: get_local_size();
   *
   * <p>PTX equivalent: blockDim
   */
  ...
  /**
   * Method used as a barrier to synchronize the order of memory operations to the local memory
   * (known as shared memory in PTX).
   *
   * <p>OpenCL equivalent: barrier(CLK_LOCAL_MEM_FENCE);
   *
   * <p>PTX equivalent: barrier.sync;
   */
  @Override
   ...
   *
   * <p>OpenCL equivalent: barrier(CLK_GLOBAL_MEM_FENCE);
   *
   * <p>PTX equivalent: barrier.sync;
   */
  @Override
