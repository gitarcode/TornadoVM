...
/**
 * Generic interface for TornadoVM to implement a Thread-Context API. This interface allows the
 * client to implement barriers, allocate memory in local memory (OpenCL) or in shared memory
 * (NVIDIA PTX).
 */
public interface ExecutionContext {

  /**
   * It adds a barrier for sync all threads within the same work-group to ensure correct order in
   * local memory (OpenCL terminology).
   *
   * <p>Similar the OpenCL construct: <code>
  ...
  /**
   * It adds a barrier for sync all threads within the same work-group to ensure correct order in
   * global memory (OpenCL terminology).
   *
   * <p>Similar the OpenCL construct: <code>
