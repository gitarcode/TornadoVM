...
public class BlackScholesMT {
  ...
  private static void blackScholesKernelThreaded(
      FloatArray input, FloatArray callResult, FloatArray putResult, int threads, Thread[] th)
          ...
          new Thread(
              () -> {
                for (int idx = lowBound; idx < finalUpperBound; idx++) {
                  ...
                  float d1 =
                      (float) ((Math.log(S / K) + ((r + (v * v / 2)) * T)) / v * Math.sqrt(T));
  ...
  public static void blackScholes(int size, int iterations, String executionType)
      throws InterruptedException {
