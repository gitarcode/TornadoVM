...
public class MatrixMultiplication2D {
  ...
  private static void parallelStreamsMxM(
      Matrix2DFloat A, Matrix2DFloat B, Matrix2DFloat C, final int size) {
  ...
  public static void main(String[] args)
      throws TornadoExecutionPlanException, FileNotFoundException {
    ...
    TaskGraph taskGraph =
        new TaskGraph("s0") //
            ...
            .task(
                "t0",
                MatrixMultiplication2D::matrixMultiplication,
                matrixA,
                matrixB,
                matrixC,
                size) //
    ...
    System.out.println(
        "\tStreams Execution: "
            + formatStreamFGlops
            + " GFlops, Total time = "
            + (nanoSecStreamElaptedTime)
            + " ns");
    System.out.println(
        "\tTornadoVM Execution on "
            + deviceType
            + " (Accelerated): "
            + formatGPUFGlops
            + " GFlops, Total Time = "
    ...
    for (int i = 0; i < javaElapsedTime.size(); i++) {
      fileWriter.println(
          javaElapsedTime.get(i)
              + ","
              + streamsElapsedTime.get(i)
              + ","
              + tornadoElapsedTime.get(i));
    }
    fileWriter.close();
  }
