...
public class MatrixVector {
  ...
  private static void computeWithVectors(
  ...
  private static void runWithVectorTypes(int size, TornadoDevice device) {
    ...
    IntStream.range(0, size)
        .forEach(
            idx ->
                vectorFloat.set(
                    idx, new Float4(r.nextFloat(), r.nextFloat(), r.nextFloat(), r.nextFloat())));
    IntStream.range(0, size)
        .forEach(
            idx ->
                IntStream.range(0, s) //
  ...
  private static void computeWithStreams(
      final int size, Matrix2DFloat matrix, VectorFloat vector, VectorFloat output) {
    IntStream.range(0, size)
        .parallel()
        .forEach(
            i -> {
