...
public class DFTMT {
  ...
  public static void computeDFT(
    ...
    for (@Parallel int k = 0; k < n; k++) { // For each output element
      ...
      for (int t = 0; t < n; t++) { // For each input element
        ...
        sumimag +=
            -(inreal.get(t) * (TornadoMath.sin(angle)) + inimag.get(t) * (TornadoMath.cos(angle)));
      }
  ...
  public static void computeDFTThreads(
      FloatArray inreal,
      FloatArray inimag,
      FloatArray outreal,
      FloatArray outimag,
              ...
              () -> {
                for (int k = lowBound; k < finalUpperBound; k++) {
                  ...
                  for (int t = 0; t < inreal.getSize(); t++) { // For each input element
                    ...
                    sumreal +=
                        (float)
                            (inreal.get(t) * (Math.cos(angle)) + inimag.get(t) * (Math.sin(angle)));
                    sumimag +=
                        -(float)
                            (inreal.get(t) * (Math.sin(angle)) + inimag.get(t) * (Math.cos(angle)));
                  }
                  outreal.set(k, sumreal);
                  outimag.set(k, sumimag);
                }
