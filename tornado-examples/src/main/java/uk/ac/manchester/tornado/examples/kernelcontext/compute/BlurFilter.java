 ...
 *
 * <p>Example borrowed from the Marawacc parallel programming framework with the permission from the
 * author.
 *
 * <p>How to run? <code>
...
public class BlurFilter {
  ...
  public static class BlurFilterImage extends Component {
    ...
    public static final boolean PARALLEL_COMPUTATION =
        Boolean.parseBoolean(System.getProperty("run:parallel", "True"));
    ...
    private static void channelConvolutionSequential(
        IntArray channel,
      ...
      for (int i = 0; i < w; i++) {
        for (int j = 0; j < h; j++) {
          Color c =
              new Color(
                  redFilter.get(i * h + j),
                  greenFilter.get(i * h + j),
                  blueFilter.get(i * h + j),
                  alphaChannel.get(i * h + j));
