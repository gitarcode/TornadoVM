...
/**
 * Example of Matrix Multiplication of two-dimensional arrays using Local Memory and Loop Tiling.
 *
 * <p>This program requires both OpenCL and PTX backends to be built. It compares the following
 * implementations against the Java functionally equivalent code.
 *
 * <p>a) CUDA/OpenCL Old API: using the TaskSchedule API and the @Parallel annotations to express
 * loop parallelism.
 *
 * <p>b) CUDA/OpenCL Advanced API: using a {@link KernelContext} to express Threading Attributes,
 * Local Memory Allocation and Loop Tiling.
 *
 * <p>How to run: <code>
...
public class MatrixMul2DLocalMemory {
  ...
  /**
   * Example of Matrix Multiplication with Local Memory and Loop Tiling. The tiles are flattened in
   * one dimension.
   *
   * <p>The algorithm of this method follows the implementation in:
   * https://github.com/cnugteren/myGEMM.
   */
  public static void matrixMultiplicationLocalMemory(
  ...
  public static void main(String[] args) throws Exception {
    ...
    TaskGraph scheduleCUDA =
        new TaskGraph("cuda_old_api") //
            .transferToDevice(DataTransferMode.FIRST_EXECUTION, matrixA, matrixB) //
            .task(
                "t0",
                MatrixMul2DLocalMemory::matrixMultiplication,
                matrixA,
                matrixB,
                matrixCCUDA,
                N) //
    ...
    TaskGraph scheduleOCL =
        new TaskGraph("ocl_old_api") //
            .transferToDevice(DataTransferMode.FIRST_EXECUTION, matrixA, matrixB) //
            .task(
                "t0",
                MatrixMul2DLocalMemory::matrixMultiplication,
    ...
    for (int i = 0; i < oclDriver.getNumDevices(); i++) {
      ...
      if (device
          .getPhysicalDevice()
          .getDeviceName()
          .equalsIgnoreCase(cudaDevice.getPhysicalDevice().getDeviceName())) {
        oclDevice = device;
      }
    }
    if (oclDevice == null) {
    ...
    GridScheduler gridSchedulerOpenCLNew =
        new GridScheduler("ocl_advanced_api.t0", workerOpenCLNew);
    KernelContext context = new KernelContext();

    TaskGraph oclNewApiTask =
        new TaskGraph("ocl_advanced_api") //
            .transferToDevice(DataTransferMode.FIRST_EXECUTION, matrixA, matrixB) //
            .task(
                "t0",
                MatrixMul2DLocalMemory::matrixMultiplicationLocalMemory,
                context,
                matrixA,
                matrixB,
                matrixCOCLNewApi,
    ...
    TaskGraph cudaNewApiTask =
        new TaskGraph("cuda_advanced_api") //
            .transferToDevice(DataTransferMode.FIRST_EXECUTION, matrixA, matrixB) //
            .task(
                "t0",
                MatrixMul2DLocalMemory::matrixMultiplicationLocalMemory,
    ...
    if (avgCUDAOptionalNewApi.isPresent()) averageCUDANewApi = avgCUDAOptionalNewApi.getAsDouble();
    else throw new Exception("Could not get average execution time");
    ...
    if (CHECK_RESULT) {
      for (int i = 0; i < N * N; i++) {
        ...
        if (Math.abs(matrixCCUDANewApi.get(i) - matrixCSeq.get(i)) > DELTA) {
          ...
          System.out.println(
              "Result is ("
                  + matrixCCUDANewApi.get(i)
                  + ") - while should be ("
                  + matrixCSeq.get(i)
                  + ")");
        }
        correctResult =
            validationCUDA && validationOCL && validationOCLNewApi && validationCUDANewApi;
    ...
    System.out.println(
        "\tOpenCL Execution: "
            ...
            + " ms");
    System.out.println(
        "\tOpenCL Execution with Local Memory and Loop Tiling: "
            + formatOpenCLNewApiFGlops
            + " GFlops, Total time = "
            + averageOpenCLNewApi
            + " ms");
    System.out.println(
        "\tPTX Execution: " + formatCUDAFGlops + " GFlops, Total Time = " + averageCUDA + " ms");
    System.out.println(
        "\tPTX Execution with Local Memory and Loop Tiling: "
            + formatCUDANewApiFGlops
            + " GFlops, Total time = "
            + averageCUDANewApi
            + " ms");
    System.out.println("\tOpenCL Speedup: " + OpenCLspeedup + "x");
    System.out.println(
        "\tOpenCL Speedup with Local Memory and Loop Tiling: " + OpenCLNewApispeedup + "x");
    System.out.println("\tPTX Speedup: " + CUDAspeedup + "x");
    System.out.println(
        "\tPTX Speedup with Local Memory and Loop Tiling: " + CUDANewApispeedup + "x");
    System.out.println();
  }
}
// CHECKSTYLE:ON
