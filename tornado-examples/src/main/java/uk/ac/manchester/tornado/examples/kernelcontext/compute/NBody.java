...
/**
 * Montecarlo algorithm to approximate the PI value. This version has been adapted from Marawacc
 * test-suite.
 *
 * <p>How to run? <code>
...
public class NBody {
  ...
  private static void nBody(
      KernelContext context, int numBodies, FloatArray refPos, FloatArray refVel) {
    ...
    for (int k = 0; k < 3; k++) {
      refPos.set(
          body + k,
          refPos.get(body + k) + refVel.get(body + k) * DELT + 0.5f * acc[k] * DELT * DELT);
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        "Running Nbody with " + numBodies + " bodies" + " and " + iterations + " iterations");
    ...
    for (int i = 0; i < iterations; i++) {
      ...
      resultsIterations.append(
          "\tSequential execution time of iteration " + i + "is: " + (end - start) + " ns");
      resultsIterations.append("\n");
    }
    ...
    for (int i = 0; i < iterations; i++) {
      ...
      resultsIterations.append(
          "\tTornado execution time of iteration " + i + " is: " + (end - start) + " ns");
      resultsIterations.append("\n");
    }
